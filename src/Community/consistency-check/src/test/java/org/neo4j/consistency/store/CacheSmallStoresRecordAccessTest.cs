...
//ORIGINAL LINE: @SuppressWarnings("unchecked") private static <T extends org.neo4j.kernel.impl.store.record.AbstractBaseRecord> org.hamcrest.Matcher<RecordReference<T>> isDirectReferenceTo(T record)
		 private static Matcher<RecordReference<T>> IsDirectReferenceTo<T>( T record ) where T : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
			  return ( Matcher ) new DirectReferenceMatcher<RecordReference<T>>( record );
		 }

		 private class DirectReferenceMatcher<T> : TypeSafeMatcher<DirectRecordReference<T>> where T : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
