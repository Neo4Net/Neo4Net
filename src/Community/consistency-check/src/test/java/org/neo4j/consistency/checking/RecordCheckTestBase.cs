...
//	import static org.mockito.Mockito.mock;
	...
	public abstract class RecordCheckTestBase<RECORD, REPORT, CHECKER> where RECORD : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord where REPORT : Neo4Net.Consistency.report.ConsistencyReport where CHECKER : RecordCheck<RECORD, REPORT>
	{
...
//ORIGINAL LINE: public static <RECORD extends org.neo4j.kernel.impl.store.record.AbstractBaseRecord, REPORT extends org.neo4j.consistency.report.ConsistencyReport> REPORT check(Class<REPORT> reportClass, RecordCheck<RECORD, REPORT> checker, RECORD record, final org.neo4j.consistency.store.RecordAccessStub records)
		 public static REPORT Check<RECORD, REPORT>( Type reportClass, RecordCheck<RECORD, REPORT> checker, RECORD record, RecordAccessStub records ) where RECORD : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord where REPORT : Neo4Net.Consistency.report.ConsistencyReport
		 {
				 reportClass = typeof( REPORT );
...
//ORIGINAL LINE: public static <RECORD extends org.neo4j.kernel.impl.store.record.AbstractBaseRecord, REPORT extends org.neo4j.consistency.report.ConsistencyReport> void check(REPORT report, RecordCheck<RECORD, REPORT> checker, RECORD record, final org.neo4j.consistency.store.RecordAccessStub records)
		 public static void Check<RECORD, REPORT>( REPORT report, RecordCheck<RECORD, REPORT> checker, RECORD record, RecordAccessStub records ) where RECORD : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord where REPORT : Neo4Net.Consistency.report.ConsistencyReport
		 {
			  checker.Check( record, records.Engine( record, report ), records );
			  records.CheckDeferred();
		 }

		 internal virtual R AddChange<R>( R oldRecord, R newRecord ) where R : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
			  return Records.addChange( oldRecord, newRecord );
		 }

		 internal virtual R Add<R>( R record ) where R : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
		 ...
		 public static R InUse<R>( R record ) where R : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
			  record.InUse = true;
			  return record;
		 }

		 public static R NotInUse<R>( R record ) where R : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
			  record.InUse = false;
			  return record;
		 }
