...
//	import static org.mockito.Mockito.verifyNoMoreInteractions;
	...
	{
		 internal NodeRecordCheckTest() : base(new NodeRecordCheck(RelationshipField.NEXT_REL, LabelsField.LABELS, new PropertyChain<>(from -> null)), typeof(Neo4Net.consistency.report.ConsistencyReport_NodeConsistencyReport), new int[0])
		 {
		 }
...
//ORIGINAL LINE: final Neo4Net.kernel.impl.store.record.NodeRecord node = inUse(new Neo4Net.kernel.impl.store.record.NodeRecord(42, false, NONE, NONE));
			  NodeRecord node = InUse( new NodeRecord( 42, false, NONE, NONE ) );
