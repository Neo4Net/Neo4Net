...
//JAVA TO C# CONVERTER TODO TASK: This Java 'import static' statement cannot be converted to C#:
//	import static Neo4Net.helpers.collection.Iterables.resourceIterable;
...
//ORIGINAL LINE: public <RECORD extends Neo4Net.kernel.impl.store.record.AbstractBaseRecord, REPORT extends Neo4Net.consistency.report.ConsistencyReport> Neo4Net.consistency.checking.CheckerEngine<RECORD, REPORT> engine(final RECORD record, final REPORT report)
...
//ORIGINAL LINE: @Override @SuppressWarnings("unchecked") void checkReference(Neo4Net.consistency.checking.ComparativeRecordChecker checker, Neo4Net.kernel.impl.store.record.AbstractBaseRecord oldReference, Neo4Net.kernel.impl.store.record.AbstractBaseRecord newReference)
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are ignored unless the option to convert to C# 7.2 'in' parameters is selected:
//ORIGINAL LINE: public <RECORD extends Neo4Net.kernel.impl.store.record.AbstractBaseRecord, REPORT extends Neo4Net.consistency.report.ConsistencyReport> Neo4Net.consistency.checking.CheckerEngine<RECORD, REPORT> engine(final RECORD oldRecord, final RECORD newRecord, REPORT report)
...
//ORIGINAL LINE: @Override @SuppressWarnings("unchecked") void checkReference(Neo4Net.consistency.checking.ComparativeRecordChecker checker, Neo4Net.kernel.impl.store.record.AbstractBaseRecord oldReference, Neo4Net.kernel.impl.store.record.AbstractBaseRecord newReference)
		 ...
		 private abstract class Engine<RECORD, REPORT> : CheckerEngine<RECORD, REPORT> where RECORD : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord where REPORT : Neo4Net.Consistency.report.ConsistencyReport
		 {
			 private readonly RecordAccessStub _outerInstance;
...
//ORIGINAL LINE: public <REFERRED extends Neo4Net.kernel.impl.store.record.AbstractBaseRecord> void comparativeCheck(final RecordReference<REFERRED> other, final Neo4Net.consistency.checking.ComparativeRecordChecker<RECORD, ? super REFERRED, REPORT> checker)
//JAVA TO C# CONVERTER WARNING: 'final' parameters are ignored unless the option to convert to C# 7.2 'in' parameters is selected:
		 ...
		 public RecordAccessStub() : this(Neo4Net.consistency.checking.full.Stage_Fields.SequentialForward, MultiPassStore.values())
		 {
		 }
		 ...
		 private class Delta<R> where R : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
...
//           NEW { <R> R get(Delta<R> delta) { return delta.oldRecord == null ? null : delta.newRecord; } };
		 ...
		 private static R Add<R>( IDictionary<long, Delta<R>> records, R record ) where R : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
			  records[record.Id] = new Delta<R>( record );
			  return record;
		 }

		 private static void Add<R>( IDictionary<long, Delta<R>> records, R oldRecord, R newRecord ) where R : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
		 ...
		 public virtual R AddChange<R>( R oldRecord, R newRecord ) where R : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
		 ...
		 public virtual R Add<R>( R record ) where R : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
		 ...
		 private DirectRecordReference<R> Reference<R>( IDictionary<long, Delta<R>> records, long id, Version version ) where R : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
			  return new DirectRecordReference<R>( Record( records, id, version ), this );
		 }

		 private static R Record<R>( IDictionary<long, Delta<R>> records, long id, Version version ) where R : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are ignored unless the option to convert to C# 7.2 'in' parameters is selected:
//ORIGINAL LINE: public java.util.Iterator<Neo4Net.kernel.impl.store.record.PropertyRecord> rawPropertyChain(final long firstId)
