...
//	import static Neo4Net.helpers.Exceptions.stringify;
	...
	public class ConsistencyReporter : ConsistencyReport_Reporter
	{
		 ...
		 private void Dispatch<RECORD, REPORT>( RecordType type, ProxyFactory<REPORT> factory, RECORD record, RecordCheck<RECORD, REPORT> checker ) where RECORD : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord where REPORT : ConsistencyReport
		 {
		 ...
		 public virtual REPORT Report<RECORD, REPORT>( RECORD record, Type cls, RecordType recordType ) where RECORD : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord where REPORT : ConsistencyReport
		 {
				 cls = typeof( REPORT );
		 ...
		 public abstract class ReportInvocationHandler <RECORD, REPORT> : CheckerEngine<RECORD, REPORT>, InvocationHandler where RECORD : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord where REPORT : ConsistencyReport
		 {
...
//ORIGINAL LINE: public <REFERRED extends Neo4Net.kernel.impl.store.record.AbstractBaseRecord> void comparativeCheck(Neo4Net.consistency.store.RecordReference<REFERRED> reference, Neo4Net.consistency.checking.ComparativeRecordChecker<RECORD, ? super REFERRED, REPORT> checker)
		 ...
		 public class ReportHandler <RECORD, REPORT> : ReportInvocationHandler<RECORD, REPORT> where RECORD : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord where REPORT : ConsistencyReport
		 {
