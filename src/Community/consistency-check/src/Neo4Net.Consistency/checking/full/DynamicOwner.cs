...
//	import static Neo4Net.consistency.store.RecordReference_SkippingReference.skipReference;
	...
	internal abstract class DynamicOwner<RECORD> : Owner where RECORD : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
	{
		 ...
		 internal abstract class NameOwner<RECORD, REPORT> : DynamicOwner<RECORD>, ComparativeRecordChecker<RECORD, AbstractBaseRecord, REPORT> where RECORD : Neo4Net.Kernel.Impl.Store.Records.TokenRecord where REPORT : Neo4Net.Consistency.report.ConsistencyReport_NameConsistencyReport
		 {
...
//ORIGINAL LINE: @SuppressWarnings("ConstantConditions") @Override public void checkReference(RECORD name, Neo4Net.kernel.impl.store.record.AbstractBaseRecord record, Neo4Net.consistency.checking.CheckerEngine<RECORD, REPORT> engine, Neo4Net.consistency.store.RecordAccess records)
