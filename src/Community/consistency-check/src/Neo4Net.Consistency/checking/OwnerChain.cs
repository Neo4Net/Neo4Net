...
//       NEW { RecordReference<Neo4Net.kernel.impl.store.record.PropertyRecord> property(Neo4Net.consistency.store.RecordAccess records, long id) { return records.property(id); } RecordReference<Neo4Net.kernel.impl.store.record.NodeRecord> node(Neo4Net.consistency.store.RecordAccess records, long id) { return records.node(id); } RecordReference<Neo4Net.kernel.impl.store.record.RelationshipRecord> relationship(Neo4Net.consistency.store.RecordAccess records, long id) { return records.relationship(id); } RecordReference<Neo4Net.kernel.impl.store.record.NeoStoreRecord> graph(Neo4Net.consistency.store.RecordAccess records) { return records.graph(); } void wrongOwner(Neo4Net.consistency.report.ConsistencyReport_PropertyConsistencyReport report) { report.ownerDoesNotReferenceBack(); } };
