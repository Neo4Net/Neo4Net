...
//ORIGINAL LINE: final int type = record.getType();
		 ...
		 private class MultiPassAvoidanceCondition<T> : System.Predicate<T> where T : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
		 ...
		 private static ISet<long> LabelsFor<T1>( RecordStore<NodeRecord> nodeStore, CheckerEngine<T1> engine, RecordAccess recordAccess, long nodeId ) where T1 : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
