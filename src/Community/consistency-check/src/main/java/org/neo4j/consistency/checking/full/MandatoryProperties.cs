...
//ORIGINAL LINE: public System.Func<org.neo4j.kernel.impl.store.record.RelationshipRecord,Check<org.neo4j.kernel.impl.store.record.RelationshipRecord,org.neo4j.consistency.report.ConsistencyReport_RelationshipConsistencyReport>> forRelationships(final org.neo4j.consistency.report.ConsistencyReporter reporter)
		 public virtual System.Func<RelationshipRecord, Check<RelationshipRecord, Neo4Net.Consistency.report.ConsistencyReport_RelationshipConsistencyReport>> ForRelationships( ConsistencyReporter reporter )
		 {
			  return relationship =>
			  {
				...
				if ( propertyKeys != null )
				{
					 ...
					 return new RealCheck<Neo4Net.Kernel.impl.store.record.RelationshipRecord, Check<Neo4Net.Kernel.impl.store.record.RelationshipRecord, Neo4Net.Consistency.report.ConsistencyReport_RelationshipConsistencyReport>>( relationship, typeof( ConsistencyReport.RelationshipConsistencyReport ), reporter, RecordType.Relationship, keys );
				}
				return NoCheck();
			  };
		 }
		 ...
		 public interface Check<RECORD, REPORT> : AutoCloseable where RECORD : Neo4Net.Kernel.impl.store.record.PrimitiveRecord where REPORT : Neo4Net.Consistency.report.ConsistencyReport_PrimitiveConsistencyReport
		 {
...
//ORIGINAL LINE: @SuppressWarnings("unchecked") private static <RECORD extends org.neo4j.kernel.impl.store.record.PrimitiveRecord, REPORT extends org.neo4j.consistency.report.ConsistencyReport_PrimitiveConsistencyReport> Check<RECORD,REPORT> noCheck()
		 private static Check<RECORD, REPORT> NoCheck<RECORD, REPORT>() where RECORD : Neo4Net.Kernel.impl.store.record.PrimitiveRecord where REPORT : Neo4Net.Consistency.report.ConsistencyReport_PrimitiveConsistencyReport
		 {
			  return NONE;
		 }
...
//ORIGINAL LINE: @SuppressWarnings("rawtypes") private static final Check NONE = new Check()
		 ...
		 private class RealCheck<RECORD, REPORT> : Check<RECORD, REPORT> where RECORD : Neo4Net.Kernel.impl.store.record.PrimitiveRecord where REPORT : Neo4Net.Consistency.report.ConsistencyReport_PrimitiveConsistencyReport
		 {
