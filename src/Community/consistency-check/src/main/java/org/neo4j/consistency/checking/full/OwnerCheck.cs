...
//ORIGINAL LINE: final java.util.concurrent.ConcurrentMap<long, DynamicOwner> dynamicOwners = dynamicOwners(type);
		 ...
		 private abstract class PrimitiveCheckerDecorator<RECORD, REPORT> : OwningRecordCheck<RECORD, REPORT> where RECORD : Neo4Net.Kernel.impl.store.record.PrimitiveRecord where REPORT : Neo4Net.Consistency.report.ConsistencyReport_PrimitiveConsistencyReport
		 {
			 private readonly OwnerCheck _outerInstance;
...
//ORIGINAL LINE: @Override @SuppressWarnings("unchecked") public void check(RECORD record, org.neo4j.consistency.checking.CheckerEngine<RECORD, REPORT> engine, org.neo4j.consistency.store.RecordAccess records)
		 ...
		 private abstract class NameCheckerDecorator <RECORD, REPORT> : RecordCheck<RECORD, REPORT> where RECORD : Neo4Net.Kernel.impl.store.record.TokenRecord where REPORT : Neo4Net.Consistency.report.ConsistencyReport_NameConsistencyReport
		 {
