...
namespace Neo4Net.Kernel.Api.Impl.Index.collector
{
	...
	{
		 /// <returns> the current value of the main field that is driving the values. </returns>
		 long Current();

		 /// <returns> the value of an additional sidecar field. </returns>
		 /// <exception cref="IllegalStateException"> if no such field is indexed. </exception>
