...
namespace Neo4Net.GraphDb
{
	...
	/// node, an end node and a <seealso cref="IRelationshipType type"/>. You can attach
	...
	/// <para>
	/// <code>
	/// Relationship rel = node.<seealso cref="INode.createRelationshipTo(INode, IRelationshipType) createRelationshipTo"/>( otherNode, MyRels.REL_TYPE );
	/// </code>
	/// </para>
	/// <para>
	...
	/// <pre>
	/// <code>
	/// <seealso cref="INode"/> a = graphDb.<seealso cref="GraphDatabaseService.createNode() createNode"/>();
	/// <seealso cref="INode"/> b = graphDb.<seealso cref="GraphDatabaseService.createNode() createNode"/>();
	/// <seealso cref="IRelationship"/> rel = a.{@link Node#createRelationshipTo(Node, RelationshipType)
	/// createRelationshipTo}( b, <seealso cref="IRelationshipType MyRels.REL_TYPE"/> );
	/// // Now we have: (a) --- REL_TYPE ---&gt; (b)
	/// 
	/// assert rel.<seealso cref="IRelationship.getStartNode() getStartNode"/>().equals( a );
	/// assert rel.<seealso cref="IRelationship.getEndNode() getEndNode"/>().equals( b );
	/// assert rel.<seealso cref="IRelationship.getNodes() getNodes"/>()[0].equals( a ) &amp;&amp;
	///        rel.<seealso cref="IRelationship.getNodes() getNodes"/>()[1].equals( b );
	/// </code>
	/// </pre>
	/// 
	...
	/// </para>
	/// <para>
	/// Furthermore, Neo4Net guarantees that a relationship is never "hanging freely,"
	/// i.e. <seealso cref="getStartNode()"/>, <seealso cref="getEndNode()"/>,
	/// <seealso cref="getOtherNode(INode)"/> and <seealso cref="getNodes()"/> are guaranteed to always
	/// return valid, non-null nodes.
	/// </para>
	/// <para>
	/// A relationship's id is unique, but note the following: Neo4Net reuses its internal ids
	...
	public interface IRelationship : IEntity
	{
		 ...
		 /// <seealso cref="INode.getRelationships() relationship accessors"/> in Node, see the
		 /// class documentation of Relationship.
		 /// </summary>
		 ...
		 /// by invoking one of the <seealso cref="INode.getRelationships() getRelationships()"/>
		 ...
		 /// <seealso cref="INode.createRelationshipTo creation"/>. Remember that relationship
		 /// types are semantically equivalent if their
		 /// <seealso cref="IRelationshipType.name() names"/> are {@link Object#equals(Object)
		 ...
		 /// certain type, use the <seealso cref="isType(IRelationshipType) isType()"/>
		 /// operation.
		 /// </summary>
