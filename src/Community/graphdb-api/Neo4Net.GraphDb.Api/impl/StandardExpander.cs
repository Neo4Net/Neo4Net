	...
	public abstract class StandardExpander : IPathExpander
	{
		 ...
		 internal abstract class StandardExpansion<T> : ResourceIterable<T>
		 {
			 ...
			 public abstract ResourceIterator<T> Iterator();
			  ...
			  internal readonly IPath Path;
			  internal readonly BranchState State;

			  internal StandardExpansion( StandardExpander expander, IPath path, BranchState state )
			  {
...
//ORIGINAL LINE: public StandardExpansion<T> filterRelationships(System.Predicate<? super org.Neo4Net.graphdb.Relationship> filter)
	...
	//JAVA TO C# CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
		 ...
		 {
			  internal RelationshipExpansion( StandardExpander expander, IPath path, BranchState state ) : base( expander, path, state )
			  {
			  }
		 ...
		 {
			  internal NodeExpansion( StandardExpander expander, IPath path, BranchState state ) : base( expander, path, state )
			  {
			  }
