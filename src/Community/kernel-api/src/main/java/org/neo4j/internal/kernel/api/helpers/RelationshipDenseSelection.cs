...
namespace Neo4Net.@internal.Kernel.Api.helpers
{
	...
	internal abstract class RelationshipDenseSelection
	{
		 ...
		 private void SetupCursors()
		 {
			  ...
			  switch ( d )
			  {
			  case Neo4Net.@internal.Kernel.Api.helpers.RelationshipDenseSelection.Dir.Out:
					_groupCursor.outgoing( RelationshipCursor );
					_onRelationship = RelationshipCursor.next();
					break;

			  case Neo4Net.@internal.Kernel.Api.helpers.RelationshipDenseSelection.Dir.In:
					_groupCursor.incoming( RelationshipCursor );
					_onRelationship = RelationshipCursor.next();
					break;

			  case Neo4Net.@internal.Kernel.Api.helpers.RelationshipDenseSelection.Dir.Loop:
