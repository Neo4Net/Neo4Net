...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 internal virtual void TestAdminUsage()
		 {
			  ...
			  using ( MemoryStream baos = new MemoryStream() )
			  {
					...
					Usage usage = new Usage( "Neo4Net-admin", commandLocator );

					HelpCommand helpCommand = new HelpCommand( usage, ps.println, commandLocator );

					helpCommand.Execute();

					assertEquals( string.Format( "usage: Neo4Net-admin <command>%n" + "%n" + "Manage your Neo4Net instance.%n" + "%n" + "environment variables:%n" + "    Neo4Net_CONF    Path to directory which contains Neo4Net.conf.%n" + "    Neo4Net_DEBUG   Set to anything to enable debug output.%n" + "    Neo4Net_HOME    Neo4Net home directory.%n" + "    HEAP_SIZE     Set JVM maximum heap size during command execution.%n" + "                  Takes a number and a unit, for example 512m.%n" + "%n" + "available commands:%n" + "%n" + "General%n" + "    bar%n" + "        null%n" + "    baz%n" + "        null%n" + "    foo%n" + "        null%n" + "%n" + "Use Neo4Net-admin help <command> for more details.%n" ), baos.ToString() );
			  }
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 internal virtual void ShowsArgumentsAndDescriptionForSpecifiedCommand()
		 {
			  ...
			  using ( MemoryStream baos = new MemoryStream() )
			  {
					...
					HelpCommand helpCommand = new HelpCommand( new Usage( "Neo4Net-admin", commandLocator ), ps.println, commandLocator );
					helpCommand.Execute( "foobar" );

					assertEquals( string.Format( "usage: Neo4Net-admin foobar [--database=<name>]%n" + "%n" + "environment variables:%n" + "    Neo4Net_CONF    Path to directory which contains Neo4Net.conf.%n" + "    Neo4Net_DEBUG   Set to anything to enable debug output.%n" + "    Neo4Net_HOME    Neo4Net home directory.%n" + "    HEAP_SIZE     Set JVM maximum heap size during command execution.%n" + "                  Takes a number and a unit, for example 512m.%n" + "%n" + "This is a description of the foobar command.%n" + "%n" + "options:%n" + "  --database=<name>   Name of database. [default:" + GraphDatabaseSettings.DEFAULT_DATABASE_NAME + "]%n" ), baos.ToString() );
			  }
		 }
