...
//	import static Neo4Net.commandline.dbms.DiagnosticsReportCommand.PID_KEY;
	...
	public class JMXDumper
	{
		 ...
		 public virtual Optional<JmxDump> JMXDump
		 {
			 get
			 {
				  @out.println( "Finding running instance of Neo4Net" );
				  ...
				  {
						@out.println( "No running instance of Neo4Net was found. Online reports will be omitted." );
						@out.println( "If Neo4Net is running but not detected, you can supply the process id of the running instance with --" + PID_KEY );
						return null;
				  }
			 }
		 }
		 ...
		 private long? Pid
		 {
			 get
			 {
				  Path pidFile = this._homeDir.resolve( "run/Neo4Net.pid" );
