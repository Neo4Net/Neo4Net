...
namespace Neo4Net.Kernel.impl.transaction.command
{
	...
	public class NeoStoreTransactionApplier : Neo4Net.Kernel.Impl.Api.TransactionApplier_Adapter
	{
		 ...
		 private void UpdateStore<RECORD>( RecordStore<RECORD> store, BaseCommand<RECORD> command ) where RECORD : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
			  store.UpdateRecord( SelectRecordByCommandVersion( command ) );
		 }

		 private RECORD SelectRecordByCommandVersion<RECORD>( BaseCommand<RECORD> command ) where RECORD : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
			  switch ( _version )
			  {
