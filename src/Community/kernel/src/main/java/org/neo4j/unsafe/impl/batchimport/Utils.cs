...
namespace Neo4Net.@unsafe.Impl.Batchimport
{
	...
	public class Utils
	{
		 ...
		 { // works for signed and unsigned values
			  switch ( compareType )
			  {
			  case Neo4Net.@unsafe.Impl.Batchimport.Utils.CompareType.Eq:
					return dataA == dataB;
			  case Neo4Net.@unsafe.Impl.Batchimport.Utils.CompareType.Ge:
					if ( dataA == dataB )
					{
						 return true;
					}
					// fall through to GT
			  case Neo4Net.@unsafe.Impl.Batchimport.Utils.CompareType.Gt:
					return dataA < dataB == ( ( dataA < 0 ) != ( dataB < 0 ) );
			  case Neo4Net.@unsafe.Impl.Batchimport.Utils.CompareType.Le:
					if ( dataA == dataB )
					{
						 return true;
					}
					// fall through to LT
			  case Neo4Net.@unsafe.Impl.Batchimport.Utils.CompareType.Lt:
					return ( dataA < dataB ) ^ ( ( dataA < 0 ) != ( dataB < 0 ) );
			  case Neo4Net.@unsafe.Impl.Batchimport.Utils.CompareType.Ne:
					return false;
