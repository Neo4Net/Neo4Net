...
{
	/// @deprecated since from 2.2.4 and onwards there's only one version in town, namely <seealso cref="LogEntryVersion"/>.
	...
	/// reading log headers. 
	[Obsolete("since from 2.2.4 and onwards there's only one version in town, namely <seealso cref=\"LogEntryVersion\"/>.")]
	public class LogVersions
	{
		 ...
		 public const sbyte CURRENT_LOG_VERSION = 6;

		 // on disk current format version
		 internal static readonly short CurrentFormatVersion = CURRENT_LOG_VERSION & 0xFF;
	}
