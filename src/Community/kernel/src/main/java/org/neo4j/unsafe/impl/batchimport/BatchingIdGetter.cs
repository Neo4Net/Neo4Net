...
namespace Neo4Net.@unsafe.Impl.Batchimport
{
	...
	public class BatchingIdGetter : PrimitiveLongCollections.PrimitiveLongBaseIterator, IdSequence
	{
		 ...
		 public BatchingIdGetter<T1>( RecordStore<T1> source ) where T1 : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord : this( source, source.RecordsPerPage )
		 {
		 }

		 public BatchingIdGetter<T1>( RecordStore<T1> source, int batchSize ) where T1 : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
			  this._source = source;
