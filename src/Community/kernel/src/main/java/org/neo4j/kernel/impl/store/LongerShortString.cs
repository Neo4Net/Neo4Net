...
//       UPPERHEX(12, 4) { int encTranslate(byte b) { if(b >= '0' && b <= '9') { return b - '0'; } if(b >= 'A' && b <= 'F') { return b - 'A' + 10; } throw cannotEncode(b); } int encPunctuation(byte b) { throw cannotEncode(b); } char decTranslate(byte codePoint) { if(codePoint < 10) { return(char)(codePoint + '0'); } return(char)(codePoint + 'A' - 10); } };
		  ...
		  */
		 public static bool Encode( int keyId, string @string, Neo4Net.Kernel.impl.store.record.PropertyBlock target, int payloadSize )
		 {
		 ...
		 private static bool EncodeWithCharSet( int keyId, string @string, Neo4Net.Kernel.impl.store.record.PropertyBlock target, int payloadSize, int stringLength )
		 {
...
//ORIGINAL LINE: private static boolean tryEncode(int encodings, int keyId, org.neo4j.kernel.impl.store.record.PropertyBlock target, int payloadSize, byte[] data, final int length)
		 private static bool TryEncode( int encodings, int keyId, Neo4Net.Kernel.impl.store.record.PropertyBlock target, int payloadSize, sbyte[] data, int length )
		 {
...
//ORIGINAL LINE: final LongerShortString[] retainUri = {NUMERICAL, DATE, UPPER, LOWER, EMAIL, ALPHANUM, ALPHASYM, EUROPEAN, LOWERHEX, UPPERHEX};
		 ...
		 public static Neo4Net.Values.Storable.TextValue Decode( Neo4Net.Kernel.impl.store.record.PropertyBlock block )
		 {
		 ...
		 private static bool EncodeLatin1( int keyId, string @string, Neo4Net.Kernel.impl.store.record.PropertyBlock target )
		 {
		 ...
		 private static bool EncodeUTF8( int keyId, string @string, Neo4Net.Kernel.impl.store.record.PropertyBlock target, int payloadSize )
		 {
...
//ORIGINAL LINE: private boolean doEncode(int keyId, byte[] data, org.neo4j.kernel.impl.store.record.PropertyBlock target, int payloadSize, final int length)
		 private bool DoEncode( int keyId, sbyte[] data, Neo4Net.Kernel.impl.store.record.PropertyBlock target, int payloadSize, int length )
		 {
