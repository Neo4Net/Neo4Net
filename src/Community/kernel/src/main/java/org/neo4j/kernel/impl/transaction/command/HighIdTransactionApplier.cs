...
//ORIGINAL LINE: for (java.util.Map.Entry<org.neo4j.kernel.impl.store.RecordStore<?>,HighId> highId : highIds.entrySet())
		 ...
		 private void Track<RECORD>( RecordStore<RECORD> store, BaseCommand<RECORD> command ) where RECORD : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
			  Track( store, command.After );
		 }

		 private void Track<T1, T2>( RecordStore<T1> store, ICollection<T2> records ) where T2 : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
		 ...
		 private void TrackToken<RECORD>( TokenStore<RECORD> tokenStore, TokenCommand<RECORD> tokenCommand ) where RECORD : Neo4Net.Kernel.impl.store.record.TokenRecord
		 {
