...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: election { @Override public ElectionState handle(ElectionContext context, org.neo4j.cluster.com.message.Message<ElectionMessage> message, org.neo4j.cluster.com.message.MessageHolder outgoing) { org.neo4j.logging.Log log = context.getLog(ElectionState.class); switch(message.getMessageType()) { case demote: { if(!context.electionOk()) { log.warn("Context says election is not OK to proceed. " + "Failed instances are: " + context.getFailed() + ", cluster members are: " + context.getMembers()); break; } org.neo4j.cluster.InstanceId demoteNode = message.getPayload(); context.nodeFailed(demoteNode); if(context.isInCluster()) { java.util.List<org.neo4j.cluster.InstanceId> aliveInstances = org.neo4j.helpers.collection.Iterables.asList(context.getAlive()); java.util.Collections.sort(aliveInstances); boolean isElector = aliveInstances.indexOf(context.getMyId()) == 0; if(isElector) { log.debug("I (" + context.getMyId() + ") am the elector, executing the election"); Iterable<String> rolesRequiringElection = context.getRolesRequiringElection(); for(String role : rolesRequiringElection) { if(!context.isElectionProcessInProgress(role)) { log.debug("Starting election process for role " + role); context.startElectionProcess(role); for(java.util.Map.Entry<org.neo4j.cluster.InstanceId, java.net.URI> server : context.getMembers().entrySet()) { if(!context.getFailed().contains(server.getKey())) { outgoing.offer(org.neo4j.cluster.com.message.Message.to(ElectionMessage.vote, server.getValue(), context.voteRequestForRole(new ElectionRole(role)))); } } context.setTimeout("election-" + role, org.neo4j.cluster.com.message.Message.timeout(ElectionMessage.electionTimeout, message, new ElectionTimeoutData(role, message))); } else { log.debug("Election already in progress for role " + role); } } } } break; } case performRoleElections: { if(!context.electionOk()) { log.warn("Context says election is not OK to proceed. " + "Failed instances are: " + context.getFailed() + ", cluster members are: " + context.getMembers()); break; } if(context.isInCluster()) { boolean isElector = context.isElector(); if(isElector) { context.getLog(ElectionState.class).info("I am the elector, doing election..."); Iterable<ElectionRole> rolesRequiringElection = context.getPossibleRoles(); for(ElectionRole role : rolesRequiringElection) { String roleName = role.getName(); if(!context.isElectionProcessInProgress(roleName)) { context.getLog(ElectionState.class).debug("Starting election process for role " + roleName); context.startElectionProcess(roleName); boolean sentSome = false; for(java.util.Map.Entry<org.neo4j.cluster.InstanceId, java.net.URI> server : context.getMembers().entrySet()) { if(!context.isFailed(server.getKey()) && !server.getKey().equals(context.getElected(roleName))) { outgoing.offer(org.neo4j.cluster.com.message.Message.to(ElectionMessage.vote, server.getValue(), context.voteRequestForRole(role))); sentSome = true; } } if(!sentSome) { outgoing.offer(org.neo4j.cluster.com.message.Message.internal(ElectionMessage.vote, context.voteRequestForRole(new ElectionRole(roleName)))); } else { context.setTimeout("election-" + roleName, org.neo4j.cluster.com.message.Message.timeout(ElectionMessage.electionTimeout, message, new ElectionTimeoutData(roleName, message))); } } else { log.debug("Election already in progress for role " + roleName); } } } else { java.util.Set<org.neo4j.cluster.InstanceId> aliveInstances = org.neo4j.helpers.collection.Iterables.asSet(context.getAlive()); aliveInstances.removeAll(context.getFailed()); java.util.List<org.neo4j.cluster.InstanceId> adjustedAlive = org.neo4j.helpers.collection.Iterables.asList(aliveInstances); java.util.Collections.sort(adjustedAlive); context.getLog(ElectionState.class).info("I am NOT the elector, sending to " + adjustedAlive); outgoing.offer(message.setHeader(org.neo4j.cluster.com.message.Message.HEADER_TO, context.getUriForId(firstOrNull(adjustedAlive)).toString())); } } break; } case vote: { Object request = message.getPayload(); ElectionContext_VoteRequest voteRequest = (ElectionContext_VoteRequest) request; outgoing.offer(org.neo4j.cluster.com.message.Message.respond(ElectionMessage.voted, message, new ElectionMessage.VersionedVotedData(voteRequest.getRole(), context.getMyId(), context.getCredentialsForRole(voteRequest.getRole()), voteRequest.getVersion()))); break; } case voted: { ElectionMessage.VotedData data = message.getPayload(); long version = -1; if(data instanceof ElectionMessage.VersionedVotedData) { version = ((ElectionMessage.VersionedVotedData) data).getVersion(); } boolean accepted = context.voted(data.getRole(), data.getInstanceId(), data.getElectionCredentials(), version); String voter = message.hasHeader(org.neo4j.cluster.com.message.Message.HEADER_FROM) ? message.getHeader(org.neo4j.cluster.com.message.Message.HEADER_FROM) : "I"; log.debug(voter + " voted " + data + " which i " + (accepted ? "accepted" : "did not accept")); if(!accepted) { break; } org.neo4j.cluster.InstanceId currentElected = context.getElected(data.getRole()); if(context.getVoteCount(data.getRole()) == context.getNeededVoteCount()) { org.neo4j.cluster.InstanceId winner = context.getElectionWinner(data.getRole()); context.cancelTimeout("election-" + data.getRole()); context.forgetElection(data.getRole()); if(winner != null) { log.debug("Elected " + winner + " as " + data.getRole()); org.neo4j.cluster.protocol.cluster.ClusterMessage.VersionedConfigurationStateChange configurationChangeState = context.newConfigurationStateChange(); configurationChangeState.elected(data.getRole(), winner); outgoing.offer(org.neo4j.cluster.com.message.Message.internal(org.neo4j.cluster.protocol.atomicbroadcast.multipaxos.AtomicBroadcastMessage.broadcast, configurationChangeState)); } else { log.warn("Election could not pick a winner"); if(currentElected != null) { org.neo4j.cluster.protocol.cluster.ClusterMessage.ConfigurationChangeState configurationChangeState = new org.neo4j.cluster.protocol.cluster.ClusterMessage.ConfigurationChangeState(); configurationChangeState.unelected(data.getRole(), currentElected); outgoing.offer(org.neo4j.cluster.com.message.Message.internal(org.neo4j.cluster.protocol.atomicbroadcast.multipaxos.ProposerMessage.propose, configurationChangeState)); } } } else if(context.getVoteCount(data.getRole()) == context.getNeededVoteCount() - 1 && currentElected != null && !context.hasCurrentlyElectedVoted(data.getRole(), currentElected)) { outgoing.offer(org.neo4j.cluster.com.message.Message.to(ElectionMessage.vote, context.getUriForId(currentElected), context.voteRequestForRole(new ElectionRole(data.getRole())))); } break; } case electionTimeout: { ElectionTimeoutData electionTimeoutData = message.getPayload(); log.warn(String.format("Election timed out for '%s'- trying again", electionTimeoutData.getRole())); context.forgetElection(electionTimeoutData.getRole()); outgoing.offer(electionTimeoutData.getMessage()); break; } case leave: { return start; } default: break; } return this; } };
		 election
		 {
			 public ElectionState handle( ElectionContext context, Message<ElectionMessage> message, MessageHolder outgoing )
			 {
