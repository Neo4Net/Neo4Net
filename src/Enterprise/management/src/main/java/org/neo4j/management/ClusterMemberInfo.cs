...
//ORIGINAL LINE: @Override @SuppressWarnings("boxing") public String toString()
		 ...
		 {
			  return string.Format( "Neo4jHaInstance[id={0},available={1},haRole={2},HA URI={3}]", _instanceId, _available, _haRole, Arrays.ToString( _uris ) );
		 }

		 public virtual Pair<Neo4jManager, HighAvailability> Connect()
		 {
			  return Connect( null, null );
		 }

		 public virtual Pair<Neo4jManager, HighAvailability> Connect( string username, string password )
		 {
			  URI address = null;
	...
	//                address = uri;
			  ...
			  Neo4jManager manager = Neo4jManager.Get( Url( address ), username, password, _instanceId );
