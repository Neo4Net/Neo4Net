...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldShuffleServers()
		 {
			  ...
			  long ttl = 1000;
			  Neo4Net.causalclustering.routing.load_balancing.LoadBalancingProcessor_Result result = new LoadBalancingResult( new List<Endpoint>( routers ), new List<Endpoint>( writers ), new List<Endpoint>( readers ), ttl );
			  ...
			  {
					// when
					Neo4Net.causalclustering.routing.load_balancing.LoadBalancingProcessor_Result shuffledResult = plugin.Run( Collections.emptyMap() );
