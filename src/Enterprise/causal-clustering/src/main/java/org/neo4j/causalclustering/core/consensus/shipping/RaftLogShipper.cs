...
namespace Neo4Net.causalclustering.core.consensus.shipping
{
	...
	using InFlightLogEntryReader = Neo4Net.causalclustering.core.state.InFlightLogEntryReader;
	...
	public class RaftLogShipper
	{
		 ...
		 public virtual void OnMismatch( long lastRemoteAppendIndex, LeaderContext leaderContext )
		 {
			 lock ( this )
			 {
				  switch ( _mode )
				  {
						case Neo4Net.causalclustering.core.consensus.shipping.RaftLogShipper.Mode.Mismatch:
							 long logIndex = max( min( _lastSentIndex - 1, lastRemoteAppendIndex ), MIN_INDEX );
							 SendEmpty( logIndex, leaderContext );
							 break;
						case Neo4Net.causalclustering.core.consensus.shipping.RaftLogShipper.Mode.Pipeline:
						case Neo4Net.causalclustering.core.consensus.shipping.RaftLogShipper.Mode.Catchup:
		 ...
		 private void SendCommitUpdate( LeaderContext leaderContext )
		 {
			   ...
			   */
			  Neo4Net.causalclustering.core.consensus.RaftMessages_Heartbeat appendRequest = new Neo4Net.causalclustering.core.consensus.RaftMessages_Heartbeat( _leader, leaderContext.Term, leaderContext.CommitIndex, leaderContext.Term );
		 ...
		 private void SendNewEntries( long prevLogIndex, long prevLogTerm, RaftLogEntry[] newEntries, LeaderContext leaderContext )
		 {
			  ...
			  Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Request appendRequest = new Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Request( _leader, leaderContext.Term, prevLogIndex, prevLogTerm, newEntries, leaderContext.CommitIndex );
		 ...
		 private void SendEmpty( long logIndex, LeaderContext leaderContext )
		 {
			  ...
			  try
			  {
					...
					Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Request appendRequest = new Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Request( _leader, leaderContext.Term, prevLogIndex, prevLogTerm, RaftLogEntry.empty, leaderContext.CommitIndex );
					_outbound.send( _follower, appendRequest );
			  }
			  catch ( IOException e )
			  {
		 ...
		 private void SendRange( long startIndex, long endIndex, LeaderContext leaderContext )
		 {
			  ...
			  try
			  {
					...
					{
						 Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Request appendRequest = new Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Request( _leader, leaderContext.Term, prevLogIndex, prevLogTerm, entries, leaderContext.CommitIndex );
		 ...
		 private void SendLogCompactionInfo( LeaderContext leaderContext )
		 {
			  ...
			  _outbound.send( _follower, new Neo4Net.causalclustering.core.consensus.RaftMessages_LogCompactionInfo( _leader, leaderContext.Term, _raftLog.prevIndex() ) );
		 }
