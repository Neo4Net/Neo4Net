...
namespace Neo4Net.causalclustering.handlers
{
	...
	public class ExceptionLoggingHandler : ChannelHandlerAdapter
	{
		 ...
		 public override void ExceptionCaught( ChannelHandlerContext ctx, Exception cause )
		 {
			  if ( ctx != null )
			  {
					_log.error( format( "Failed to process message on channel %s.", ctx.channel() ), cause );
					ctx.fireExceptionCaught( cause );
			  }
			  else
			  {
					_log.error( "Failed to process message on a null channel.", cause );
			  }
		 }
	}
