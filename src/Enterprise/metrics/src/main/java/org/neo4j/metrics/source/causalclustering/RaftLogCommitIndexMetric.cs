...
namespace Neo4Net.metrics.source.causalclustering
{
	...
	public class RaftLogCommitIndexMetric : RaftLogCommitIndexMonitor
	{
		 private AtomicLong _commitIndex = new AtomicLong( 0 );

		 public override long CommitIndex()
		 {
			  return _commitIndex.get();
		 }

		 public override void CommitIndex( long commitIndex )
		 {
			  this._commitIndex.set( commitIndex );
		 }
	}
