...
//ORIGINAL LINE: private <C extends org.neo4j.kernel.impl.transaction.command.Command, R extends org.neo4j.kernel.impl.store.record.AbstractBaseRecord> boolean scan(org.neo4j.kernel.impl.transaction.log.files.LogFiles logFiles, InconsistenciesHandler handler, org.neo4j.tools.txlog.checktypes.CheckType<C,R> check, boolean checkTxIds) throws java.io.IOException
		 private bool Scan<C, R>( LogFiles logFiles, InconsistenciesHandler handler, CheckType<C, R> check, bool checkTxIds ) where C : Neo4Net.Kernel.impl.transaction.command.Command where R : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
		 ...
		 private bool CheckAndHandleInconsistencies<C, R>( CommandAndLogVersion txCommand, CheckType<C, R> check, CommittedRecords<R> state, long txId, InconsistenciesHandler handler ) where C : Neo4Net.Kernel.impl.transaction.command.Command where R : Neo4Net.Kernel.impl.store.record.AbstractBaseRecord
		 {
