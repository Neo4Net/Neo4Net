...
//	import static org.junit.Assert.assertEquals;
	...
	public class Neo4NetAdminUsageTest
	{
...
//ORIGINAL LINE: final StringBuilder sb = new StringBuilder();
			  ...
			  assertEquals( "usage: Neo4Net-admin <command>\n" + "\n" + "Manage your Neo4Net instance.\n" + "\n" + "environment variables:\n" + "    Neo4Net_CONF    Path to directory which contains Neo4Net.conf.\n" + "    Neo4Net_DEBUG   Set to anything to enable debug output.\n" + "    Neo4Net_HOME    Neo4Net home directory.\n" + "    HEAP_SIZE     Set JVM maximum heap size during command execution.\n" + "                  Takes a number and a unit, for example 512m.\n" + "\n" + "available commands:\n" + "\n" + "General\n" + "    check-consistency\n" + "        Check the consistency of a database.\n" + "    import\n" + "        Import from a collection of CSV files or a pre-3.0 database.\n" + "    memrec\n" + "        Print Neo4Net heap and pagecache memory settings recommendations.\n" + "    report\n" + "        Produces a zip/tar of the most common information needed for remote assessments.\n" + "    store-info\n" + "        Prints information about a Neo4Net database store.\n" + "\n" + "Authentication\n" + "    set-default-admin\n" + "        Sets the default admin user when no roles are present.\n" + "    set-initial-password\n" + "        Sets the initial password of the initial admin user ('Neo4Net').\n" + "\n" + "Clustering\n" + "    unbind\n" + "        Removes cluster state data for the specified database.\n" + "\n" + "Offline backup\n" + "    dump\n" + "        Dump a database into a single-file archive.\n" + "    load\n" + "        Load a database from an archive created with the dump command.\n" + "\n" + "Online backup\n" + "    backup\n" + "        Perform an online backup from a running Neo4Net enterprise server.\n" + "    restore\n" + "        Restore a backed up database.\n" + "\n" + "Use Neo4Net-admin help <command> for more details.\n", sb.ToString() );
		 }
	}
