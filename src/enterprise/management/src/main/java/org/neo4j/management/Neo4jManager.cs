...
namespace Neo4Net.management
{
	...
	public sealed class Neo4NetManager : KernelProxy, Kernel
	{
		 public static Neo4NetManager Get()
		 {
			  return get( PlatformMBeanServer );
		 }

		 public static Neo4NetManager Get( string kernelIdentifier )
		 {
			  return get( PlatformMBeanServer, kernelIdentifier );
		 }

		 public static Neo4NetManager Get( JMXServiceURL url )
		 {
			  return Get( Connect( url, null, null ) );
		 }

		 public static Neo4NetManager Get( JMXServiceURL url, string kernelIdentifier )
		 {
			  return Get( Connect( url, null, null ), kernelIdentifier );
		 }

		 public static Neo4NetManager Get( JMXServiceURL url, string username, string password )
		 {
			  return Get( Connect( url, username, password ) );
		 }

		 public static Neo4NetManager Get( JMXServiceURL url, string username, string password, string kernelIdentifier )
		 {
		 ...
		 public static Neo4NetManager Get( MBeanServerConnection server )
		 {
			  try
			  {
		 ...
		 public static Neo4NetManager Get( MBeanServerConnection server, string kernelIdentifier )
		 {
			  try
			  {
		 ...
		 public static Neo4NetManager[] GetAll( MBeanServerConnection server )
		 {
			  try
			  {
					ISet<ObjectName> kernels = server.queryNames( CreateObjectName( "*", typeof( Kernel ) ), null );
					Neo4NetManager[] managers = new Neo4NetManager[kernels.Count];
					IEnumerator<ObjectName> it = kernels.GetEnumerator();
					for ( int i = 0; i < managers.Length; i++ )
					{
//JAVA TO C# CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
						 managers[i] = new Neo4NetManager( server, it.next() );
					}
					return managers;
			  }
			  catch ( IOException e )
		 ...
		 private static Neo4NetManager Get( MBeanServerConnection server, ICollection<ObjectName> kernels )
		 {
			  if ( kernels.Count == 0 )
			  {
					throw new NoSuchElementException( "No matching Neo4Net Graph Database running on server" );
			  }
			  else if ( kernels.Count == 1 )
			  {
					return new Neo4NetManager( server, kernels.GetEnumerator().next() );
			  }
			  else
			  {
					throw new NoSuchElementException( "Too many matching Neo4Net Graph Databases running on server" );
			  }
		 }
		 ...
		 public Neo4NetManager( Kernel kernel ) : this( GetServer( kernel ), GetName( kernel ) )
		 {
		 }
		 ...
		 private static MBeanServerConnection GetServer( Kernel kernel )
		 {
			  ...
			  else if ( kernel is Neo4NetManager )
			  {
					return ( ( Neo4NetManager ) kernel ).Server;
			  }
		 ...
		 private static ObjectName GetName( Kernel kernel )
		 {
			  ...
			  {
					return ( ( Neo4NetManager ) kernel ).Kernel;
			  }
			  throw new System.NotSupportedException( "Cannot get name for kernel: " + kernel );
		 }

		 private Neo4NetManager( MBeanServerConnection server, ObjectName kernel ) : base( server, kernel )
		 {
