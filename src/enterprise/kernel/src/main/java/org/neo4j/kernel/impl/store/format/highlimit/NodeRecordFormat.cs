...
namespace Neo4Net.Kernel.impl.store.format.highlimit
{
	...
	/// => 15B
	/// </summary>
	internal class NodeRecordFormat : BaseHighLimitRecordFormat<NodeRecord>
	{
		 ...
		 internal NodeRecordFormat() : this(RECORD_SIZE)
		 {
		 }

		 internal NodeRecordFormat( int recordSize ) : base( fixedRecordSize( recordSize ), 0, HighLimitFormatSettings.NODE_MAXIMUM_ID_BITS )
		 {
		 }
