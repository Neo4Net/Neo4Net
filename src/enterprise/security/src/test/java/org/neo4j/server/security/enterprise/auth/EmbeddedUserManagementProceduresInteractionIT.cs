...
//ORIGINAL LINE: protected NeoInteractionLevel<Neo4Net.kernel.enterprise.api.security.EnterpriseLoginContext> setUpNeoServer(java.util.Map<String, String> config) throws Throwable
		  ...
		  */
		 //@Test
		 public virtual void ShouldAllowOptionalMatch()
		 {
			  AssertEmpty( AdminSubject, AddMovieDatasetQuery );

			  string movieOptionalMatch = "match (n:Movie)<--()-->(m:Movie)<--()-->(o:Movie)<--()-->(p:Movie)<--()-->(q:Movie) " +
						 "OPTIONAL MATCH (k) " +
						 "WHERE id(k) = 32 " +
						 "WITH n,p,count(*) as c " +
			  ...
			  {
				IList<object> counts = GetObjectsAsList( r, "count" );
				assertThat( Counts.size(), equalTo(1) );
				assertThat( Counts.get( 0 ), equalTo( 1395L ) );
			  });
		 }

		 internal string AddMovieDatasetQuery = "CREATE (TheMatrix:Movie {title:'The Matrix', released:1999, tagline:'Welcome to the Real World'}) " +
