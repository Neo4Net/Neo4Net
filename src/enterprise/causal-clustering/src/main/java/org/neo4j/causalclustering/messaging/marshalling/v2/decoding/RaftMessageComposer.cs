...
namespace Neo4Net.causalclustering.messaging.marshalling.v2.decoding
{
	...
	public class RaftMessageComposer : MessageToMessageDecoder<object>
	{
		 ...
		 protected internal override void Decode( ChannelHandlerContext ctx, object msg, IList<object> @out )
		 {
			  ...
			  if ( _messageComposer != null )
			  {
					Optional<Neo4Net.causalclustering.core.consensus.RaftMessages_ClusterIdAwareMessage> clusterIdAwareMessage = _messageComposer.maybeCompose( _clock, _raftLogEntryTerms, _replicatedContents );
		 ...
		 private void Clear( Neo4Net.causalclustering.core.consensus.RaftMessages_ClusterIdAwareMessage message )
		 {
