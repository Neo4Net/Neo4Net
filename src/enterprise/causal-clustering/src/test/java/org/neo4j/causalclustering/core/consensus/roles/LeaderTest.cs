...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldNotRespondToSuccessResponseFromFollowerThatWillSoonUpToDateViaInFlightMessages()
		 {
			  ...
			  Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response response = appendEntriesResponse().success().matchIndex(90).term(4).from(instance2).build();
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldNotRespondToSuccessResponseThatIndicatesUpToDateFollower()
		 {
			  ...
			  // when
			  // that leader is asked to handle a response from that follower that says that the follower is up to date
			  Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response response = appendEntriesResponse().success().matchIndex(100).term(4).from(instance2).build();
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldRespondToSuccessResponseThatIndicatesLaggingFollowerWithJustWhatItsMissing()
		 {
			  ...
			  // missing things
			  Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response response = appendEntriesResponse().success().matchIndex(89).term(231).from(instance2).build();
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldIgnoreSuccessResponseThatIndicatesLaggingWhileLocalStateIndicatesFollowerIsCaughtUp()
		 {
			  ...
			  // when that leader is asked to handle a response from that follower that says that the follower is still
			  // missing things
			  Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response response = appendEntriesResponse().success().matchIndex(80).term(4).from(instance2).build();
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldSpawnMismatchCommandOnFailure()
		 {
			  ...
			  // things
			  Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response response = appendEntriesResponse().failure().appendIndex(0).matchIndex(-1).term(4).from(instance2).build();
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldSendCompactionInfoIfFailureWithNoEarlierEntries()
		 {
			  ...
			  Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response incomingResponse = appendEntriesResponse().failure().term(term).appendIndex(followerIndex).from(_member1).build();

			  // when
			  Outcome outcome = leader.Handle( incomingResponse, state, Log() );

			  // then
			  Neo4Net.causalclustering.core.consensus.RaftMessages_RaftMessage outgoingMessage = messageFor( outcome, _member1 );
			  assertThat( outgoingMessage, instanceOf( typeof( Neo4Net.causalclustering.core.consensus.RaftMessages_LogCompactionInfo ) ) );

			  Neo4Net.causalclustering.core.consensus.RaftMessages_LogCompactionInfo typedOutgoingMessage = ( Neo4Net.causalclustering.core.consensus.RaftMessages_LogCompactionInfo ) outgoingMessage;
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldIgnoreAppendResponsesFromOldTerms()
		 {
			  ...
			  RaftState state = raftState().term(leaderTerm).build();
						 ...
						 Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response incomingResponse = appendEntriesResponse().failure().term(followerTerm).from(_member1).build();
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldRejectAppendEntriesResponseWithNewerTermAndBecomeAFollower()
		 {
			  ...
			  // when
			  Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response message = appendEntriesResponse().from(_member1).term(state.Term() + 1).build();
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldSendHeartbeatsToAllClusterMembersOnReceiptOfHeartbeatTick()
		 {
			  ...
			  Leader leader = new Leader();
			  leader.Handle( new Neo4Net.causalclustering.core.consensus.RaftMessages_HeartbeatResponse( _member1 ), state, Log() ); // make sure it has quorum.

			  // when
			  Outcome outcome = leader.Handle( new RaftMessages_Timeout_Heartbeat( _myself ), state, Log() );

			  // then
			  assertTrue( messageFor( outcome, _member1 ) is Neo4Net.causalclustering.core.consensus.RaftMessages_Heartbeat );
			  assertTrue( messageFor( outcome, _member2 ) is Neo4Net.causalclustering.core.consensus.RaftMessages_Heartbeat );
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldStepDownWhenLackingHeartbeatResponses()
		 {
			  ...
			  Leader leader = new Leader();
			  leader.Handle( new Neo4Net.causalclustering.core.consensus.RaftMessages_Timeout_Election( _myself ), state, Log() );

			  // when
			  Outcome outcome = leader.Handle( new Neo4Net.causalclustering.core.consensus.RaftMessages_Timeout_Election( _myself ), state, Log() );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldNotStepDownWhenReceivedQuorumOfHeartbeatResponses()
		 {
			  ...
			  // when
			  Outcome outcome = leader.Handle( new Neo4Net.causalclustering.core.consensus.RaftMessages_HeartbeatResponse( _member1 ), state, Log() );
			  state.Update( outcome );

			  // we now have quorum and should not step down
			  outcome = leader.Handle( new Neo4Net.causalclustering.core.consensus.RaftMessages_Timeout_Election( _myself ), state, Log() );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldDecideToAppendToItsLogAndSendAppendEntriesMessageOnReceiptOfClientProposal()
		 {
			  ...
			  Neo4Net.causalclustering.core.consensus.RaftMessages_NewEntry_Request newEntryRequest = new Neo4Net.causalclustering.core.consensus.RaftMessages_NewEntry_Request( member( 9 ), _content );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldCommitOnMajorityResponse()
		 {
			  ...
			  Outcome outcome = leader.Handle( new Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response( _member1, 0, true, 0, 0 ), state, Log() );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void LeaderShouldCommitAllPreviouslyAppendedEntriesWhenCommittingLaterEntryInSameTerm()
		 {
			  ...
			  // when
			  Outcome outcome = leader.Handle( new Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response( _member1, 0, true, 2, 2 ), state, Log() );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldSendNegativeResponseForVoteRequestFromTermNotGreaterThanLeader()
		 {
			  ...
			  // when
			  Outcome outcome = leader.Handle( new Neo4Net.causalclustering.core.consensus.RaftMessages_Vote_Request( _member1, rivalTerm, _member1, leaderCommitIndex, leaderTerm ), state, Log() );

			  // then
			  assertThat( outcome.Role, equalTo( LEADER ) );
			  assertThat( outcome.Term, equalTo( leaderTerm ) );

			  Neo4Net.causalclustering.core.consensus.RaftMessages_RaftMessage response = messageFor( outcome, _member1 );
			  assertThat( response, instanceOf( typeof( Neo4Net.causalclustering.core.consensus.RaftMessages_Vote_Response ) ) );
			  Neo4Net.causalclustering.core.consensus.RaftMessages_Vote_Response typedResponse = ( Neo4Net.causalclustering.core.consensus.RaftMessages_Vote_Response ) response;
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldIgnoreHeartbeatFromOlderTerm()
		 {
			  ...
			  // when
			  Outcome outcome = leader.Handle( new Neo4Net.causalclustering.core.consensus.RaftMessages_Heartbeat( _member1, rivalTerm, leaderCommitIndex, leaderTerm ), state, Log() );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldRespondNegativelyToAppendEntriesRequestFromEarlierTerm()
		 {
			  ...
			  // when
			  Outcome outcome = leader.Handle( new Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Request( _member1, rivalTerm, logIndex, leaderTerm, entries, leaderCommitIndex ), state, Log() );
			  ...
			  Neo4Net.causalclustering.core.consensus.RaftMessages_RaftMessage response = messageFor( outcome, _member1 );
			  assertThat( response, instanceOf( typeof( Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response ) ) );
			  Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response typedResponse = ( Neo4Net.causalclustering.core.consensus.RaftMessages_AppendEntries_Response ) response;
