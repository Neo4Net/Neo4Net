...
//ORIGINAL LINE: @Test public void subClusterRoutingProcedureShouldHaveCorrectSignature()
		 public virtual void SubClusterRoutingProcedureShouldHaveCorrectSignature()
		 {
			  ...
			  IList<FieldSignature> input = Collections.singletonList( FieldSignature.inputField( "database", Neo4NetTypes.NTString ) );
			  IList<FieldSignature> output = Arrays.asList( FieldSignature.outputField( "ttl", Neo4NetTypes.NTInteger ), FieldSignature.outputField( "routers", Neo4NetTypes.NTList( Neo4NetTypes.NTMap ) ) );
