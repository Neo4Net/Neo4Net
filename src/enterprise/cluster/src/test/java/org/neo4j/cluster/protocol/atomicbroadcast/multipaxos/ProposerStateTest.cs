...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void IfProposingWithClosedInstanceThenRetryWithNextInstance()
		 {
			  ...
			  Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.InstanceId instanceId = new Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.InstanceId( 42 );
			  ...
			  message.setHeader( Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.InstanceId.INSTANCE, instanceId.ToString() );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ProposerProposePhase1TimeoutShouldCarryOnPayload()
		 {
			  ...
			  when( context.GetPaxosInstance( any( typeof( Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.InstanceId ) ) ) ).thenReturn( instance );
			  when( context.MyId ).thenReturn( new Neo4Net.cluster.InstanceId( 0 ) );
			  ...
			  // THEN
			  verify( context ).setTimeout( eq( new Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.InstanceId( instanceId ) ), argThat( ( new MessageArgumentMatcher<>() ).withPayload(payload) ) );
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ProposerRejectAcceptShouldCarryOnPayload()
		 {
			  // GIVEN
			  string instanceId = "1";
			  PaxosInstance instance = new PaxosInstance( mock( typeof( PaxosInstanceStore ) ), new Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.InstanceId( instanceId ) );
			  Serializable payload = "myPayload";
			  ...
			  when( context.GetLog( any( typeof( Type ) ) ) ).thenReturn( NullLog.Instance );
			  when( context.GetPaxosInstance( any( typeof( Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.InstanceId ) ) ) ).thenReturn( instance );
			  when( context.MyId ).thenReturn( new Neo4Net.cluster.InstanceId( parseInt( instanceId ) ) );
			  TrackingMessageHolder outgoing = new TrackingMessageHolder();
