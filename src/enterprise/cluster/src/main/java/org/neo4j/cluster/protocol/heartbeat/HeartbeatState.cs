...
//ORIGINAL LINE: start { @Override public HeartbeatState handle(HeartbeatContext context, Neo4Net.cluster.com.message.Message<HeartbeatMessage> message, Neo4Net.cluster.com.message.MessageHolder outgoing) { switch(message.getMessageType()) { case addHeartbeatListener: { context.addHeartbeatListener(message.getPayload()); break; } case removeHeartbeatListener: { context.removeHeartbeatListener(message.getPayload()); break; } case join: { for(Neo4Net.cluster.InstanceId instanceId : context.getOtherInstances()) { context.setTimeout(HeartbeatMessage.i_am_alive + "-" + instanceId, timeout(HeartbeatMessage.timed_out, message, instanceId)); outgoing.offer(timeout(HeartbeatMessage.sendHeartbeat, message, instanceId)); } return heartbeat; } default: break; } return this; } },
		 start
		 {
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: heartbeat { @Override public HeartbeatState handle(HeartbeatContext context, Neo4Net.cluster.com.message.Message<HeartbeatMessage> message, Neo4Net.cluster.com.message.MessageHolder outgoing) { switch(message.getMessageType()) { case i_am_alive: { HeartbeatMessage.IAmAliveState state = message.getPayload(); if(context.isMe(state.getServer())) { break; } if(state.getServer() == null) { break; } if(context.alive(state.getServer())) { for(Neo4Net.cluster.InstanceId aliveServer : context.getAlive()) { if(!aliveServer.equals(context.getMyId())) { java.net.URI aliveServerUri = context.getUriForId(aliveServer); outgoing.offer(Neo4Net.cluster.com.message.Message.to(HeartbeatMessage.suspicions, aliveServerUri, new HeartbeatMessage.SuspicionsState(context.getSuspicionsFor(context.getMyId())))); } } } resetTimeout(context, message, state); if(message.hasHeader("last-learned")) { long lastLearned = long.Parse(message.getHeader("last-learned")); if(lastLearned > context.getLastKnownLearnedInstanceInCluster()) { Neo4Net.cluster.com.message.Message<Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.LearnerMessage> catchUpMessage = message.copyHeadersTo(internal(Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.LearnerMessage.catchUp, lastLearned), Neo4Net.cluster.com.message.Message.HEADER_FROM, Neo4Net.cluster.com.message.Message.HEADER_INSTANCE_ID); outgoing.offer(catchUpMessage); } } break; } case timed_out: { Neo4Net.cluster.InstanceId server = message.getPayload(); context.getLog(HeartbeatState.class).debug("Received timed out for server " + server); if(context.getMembers().containsKey(server)) { context.suspect(server); context.setTimeout(HeartbeatMessage.i_am_alive + "-" + server, timeout(HeartbeatMessage.timed_out, message, server)); for(Neo4Net.cluster.InstanceId aliveServer : context.getAlive()) { if(!aliveServer.equals(context.getMyId())) { java.net.URI sendTo = context.getUriForId(aliveServer); outgoing.offer(Neo4Net.cluster.com.message.Message.to(HeartbeatMessage.suspicions, sendTo, new HeartbeatMessage.SuspicionsState(context.getSuspicionsFor(context.getMyId())))); } } } else { context.serverLeftCluster(server); } break; } case sendHeartbeat: { Neo4Net.cluster.InstanceId to = message.getPayload(); if(!context.isMe(to)) { if(context.getMembers().containsKey(to)) { java.net.URI toSendTo = context.getUriForId(to); outgoing.offer(to(HeartbeatMessage.i_am_alive, toSendTo, new HeartbeatMessage.IAmAliveState(context.getMyId())).setHeader("last-learned", context.getLastLearnedInstanceId() + "")); context.setTimeout(HeartbeatMessage.sendHeartbeat + "-" + to, timeout(HeartbeatMessage.sendHeartbeat, message, to)); } } break; } case reset_send_heartbeat: { Neo4Net.cluster.InstanceId to = message.getPayload(); if(!context.isMe(to)) { String timeoutName = HeartbeatMessage.sendHeartbeat + "-" + to; context.cancelTimeout(timeoutName); context.setTimeout(timeoutName, Neo4Net.cluster.com.message.Message.timeout(HeartbeatMessage.sendHeartbeat, message, to)); } break; } case suspicions: { HeartbeatMessage.SuspicionsState suspicions = message.getPayload(); Neo4Net.cluster.InstanceId fromId = new Neo4Net.cluster.InstanceId(int.Parse(message.getHeader(Neo4Net.cluster.com.message.Message.HEADER_INSTANCE_ID))); context.getLog(HeartbeatState.class).debug(format("Received suspicions as %s from %s", suspicions, fromId)); suspicions.getSuspicions().remove(context.getMyId()); context.suspicions(fromId, suspicions.getSuspicions()); break; } case leave: { context.getLog(HeartbeatState.class).debug("Received leave"); return start; } case addHeartbeatListener: { context.addHeartbeatListener(message.getPayload()); break; } case removeHeartbeatListener: { context.removeHeartbeatListener(message.getPayload()); break; } default: break; } return this; } private void resetTimeout(HeartbeatContext context, Neo4Net.cluster.com.message.Message<HeartbeatMessage> message, HeartbeatMessage.IAmAliveState state) { String key = HeartbeatMessage.i_am_alive + "-" + state.getServer(); Neo4Net.cluster.com.message.Message<? extends Neo4Net.cluster.com.message.MessageType> oldTimeout = context.cancelTimeout(key); if(oldTimeout != null && oldTimeout.hasHeader(Neo4Net.cluster.com.message.Message.HEADER_TIMEOUT_COUNT)) { int timeoutCount = int.Parse(oldTimeout.getHeader(Neo4Net.cluster.com.message.Message.HEADER_TIMEOUT_COUNT)); if(timeoutCount > 0) { long timeout = context.getTimeoutFor(oldTimeout); context.getLog(HeartbeatState.class).debug("Received " + state + " after missing " + timeoutCount + " (" + timeout * timeoutCount + "ms)"); } } context.setTimeout(key, timeout(HeartbeatMessage.timed_out, message, state.getServer())); } }
