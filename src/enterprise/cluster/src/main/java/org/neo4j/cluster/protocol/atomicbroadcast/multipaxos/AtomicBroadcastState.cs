...
//ORIGINAL LINE: start { @Override public AtomicBroadcastState handle(AtomicBroadcastContext context, Neo4Net.cluster.com.message.Message<AtomicBroadcastMessage> message, Neo4Net.cluster.com.message.MessageHolder outgoing) { switch(message.getMessageType()) { case entered: { return broadcasting; } case join: { return joining; } default: { defaultHandling(context, message); } } return this; } },
		 start
		 {
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: joining { @Override public AtomicBroadcastState handle(AtomicBroadcastContext context, Neo4Net.cluster.com.message.Message<AtomicBroadcastMessage> message, Neo4Net.cluster.com.message.MessageHolder outgoing) { switch(message.getMessageType()) { case failed: { outgoing.offer(internal(Neo4Net.cluster.protocol.cluster.ClusterMessage.joinFailure, new java.util.concurrent.TimeoutException("Could not join cluster"))); return start; } case broadcastResponse: { if(message.getPayload() instanceof Neo4Net.cluster.protocol.cluster.ClusterMessage.ConfigurationChangeState) { outgoing.offer(message.copyHeadersTo(internal(Neo4Net.cluster.protocol.cluster.ClusterMessage.configurationChanged, message.getPayload()))); } break; } case entered: { return broadcasting; } default: { defaultHandling(context, message); } } return this; } },
		 joining
		 {
			 public AtomicBroadcastState handle( AtomicBroadcastContext context, Message<AtomicBroadcastMessage> message, MessageHolder outgoing )
			 {
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: broadcasting { @Override public AtomicBroadcastState handle(AtomicBroadcastContext context, Neo4Net.cluster.com.message.Message<AtomicBroadcastMessage> message, Neo4Net.cluster.com.message.MessageHolder outgoing) { switch(message.getMessageType()) { case broadcast: case failed: { if(context.hasQuorum()) { Neo4Net.cluster.InstanceId coordinator = context.getCoordinator(); if(coordinator != null) { java.net.URI coordinatorUri = context.getUriForId(coordinator); outgoing.offer(message.copyHeadersTo(to(ProposerMessage.propose, coordinatorUri, message.getPayload()))); context.setTimeout("broadcast-" + message.getHeader(Neo4Net.cluster.com.message.Message.HEADER_CONVERSATION_ID), timeout(AtomicBroadcastMessage.broadcastTimeout, message, message.getPayload())); } else { outgoing.offer(message.copyHeadersTo(internal(ProposerMessage.propose, message.getPayload()), Neo4Net.cluster.com.message.Message.HEADER_CONVERSATION_ID, Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.InstanceId.INSTANCE)); } } else { context.getLog(AtomicBroadcastState.class).warn("No quorum and therefor dropping broadcast msg: " + message.getPayload()); } break; } case broadcastResponse: { context.cancelTimeout("broadcast-" + message.getHeader(Neo4Net.cluster.com.message.Message.HEADER_CONVERSATION_ID)); if(message.getPayload() instanceof Neo4Net.cluster.protocol.cluster.ClusterMessage.ConfigurationChangeState) { outgoing.offer(message.copyHeadersTo(internal(Neo4Net.cluster.protocol.cluster.ClusterMessage.configurationChanged, message.getPayload()))); Neo4Net.cluster.protocol.cluster.ClusterMessage.ConfigurationChangeState change = message.getPayload(); if(change.getJoinUri() != null) { outgoing.offer(message.copyHeadersTo(Neo4Net.cluster.com.message.Message.internal(Neo4Net.cluster.protocol.heartbeat.HeartbeatMessage.i_am_alive, new Neo4Net.cluster.protocol.heartbeat.HeartbeatMessage.IAmAliveState(change.getJoin())), Neo4Net.cluster.com.message.Message.HEADER_FROM)); } } else { context.receive(message.getPayload()); } break; } case broadcastTimeout: { break; } case leave: { return start; } default: { defaultHandling(context, message); } } return this; } };
		 broadcasting
		 {
			 public AtomicBroadcastState handle( AtomicBroadcastContext context, Message<AtomicBroadcastMessage> message, MessageHolder outgoing )
			 {
