...
//	import static org.Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.LearnerContext_Fields.LEARN_GAP_THRESHOLD;
...
//ORIGINAL LINE: start { @Override public LearnerState handle(LearnerContext context, org.Neo4Net.cluster.com.message.Message<LearnerMessage> message, org.Neo4Net.cluster.com.message.MessageHolder outgoing) { if(message.getMessageType() == LearnerMessage.join) { return learner; } return this; } },
		 start
		 {
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: learner { @Override public LearnerState handle(LearnerContext context, org.Neo4Net.cluster.com.message.Message<LearnerMessage> message, org.Neo4Net.cluster.com.message.MessageHolder outgoing) throws java.io.IOException, ClassNotFoundException, java.net.URISyntaxException { switch(message.getMessageType()) { case learn: { LearnerMessage.LearnState learnState = message.getPayload(); final InstanceId instanceId = new InstanceId(message); PaxosInstance instance = context.getPaxosInstance(instanceId); org.Neo4Net.logging.Log log = context.getLog(getClass()); if(instanceId.getId() <= context.getLastDeliveredInstanceId()) { break; } context.learnedInstanceId(instanceId.getId()); instance.closed(learnState.getValue(), message.getHeader(org.Neo4Net.cluster.com.message.Message.HEADER_CONVERSATION_ID)); if(log.isDebugEnabled()) { String description; if(instance.value_2 instanceof org.Neo4Net.cluster.protocol.atomicbroadcast.Payload) { org.Neo4Net.cluster.protocol.atomicbroadcast.AtomicBroadcastSerializer atomicBroadcastSerializer = context.newSerializer(); description = atomicBroadcastSerializer.receive((org.Neo4Net.cluster.protocol.atomicbroadcast.Payload) instance.value_2).toString(); } else { description = instance.value_2.toString(); } log.debug("Learned and closed instance " + instance.id + " from conversation " + instance.conversationIdHeader + " and the content was " + description); } if(context.isMe(context.getCoordinator()) && instanceId.getId() != context.getLastDeliveredInstanceId() + 1) { context.getLog(LearnerState.class).debug("Gap developed in delivered instances," + "latest received was %s but last delivered was %d.", instanceId, context.getLastDeliveredInstanceId()); if(instanceId.getId() > context.getLastDeliveredInstanceId() + LEARN_GAP_THRESHOLD) { context.getLog(LearnerState.class).debug("Gap threshold reached (%d), proceeding to deliver everything pending " + "up until now", LEARN_GAP_THRESHOLD); boolean currentInstanceFound = false; long checkInstanceId = context.getLastDeliveredInstanceId() + 1; final long startingInstanceId = checkInstanceId; while ((instance = context.getPaxosInstance(new InstanceId(checkInstanceId))) != null) { if(checkInstanceId == instanceId.getId()) { currentInstanceFound = true; } instance.delivered(); context.setLastDeliveredInstanceId(checkInstanceId); org.Neo4Net.cluster.com.message.Message<AtomicBroadcastMessage> learnMessage = org.Neo4Net.cluster.com.message.Message.internal(AtomicBroadcastMessage.broadcastResponse, instance.value_2).setHeader(InstanceId.INSTANCE, instance.id.toString()).setHeader(org.Neo4Net.cluster.com.message.Message.HEADER_CONVERSATION_ID, instance.conversationIdHeader); outgoing.offer(learnMessage); checkInstanceId++; } context.getLog(LearnerMessage.LearnState.class).debug("Delivered everything from %d up until %d. Triggering message was %s, delivered: %b", startingInstanceId, checkInstanceId - 1, instanceId, currentInstanceFound); } } else { if(instanceId.getId() == context.getLastDeliveredInstanceId() + 1) { instance.delivered(); outgoing.offer(org.Neo4Net.cluster.com.message.Message.internal(AtomicBroadcastMessage.broadcastResponse, learnState.getValue()).setHeader(InstanceId.INSTANCE, instance.id.toString()).setHeader(org.Neo4Net.cluster.com.message.Message.HEADER_CONVERSATION_ID, instance.conversationIdHeader)); context.setLastDeliveredInstanceId(instanceId.getId()); long checkInstanceId = instanceId.getId() + 1; while ((instance = context.getPaxosInstance(new InstanceId(checkInstanceId))).isState(PaxosInstance.State.closed)) { instance.delivered(); context.setLastDeliveredInstanceId(checkInstanceId); org.Neo4Net.cluster.com.message.Message<AtomicBroadcastMessage> learnMessage = org.Neo4Net.cluster.com.message.Message.internal(AtomicBroadcastMessage.broadcastResponse, instance.value_2).setHeader(InstanceId.INSTANCE, instance.id.toString()).setHeader(org.Neo4Net.cluster.com.message.Message.HEADER_CONVERSATION_ID, instance.conversationIdHeader); outgoing.offer(learnMessage); checkInstanceId++; } if(checkInstanceId == context.getLastKnownLearnedInstanceInCluster() + 1) { context.cancelTimeout("learn"); } else { context.getLog(LearnerState.class).debug("*** HOLE! WAITING " + "FOR " + (context.getLastDeliveredInstanceId() + 1)); } } else { context.getLog(LearnerState.class).debug("*** GOT " + instanceId + ", WAITING FOR " + (context.getLastDeliveredInstanceId() + 1)); context.setTimeout("learn", org.Neo4Net.cluster.com.message.Message.timeout(LearnerMessage.learnTimedout, message)); } } break; } case learnTimedout: { if(!context.hasDeliveredAllKnownInstances()) { for(long instanceId = context.getLastDeliveredInstanceId() + 1; instanceId < context.getLastKnownLearnedInstanceInCluster(); instanceId++) { InstanceId id = new InstanceId(instanceId); PaxosInstance instance = context.getPaxosInstance(id); if(!instance.isState(PaxosInstance.State.closed) && !instance.isState(PaxosInstance.State.delivered)) { for(org.Neo4Net.cluster.InstanceId node : context.getAlive()) { java.net.URI nodeUri = context.getUriForId(node); if(!node.equals(context.getMyId())) { outgoing.offer(org.Neo4Net.cluster.com.message.Message.to(LearnerMessage.learnRequest, nodeUri, new LearnerMessage.LearnRequestState()).setHeader(InstanceId.INSTANCE, id.toString())); } } } } context.setTimeout("learn", org.Neo4Net.cluster.com.message.Message.timeout(LearnerMessage.learnTimedout, message)); } break; } case learnRequest: { InstanceId instanceId = new InstanceId(message); PaxosInstance instance = context.getPaxosInstance(instanceId); if(instance.isState(PaxosInstance.State.closed) || instance.isState(PaxosInstance.State.delivered)) { outgoing.offer(org.Neo4Net.cluster.com.message.Message.respond(LearnerMessage.learn, message, new LearnerMessage.LearnState(instance.value_2)).setHeader(InstanceId.INSTANCE, instanceId.toString()).setHeader(org.Neo4Net.cluster.com.message.Message.HEADER_CONVERSATION_ID, instance.conversationIdHeader)); } else { outgoing.offer(message.copyHeadersTo(org.Neo4Net.cluster.com.message.Message.respond(LearnerMessage.learnFailed, message, new LearnerMessage.LearnFailedState()), org.Neo4Net.cluster.protocol.atomicbroadcast.multipaxos.InstanceId.INSTANCE)); } break; } case learnFailed: { InstanceId instanceId = new InstanceId(message); context.notifyLearnMiss(instanceId); break; } case catchUp: { long catchUpTo = message.getPayload<long>(); if(context.getLastKnownLearnedInstanceInCluster() < catchUpTo) { context.setNextInstanceId(catchUpTo + 1); for(long instanceId = context.getLastLearnedInstanceId() + 1; instanceId <= catchUpTo; instanceId++) { InstanceId id = new InstanceId(instanceId); PaxosInstance instance = context.getPaxosInstance(id); if(!instance.isState(PaxosInstance.State.closed) && !instance.isState(PaxosInstance.State.delivered)) { outgoing.offer(org.Neo4Net.cluster.com.message.Message.to(LearnerMessage.learnRequest, lastKnownAliveUriOrSenderUri(context, message), new LearnerMessage.LearnRequestState()).setHeader(InstanceId.INSTANCE, id.toString())); context.setTimeout("learn", org.Neo4Net.cluster.com.message.Message.timeout(LearnerMessage.learnTimedout, message)); break; } } org.Neo4Net.cluster.InstanceId instanceId = message.hasHeader(org.Neo4Net.cluster.com.message.Message.HEADER_INSTANCE_ID) ? new org.Neo4Net.cluster.InstanceId(int.Parse(message.getHeader(org.Neo4Net.cluster.com.message.Message.HEADER_INSTANCE_ID))) : context.getMyId(); context.setLastKnownLearnedInstanceInCluster(catchUpTo, instanceId); } break; } case leave: { context.leave(); return start; } default: break; } return this; } private java.net.URI lastKnownAliveUriOrSenderUri(LearnerContext context, org.Neo4Net.cluster.com.message.Message<LearnerMessage> message) throws java.net.URISyntaxException { org.Neo4Net.cluster.InstanceId lastKnownAliveInstance = context.getLastKnownAliveUpToDateInstance(); if(lastKnownAliveInstance != null) { return context.getUriForId(lastKnownAliveInstance); } else { return new java.net.URI(message.getHeader(org.Neo4Net.cluster.com.message.Message.HEADER_FROM)); } } }
		 learner
		 {
