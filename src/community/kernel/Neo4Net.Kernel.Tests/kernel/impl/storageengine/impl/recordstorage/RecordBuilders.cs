...
namespace Neo4Net.Kernel.impl.storageengine.impl.recordstorage
{
	...
	{
		 public static IList<R> Records<R, A>( IEnumerable<Neo4Net.Kernel.impl.transaction.state.RecordAccess_RecordProxy<R, A>> changes ) where R : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
//JAVA TO C# CONVERTER TODO TASK: Method reference arbitrary object instance method syntax is not converted by Java to C# Converter:
			  return StreamSupport.stream( changes.spliterator(), false ).map(Neo4Net.Kernel.impl.transaction.state.RecordAccess_RecordProxy::forChangingData).collect(Collectors.toList());
		 }
		 ...
		 private class Loader<T, E> : Neo4Net.Kernel.impl.transaction.state.RecordAccess_Loader<T, E> where T : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
