...
//ORIGINAL LINE: private void applyExternalTransaction(long transactionId, Neo4Net.kernel.impl.transaction.command.Command...commands) throws Exception
		 ...
		 private static RECORD InUse<RECORD>( RECORD record ) where RECORD : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
			  record.InUse = true;
			  return record;
		 }

		 private static RECORD Created<RECORD>( RECORD record ) where RECORD : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
			  record.setCreated();
			  return record;
		 }
