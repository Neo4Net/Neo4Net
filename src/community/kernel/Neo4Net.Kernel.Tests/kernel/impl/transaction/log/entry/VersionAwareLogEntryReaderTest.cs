...
//ORIGINAL LINE: final LogEntryStart start = new LogEntryStart(version, 1, 2, 3, 4, new byte[]{5}, new Neo4Net.kernel.impl.transaction.log.LogPosition(0, 31));
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldReadACheckPointLogEntry()
		 {
			  ...
			  LogEntryVersion version = LogEntryVersion.CURRENT;
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final Neo4Net.kernel.impl.transaction.log.LogPosition logPosition = new Neo4Net.kernel.impl.transaction.log.LogPosition(42, 43);
...
//ORIGINAL LINE: final Neo4Net.kernel.impl.transaction.log.InMemoryClosableChannel channel = new Neo4Net.kernel.impl.transaction.log.InMemoryClosableChannel();
			  ...
			  channel.Put( version.byteCode() );
			  channel.Put( LogEntryByteCodes.Command );
			  channel.Put( Neo4Net.Kernel.impl.transaction.command.NeoCommandType_Fields.None );
