...
//ORIGINAL LINE: final Neo4Net.kernel.impl.transaction.log.entry.LogEntryReader<ReadableLogChannel> logEntryReader = new Neo4Net.kernel.impl.transaction.log.entry.VersionAwareLogEntryReader<>();
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldKernelPanicIfTransactionIdsMismatch()
		 {
			  ...
			  try
			  {
					appender.Append( batch, Neo4Net.Kernel.impl.transaction.tracing.LogAppendEvent_Fields.Null );
					fail( "should have thrown " );
			  }
