...
//ORIGINAL LINE: TX_START { @Override public LogEntry parse(LogEntryVersion version, Neo4Net.kernel.impl.transaction.log.ReadableClosableChannel channel, Neo4Net.kernel.impl.transaction.log.LogPositionMarker marker, Neo4Net.Kernel.Api.StorageEngine.CommandReaderFactory commandReader) throws java.io.IOException { Neo4Net.kernel.impl.transaction.log.LogPosition position = marker.newPosition(); int masterId = channel.getInt(); int authorId = channel.getInt(); long timeWritten = channel.getLong(); long latestCommittedTxWhenStarted = channel.getLong(); int additionalHeaderLength = channel.getInt(); byte[] additionalHeader = new byte[additionalHeaderLength]; channel.get(additionalHeader, additionalHeaderLength); return new LogEntryStart(version, masterId, authorId, timeWritten, latestCommittedTxWhenStarted, additionalHeader, position); } @Override public byte byteCode() { return LogEntryByteCodes.TX_START; } @Override public boolean skip() { return false; } },
		 TX_START
		 {
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: COMMAND { @Override public LogEntry parse(LogEntryVersion version, Neo4Net.kernel.impl.transaction.log.ReadableClosableChannel channel, Neo4Net.kernel.impl.transaction.log.LogPositionMarker marker, Neo4Net.Kernel.Api.StorageEngine.CommandReaderFactory commandReader) throws java.io.IOException { Neo4Net.Kernel.Api.StorageEngine.StorageCommand command = commandReader.byVersion(version.byteCode()).read(channel); return command == null ? null : new LogEntryCommand(version, command); } @Override public byte byteCode() { return LogEntryByteCodes.COMMAND; } @Override public boolean skip() { return false; } },
		 COMMAND
		 {
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: TX_COMMIT { @Override public LogEntry parse(LogEntryVersion version, Neo4Net.kernel.impl.transaction.log.ReadableClosableChannel channel, Neo4Net.kernel.impl.transaction.log.LogPositionMarker marker, Neo4Net.Kernel.Api.StorageEngine.CommandReaderFactory commandReader) throws java.io.IOException { long txId = channel.getLong(); long timeWritten = channel.getLong(); return new LogEntryCommit(version, txId, timeWritten); } @Override public byte byteCode() { return LogEntryByteCodes.TX_COMMIT; } @Override public boolean skip() { return false; } },
		 TX_COMMIT
		 {
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: CHECK_POINT { @Override public LogEntry parse(LogEntryVersion version, Neo4Net.kernel.impl.transaction.log.ReadableClosableChannel channel, Neo4Net.kernel.impl.transaction.log.LogPositionMarker marker, Neo4Net.Kernel.Api.StorageEngine.CommandReaderFactory commandReader) throws java.io.IOException { long logVersion = channel.getLong(); long byteOffset = channel.getLong(); return new CheckPoint(version, new Neo4Net.kernel.impl.transaction.log.LogPosition(logVersion, byteOffset)); } @Override public byte byteCode() { return LogEntryByteCodes.CHECK_POINT; } @Override public boolean skip() { return false; } }
		 CHECK_POINT
		 {
