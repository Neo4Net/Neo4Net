...
//	import static Neo4Net.kernel.impl.transaction.log.entry.LogEntryStart.checksum;
	...
	{
		 // For the graph store and schema indexes order-of-updates are managed by the high level IEntity locks
			  ...
			  lock ( _logFile )
			  {
					...
					using ( SerializeTransactionEvent serialiseEvent = logAppendEvent.BeginSerializeTransaction() )
					{
						 ...
						 while ( tx != null )
						 {
							  ...
							  // id have already been generated by another IEntity we simply check that our id
