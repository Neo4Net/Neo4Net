...
//       GEOMETRY_INVALID(0, "Invalid") { public Neo4Net.values.storable.Value decode(Neo4Net.values.storable.CoordinateReferenceSystem crs, int dimension, long[] valueBlocks, int offset) { throw new UnsupportedOperationException("Cannot decode invalid geometry"); } public int calculateNumberOfBlocksUsedForGeometry(long firstBlock) { return PropertyType.BLOCKS_USED_FOR_BAD_TYPE_OR_ENCODING; } public Neo4Net.values.storable.ArrayValue decodeArray(GeometryHeader header, byte[] data) { throw new UnsupportedOperationException("Cannot decode invalid geometry array"); } },
//JAVA TO C# CONVERTER TODO TASK: Enum value-specific class bodies are not converted by Java to C# Converter:
//       GEOMETRY_POINT(1, "Point") { public Neo4Net.values.storable.Value decode(Neo4Net.values.storable.CoordinateReferenceSystem crs, int dimension, long[] valueBlocks, int offset) { double[] coordinate = new double[dimension]; for(int i = 0; i < dimension; i++) { coordinate[i] = Double.longBitsToDouble(valueBlocks[i + 1 + offset]); } return Neo4Net.values.storable.Values.pointValue(crs, coordinate); } public int calculateNumberOfBlocksUsedForGeometry(long firstBlock) { int dimension = getDimension(firstBlock); if(dimension > GeometryType.getMaxSupportedDimensions()) { return PropertyType.BLOCKS_USED_FOR_BAD_TYPE_OR_ENCODING; } return 1 + dimension; } public Neo4Net.values.storable.ArrayValue decodeArray(GeometryHeader header, byte[] data) { byte[] dataHeader = PropertyType.ARRAY.readDynamicRecordHeader(data); byte[] dataBody = new byte[data.length - dataHeader.length]; System.arraycopy(data, dataHeader.length, dataBody, 0, dataBody.length); Neo4Net.values.storable.Value dataValue = DynamicArrayStore.getRightArray(Neo4Net.helpers.collection.Pair.of(dataHeader, dataBody)); if(dataValue instanceof Neo4Net.values.storable.FloatingPointArray) { Neo4Net.values.storable.FloatingPointArray numbers = (Neo4Net.values.storable.FloatingPointArray) dataValue; PointValue[] points = new Neo4Net.values.storable.PointValue[numbers.length() / header.dimension]; for(int i = 0; i < points.length; i++) { double[] coords = new double[header.dimension]; for(int d = 0; d < header.dimension; d++) { coords[d] = numbers.doubleValue(i * header.dimension + d); } points[i] = Neo4Net.values.storable.Values.pointValue(header.crs, coords); } return Neo4Net.values.storable.Values.pointArray(points); } else { throw new InvalidRecordException("Point array with unexpected type. Actual:" + dataValue.getClass().getSimpleName() + ". Expected: FloatingPointArray."); } } };
