...
namespace Neo4Net.Kernel.impl.store.kvstore
{
	...
	internal class ConcurrentMapState<Key> : ActiveState<Key>
	{
		 ...
		 {
			  _hasTrackedChanges.set( true );
			  return new Updater<Key>( @lock, Store, _changes, null, Neo4Net.Kernel.impl.transaction.log.TransactionIdStore_Fields.BASE_TX_ID );
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: private static <Key> boolean performLookup(ReadableState<Key> store, Neo4Net.io.pagecache.tracing.cursor.context.VersionContext versionContext, java.util.concurrent.ConcurrentMap<Key,ChangeEntry> changes, Key key, ValueSink sink) throws java.io.IOException
