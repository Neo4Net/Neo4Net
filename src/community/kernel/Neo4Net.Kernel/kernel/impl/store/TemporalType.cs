...
//       TEMPORAL_INVALID(0, "Invalid") { public Neo4Net.values.storable.Value decodeForTemporal(long[] valueBlocks, int offset) { throw new UnsupportedOperationException("Cannot decode invalid temporal"); } public int calculateNumberOfBlocksUsedForTemporal(long firstBlock) { return PropertyType.BLOCKS_USED_FOR_BAD_TYPE_OR_ENCODING; } public Neo4Net.values.storable.ArrayValue decodeArray(Neo4Net.values.storable.Value dataValue) { throw new UnsupportedOperationException("Cannot decode invalid temporal array"); } },
//JAVA TO C# CONVERTER TODO TASK: Enum value-specific class bodies are not converted by Java to C# Converter:
//       TEMPORAL_DATE(1, "Date") { public Neo4Net.values.storable.Value decodeForTemporal(long[] valueBlocks, int offset) { long epochDay = valueIsInlined(valueBlocks[offset]) ? valueBlocks[offset] >>> 33 : valueBlocks[1 + offset]; return Neo4Net.values.storable.DateValue.epochDate(epochDay); } public int calculateNumberOfBlocksUsedForTemporal(long firstBlock) { return valueIsInlined(firstBlock) ? BLOCKS_LONG_INLINED : BLOCKS_LONG_NOT_INLINED; } public Neo4Net.values.storable.ArrayValue decodeArray(Neo4Net.values.storable.Value dataValue) { if(dataValue instanceof Neo4Net.values.storable.LongArray) { Neo4Net.values.storable.LongArray numbers = (Neo4Net.values.storable.LongArray) dataValue; LocalDate[] dates = new java.time.LocalDate[numbers.length()]; for(int i = 0; i < dates.length; i++) { dates[i] = java.time.LocalDate.ofEpochDay(numbers.longValue(i)); } return Neo4Net.values.storable.Values.dateArray(dates); } else { throw new InvalidRecordException("Array with unexpected type. Actual:" + dataValue.getClass().getSimpleName() + ". Expected: LongArray."); } } private boolean valueIsInlined(long firstBlock) { return(firstBlock & 0x100000000L) > 0; } },
//JAVA TO C# CONVERTER TODO TASK: Enum value-specific class bodies are not converted by Java to C# Converter:
//       TEMPORAL_LOCAL_TIME(2, "LocalTime") { public Neo4Net.values.storable.Value decodeForTemporal(long[] valueBlocks, int offset) { long nanoOfDay = valueIsInlined(valueBlocks[offset]) ? valueBlocks[offset] >>> 33 : valueBlocks[1 + offset]; checkValidNanoOfDay(nanoOfDay); return Neo4Net.values.storable.LocalTimeValue.localTime(nanoOfDay); } public int calculateNumberOfBlocksUsedForTemporal(long firstBlock) { return valueIsInlined(firstBlock) ? BLOCKS_LONG_INLINED : BLOCKS_LONG_NOT_INLINED; } public Neo4Net.values.storable.ArrayValue decodeArray(Neo4Net.values.storable.Value dataValue) { if(dataValue instanceof Neo4Net.values.storable.LongArray) { Neo4Net.values.storable.LongArray numbers = (Neo4Net.values.storable.LongArray) dataValue; LocalTime[] times = new java.time.LocalTime[numbers.length()]; for(int i = 0; i < times.length; i++) { long nanoOfDay = numbers.longValue(i); checkValidNanoOfDay(nanoOfDay); times[i] = java.time.LocalTime.ofNanoOfDay(nanoOfDay); } return Neo4Net.values.storable.Values.localTimeArray(times); } else { throw new InvalidRecordException("Array with unexpected type. Actual:" + dataValue.getClass().getSimpleName() + ". Expected: LongArray."); } } private boolean valueIsInlined(long firstBlock) { return(firstBlock & 0x100000000L) > 0; } },
//JAVA TO C# CONVERTER TODO TASK: Enum value-specific class bodies are not converted by Java to C# Converter:
//       TEMPORAL_LOCAL_DATE_TIME(3, "LocalDateTime") { public Neo4Net.values.storable.Value decodeForTemporal(long[] valueBlocks, int offset) { long nanoOfSecond = valueBlocks[offset] >>> 32; checkValidNanoOfSecond(nanoOfSecond); long epochSecond = valueBlocks[1 + offset]; return Neo4Net.values.storable.LocalDateTimeValue.localDateTime(epochSecond, nanoOfSecond); } public int calculateNumberOfBlocksUsedForTemporal(long firstBlock) { return BLOCKS_LOCAL_DATETIME; } public Neo4Net.values.storable.ArrayValue decodeArray(Neo4Net.values.storable.Value dataValue) { if(dataValue instanceof Neo4Net.values.storable.LongArray) { Neo4Net.values.storable.LongArray numbers = (Neo4Net.values.storable.LongArray) dataValue; LocalDateTime[] dateTimes = new java.time.LocalDateTime[numbers.length() / BLOCKS_LOCAL_DATETIME]; for(int i = 0; i < dateTimes.length; i++) { long epochSecond = numbers.longValue(i * BLOCKS_LOCAL_DATETIME); long nanoOfSecond = numbers.longValue(i * BLOCKS_LOCAL_DATETIME + 1); checkValidNanoOfSecond(nanoOfSecond); dateTimes[i] = java.time.LocalDateTime.ofInstant(java.time.Instant.ofEpochSecond(epochSecond, nanoOfSecond), UTC); } return Neo4Net.values.storable.Values.localDateTimeArray(dateTimes); } else { throw new InvalidRecordException("Array with unexpected type. Actual:" + dataValue.getClass().getSimpleName() + ". Expected: LongArray."); } } },
//JAVA TO C# CONVERTER TODO TASK: Enum value-specific class bodies are not converted by Java to C# Converter:
//       TEMPORAL_TIME(4, "Time") { public Neo4Net.values.storable.Value decodeForTemporal(long[] valueBlocks, int offset) { int secondOffset = (int)(valueBlocks[offset] >>> 32); long nanoOfDay = valueBlocks[1 + offset]; checkValidNanoOfDayWithOffset(nanoOfDay, secondOffset); return Neo4Net.values.storable.TimeValue.time(nanoOfDay, java.time.ZoneOffset.ofTotalSeconds(secondOffset)); } public int calculateNumberOfBlocksUsedForTemporal(long firstBlock) { return BLOCKS_TIME; } public Neo4Net.values.storable.ArrayValue decodeArray(Neo4Net.values.storable.Value dataValue) { if(dataValue instanceof Neo4Net.values.storable.LongArray) { Neo4Net.values.storable.LongArray numbers = (Neo4Net.values.storable.LongArray) dataValue; OffsetTime[] times = new java.time.OffsetTime[(int)(numbers.length() / BLOCKS_TIME)]; for(int i = 0; i < times.length; i++) { long nanoOfDay = numbers.longValue(i * BLOCKS_TIME); int secondOffset = (int) numbers.longValue(i * BLOCKS_TIME + 1); checkValidNanoOfDay(nanoOfDay); times[i] = java.time.OffsetTime.of(java.time.LocalTime.ofNanoOfDay(nanoOfDay), java.time.ZoneOffset.ofTotalSeconds(secondOffset)); } return Neo4Net.values.storable.Values.timeArray(times); } else { throw new InvalidRecordException("Array with unexpected type. Actual:" + dataValue.getClass().getSimpleName() + ". Expected: LongArray."); } } },
//JAVA TO C# CONVERTER TODO TASK: Enum value-specific class bodies are not converted by Java to C# Converter:
//       TEMPORAL_DATE_TIME(5, "DateTime") { public Neo4Net.values.storable.Value decodeForTemporal(long[] valueBlocks, int offset) { if(storingZoneOffset(valueBlocks[offset])) { int nanoOfSecond = (int)(valueBlocks[offset] >>> 33); checkValidNanoOfSecond(nanoOfSecond); long epochSecond = valueBlocks[1 + offset]; int secondOffset = (int) valueBlocks[2 + offset]; return Neo4Net.values.storable.DateTimeValue.datetime(epochSecond, nanoOfSecond, java.time.ZoneOffset.ofTotalSeconds(secondOffset)); } else { int nanoOfSecond = (int)(valueBlocks[offset] >>> 33); checkValidNanoOfSecond(nanoOfSecond); long epochSecond = valueBlocks[1 + offset]; short zoneNumber = (short) valueBlocks[2 + offset]; return Neo4Net.values.storable.DateTimeValue.datetime(epochSecond, nanoOfSecond, java.time.ZoneId.of(Neo4Net.values.storable.TimeZones.map(zoneNumber))); } } public int calculateNumberOfBlocksUsedForTemporal(long firstBlock) { return BLOCKS_DATETIME; } public Neo4Net.values.storable.ArrayValue decodeArray(Neo4Net.values.storable.Value dataValue) { if(dataValue instanceof Neo4Net.values.storable.LongArray) { Neo4Net.values.storable.LongArray numbers = (Neo4Net.values.storable.LongArray) dataValue; ZonedDateTime[] dateTimes = new java.time.ZonedDateTime[numbers.length() / BLOCKS_DATETIME]; for(int i = 0; i < dateTimes.length; i++) { long epochSecond = numbers.longValue(i * BLOCKS_DATETIME); long nanoOfSecond = numbers.longValue(i * BLOCKS_DATETIME + 1); checkValidNanoOfSecond(nanoOfSecond); long zoneValue = numbers.longValue(i * BLOCKS_DATETIME + 2); if((zoneValue & 1) == 1) { int secondOffset = (int)(zoneValue >>> 1); dateTimes[i] = java.time.ZonedDateTime.ofInstant(java.time.Instant.ofEpochSecond(epochSecond, nanoOfSecond), java.time.ZoneOffset.ofTotalSeconds(secondOffset)); } else { short zoneNumber = (short)(zoneValue >>> 1); dateTimes[i] = java.time.ZonedDateTime.ofInstant(java.time.Instant.ofEpochSecond(epochSecond, nanoOfSecond), java.time.ZoneId.of(Neo4Net.values.storable.TimeZones.map(zoneNumber))); } } return Neo4Net.values.storable.Values.dateTimeArray(dateTimes); } else { throw new InvalidRecordException("LocalTime array with unexpected type. Actual:" + dataValue.getClass().getSimpleName() + ". Expected: LongArray."); } } private boolean storingZoneOffset(long firstBlock) { return(firstBlock & 0x100000000L) > 0; } },
//JAVA TO C# CONVERTER TODO TASK: Enum value-specific class bodies are not converted by Java to C# Converter:
//       TEMPORAL_DURATION(6, "Duration") { public Neo4Net.values.storable.Value decodeForTemporal(long[] valueBlocks, int offset) { int nanos = (int)(valueBlocks[offset] >>> 32); long months = valueBlocks[1 + offset]; long days = valueBlocks[2 + offset]; long seconds = valueBlocks[3 + offset]; return Neo4Net.values.storable.DurationValue.duration(months, days, seconds, nanos); } public int calculateNumberOfBlocksUsedForTemporal(long firstBlock) { return BLOCKS_DURATION; } public Neo4Net.values.storable.ArrayValue decodeArray(Neo4Net.values.storable.Value dataValue) { if(dataValue instanceof Neo4Net.values.storable.LongArray) { Neo4Net.values.storable.LongArray numbers = (Neo4Net.values.storable.LongArray) dataValue; DurationValue[] durations = new Neo4Net.values.storable.DurationValue[(int)(numbers.length() / BLOCKS_DURATION)]; for(int i = 0; i < durations.length; i++) { durations[i] = Neo4Net.values.storable.DurationValue.duration(numbers.longValue(i * BLOCKS_DURATION), numbers.longValue(i * BLOCKS_DURATION + 1), numbers.longValue(i * BLOCKS_DURATION + 2), numbers.longValue(i * BLOCKS_DURATION + 3)); } return Neo4Net.values.storable.Values.durationArray(durations); } else { throw new InvalidRecordException("Array with unexpected type. Actual:" + dataValue.getClass().getSimpleName() + ". Expected: LongArray."); } } };
		 ...
		 public static Neo4Net.Values.Storable.ArrayValue decodeTemporalArray( TemporalHeader header, byte[] data ) { byte[] dataHeader = PropertyType.ARRAY.readDynamicRecordHeader( data ); byte[] dataBody = new byte[data.length - dataHeader.length]; System.arraycopy( data, dataHeader.length, dataBody, 0, dataBody.length ); Neo4Net.Values.Storable.Value dataValue = DynamicArrayStore.getRightArray( Neo4Net.Collections.Helpers.Pair.of( dataHeader, dataBody ) ); return find( header.temporalType ).decodeArray( dataValue ); } private final int temporalType = new TemporalType("private static int getTemporalType(long firstBlock) { return(int)((firstBlock & TEMPORAL_TYPE_MASK) >> 28); } public static int calculateNumberOfBlocksUsed(long firstBlock) { TemporalType geometryType = find(getTemporalType(firstBlock)); return geometryType.calculateNumberOfBlocksUsedForTemporal(firstBlock); } private static TemporalType find(int temporalType) { if(temporalType < TYPES.length && temporalType >= 0) { return TYPES[temporalType]; } else { return TEMPORAL_INVALID; } } private static void checkValidNanoOfDay(long nanoOfDay) { if(nanoOfDay > java.time.LocalTime.MAX.toNanoOfDay() || nanoOfDay < java.time.LocalTime.MIN.toNanoOfDay()) { throw new InvalidRecordException("Nanosecond of day out of range:" + nanoOfDay); } } private static void checkValidNanoOfDayWithOffset(long nanoOfDayUTC, int secondOffset) { long nanoOfDay = Neo4Net.values.utils.TemporalUtil.nanosOfDayToLocal(nanoOfDayUTC, secondOffset); checkValidNanoOfDay(nanoOfDay); } private static void checkValidNanoOfSecond(long nanoOfSecond) { if(nanoOfSecond > 999_999_999 || nanoOfSecond < 0) { throw new InvalidRecordException("Nanosecond of second out of range:" + nanoOfSecond); } } public static Neo4Net.values.storable.Value decode(Neo4Net.kernel.impl.store.record.PropertyBlock block) { return decode(block.getValueBlocks(), 0); } public static Neo4Net.values.storable.Value decode(long[] valueBlocks, int offset) { long firstBlock = valueBlocks[offset]; int temporalType = getTemporalType(firstBlock); return find(temporalType).decodeForTemporal(valueBlocks, offset); } public static long[] encodeDate(int keyId, long epochDay) { return encodeLong(keyId, epochDay, TemporalType.TEMPORAL_DATE.temporalType); } public static long[] encodeLocalTime(int keyId, long nanoOfDay) { return encodeLong(keyId, nanoOfDay, TemporalType.TEMPORAL_LOCAL_TIME.temporalType); } private static long[] encodeLong(int keyId, long val, int temporalType) { int idBits = Neo4Net.kernel.impl.store.format.standard.StandardFormatSettings.PROPERTY_TOKEN_MAXIMUM_ID_BITS; long keyAndType = keyId | (((long)(PropertyType.TEMPORAL.intValue()) << idBits)); long temporalTypeBits = temporalType << (idBits + 4); long[] data; if(ShortArray.LONG.getRequiredBits(val) <= 64 - 33) { data = new long[BLOCKS_LONG_INLINED]; data[0] = keyAndType | temporalTypeBits | (1L << 32) | (val << 33); } else { data = new long[BLOCKS_LONG_NOT_INLINED]; data[0] = keyAndType | temporalTypeBits; data[1] = val; } return data; } public static long[] encodeLocalDateTime(int keyId, long epochSecond, long nanoOfSecond) { int idBits = Neo4Net.kernel.impl.store.format.standard.StandardFormatSettings.PROPERTY_TOKEN_MAXIMUM_ID_BITS; long keyAndType = keyId | (((long)(PropertyType.TEMPORAL.intValue()) << idBits)); long temporalTypeBits = TemporalType.TEMPORAL_LOCAL_DATE_TIME.temporalType << (idBits + 4); long[] data = new long[BLOCKS_LOCAL_DATETIME]; data[0] = keyAndType | temporalTypeBits | (nanoOfSecond << 32); data[1] = epochSecond; return data; } public static long[] encodeDateTime(int keyId, long epochSecond, long nanoOfSecond, String zoneId) { int idBits = Neo4Net.kernel.impl.store.format.standard.StandardFormatSettings.PROPERTY_TOKEN_MAXIMUM_ID_BITS; short zoneNumber = Neo4Net.values.storable.TimeZones.map(zoneId); long keyAndType = keyId | (((long)(PropertyType.TEMPORAL.intValue()) << idBits)); long temporalTypeBits = TemporalType.TEMPORAL_DATE_TIME.temporalType << (idBits + 4); long[] data = new long[BLOCKS_DATETIME]; data[0] = keyAndType | temporalTypeBits | (nanoOfSecond << 33); data[1] = epochSecond; data[2] = zoneNumber; return data; } public static long[] encodeDateTime(int keyId, long epochSecond, long nanoOfSecond, int secondOffset) { int idBits = Neo4Net.kernel.impl.store.format.standard.StandardFormatSettings.PROPERTY_TOKEN_MAXIMUM_ID_BITS; long keyAndType = keyId | (((long)(PropertyType.TEMPORAL.intValue()) << idBits)); long temporalTypeBits = TemporalType.TEMPORAL_DATE_TIME.temporalType << (idBits + 4); long[] data = new long[BLOCKS_DATETIME]; data[0] = keyAndType | temporalTypeBits | (1L << 32) | (nanoOfSecond << 33); data[1] = epochSecond; data[2] = secondOffset; return data; } public static long[] encodeTime(int keyId, long nanoOfDayUTC, int secondOffset) { int idBits = Neo4Net.kernel.impl.store.format.standard.StandardFormatSettings.PROPERTY_TOKEN_MAXIMUM_ID_BITS; long keyAndType = keyId | (((long)(PropertyType.TEMPORAL.intValue()) << idBits)); long temporalTypeBits = TemporalType.TEMPORAL_TIME.temporalType << (idBits + 4); long[] data = new long[BLOCKS_TIME]; data[0] = keyAndType | temporalTypeBits | ((long) secondOffset << 32); data[1] = nanoOfDayUTC; return data; } public static long[] encodeDuration(int keyId, long months, long days, long seconds, int nanos) { int idBits = Neo4Net.kernel.impl.store.format.standard.StandardFormatSettings.PROPERTY_TOKEN_MAXIMUM_ID_BITS; long keyAndType = keyId | (((long)(PropertyType.TEMPORAL.intValue()) << idBits)); long temporalTypeBits = TemporalType.TEMPORAL_DURATION.temporalType << (idBits + 4); long[] data = new long[BLOCKS_DURATION]; data[0] = keyAndType | temporalTypeBits | ((long) nanos << 32); data[1] = months; data[2] = days; data[3] = seconds; return data; } public static byte[] encodeDateArray(java.time.LocalDate[] dates) { long[] data = new long[dates.length]; for(int i = 0; i < data.length; i++) { data[i] = dates[i].toEpochDay(); } TemporalHeader header = new TemporalHeader(TemporalType.TEMPORAL_DATE.temporalType); byte[] bytes = DynamicArrayStore.encodeFromNumbers(data, DynamicArrayStore.TEMPORAL_HEADER_SIZE); header.writeArrayHeaderTo(bytes); return bytes; } public static byte[] encodeLocalTimeArray(java.time.LocalTime[] times) { long[] data = new long[times.length]; for(int i = 0; i < data.length; i++) { data[i] = times[i].toNanoOfDay(); } TemporalHeader header = new TemporalHeader(TemporalType.TEMPORAL_LOCAL_TIME.temporalType); byte[] bytes = DynamicArrayStore.encodeFromNumbers(data, DynamicArrayStore.TEMPORAL_HEADER_SIZE); header.writeArrayHeaderTo(bytes); return bytes; } public static byte[] encodeLocalDateTimeArray(java.time.LocalDateTime[] dateTimes) { long[] data = new long[dateTimes.length * BLOCKS_LOCAL_DATETIME]; for(int i = 0; i < dateTimes.length; i++) { data[i * BLOCKS_LOCAL_DATETIME] = dateTimes[i].toEpochSecond(UTC); data[i * BLOCKS_LOCAL_DATETIME + 1] = dateTimes[i].getNano(); } TemporalHeader header = new TemporalHeader(TemporalType.TEMPORAL_LOCAL_DATE_TIME.temporalType); byte[] bytes = DynamicArrayStore.encodeFromNumbers(data, DynamicArrayStore.TEMPORAL_HEADER_SIZE); header.writeArrayHeaderTo(bytes); return bytes; } public static byte[] encodeTimeArray(java.time.OffsetTime[] times) { long[] data = new long[(int)(Math.ceil(times.length * BLOCKS_TIME))]; int i; for(i = 0; i < times.length; i++) { data[i * BLOCKS_TIME] = times[i].toLocalTime().toNanoOfDay(); data[i * BLOCKS_TIME + 1] = times[i].getOffset().getTotalSeconds(); } TemporalHeader header = new TemporalHeader(TemporalType.TEMPORAL_TIME.temporalType); byte[] bytes = DynamicArrayStore.encodeFromNumbers(data, DynamicArrayStore.TEMPORAL_HEADER_SIZE); header.writeArrayHeaderTo(bytes); return bytes; } public static byte[] encodeDateTimeArray(java.time.ZonedDateTime[] dateTimes) { long[] data = new long[dateTimes.length * BLOCKS_DATETIME]; int i; for(i = 0; i < dateTimes.length; i++) { data[i * BLOCKS_DATETIME] = dateTimes[i].toEpochSecond(); data[i * BLOCKS_DATETIME + 1] = dateTimes[i].getNano(); if(dateTimes[i].getZone() instanceof java.time.ZoneOffset) { java.time.ZoneOffset offset = (java.time.ZoneOffset) dateTimes[i].getZone(); int secondOffset = offset.getTotalSeconds(); data[i * BLOCKS_DATETIME + 2] = secondOffset << 1 | 1L; } else { String timeZoneId = dateTimes[i].getZone().getId(); short zoneNumber = Neo4Net.values.storable.TimeZones.map(timeZoneId); data[i * BLOCKS_DATETIME + 2] = zoneNumber << 1; } } TemporalHeader header = new TemporalHeader(TemporalType.TEMPORAL_DATE_TIME.temporalType); byte[] bytes = DynamicArrayStore.encodeFromNumbers(data, DynamicArrayStore.TEMPORAL_HEADER_SIZE); header.writeArrayHeaderTo(bytes); return bytes; } public static byte[] encodeDurationArray(Neo4Net.values.storable.DurationValue[] durations) { long[] data = new long[durations.length * BLOCKS_DURATION]; for(int i = 0; i < durations.length; i++) { data[i * BLOCKS_DURATION] = durations[i].get(java.time.temporal.ChronoUnit.MONTHS); data[i * BLOCKS_DURATION + 1] = durations[i].get(java.time.temporal.ChronoUnit.DAYS); data[i * BLOCKS_DURATION + 2] = durations[i].get(java.time.temporal.ChronoUnit.SECONDS); data[i * BLOCKS_DURATION + 3] = durations[i].get(java.time.temporal.ChronoUnit.NANOS); } TemporalHeader header = new TemporalHeader(TemporalType.TEMPORAL_DURATION.temporalType); byte[] bytes = DynamicArrayStore.encodeFromNumbers(data, DynamicArrayStore.TEMPORAL_HEADER_SIZE); header.writeArrayHeaderTo(bytes); return bytes; } public static Neo4Net.values.storable.ArrayValue decodeTemporalArray(TemporalHeader header, byte[] data) { byte[] dataHeader = PropertyType.ARRAY.readDynamicRecordHeader(data); byte[] dataBody = new byte[data.length - dataHeader.length]; System.arraycopy(data, dataHeader.length, dataBody, 0, dataBody.length); Neo4Net.values.storable.Value dataValue = DynamicArrayStore.getRightArray(Neo4Net.helpers.collection.Pair.of(dataHeader, dataBody)); return find(header.temporalType).decodeArray(dataValue); } private final int temporalType", InnerEnum.private static int getTemporalType(long firstBlock) { return(int)((firstBlock & TEMPORAL_TYPE_MASK) >> 28); } public static int calculateNumberOfBlocksUsed(long firstBlock) { TemporalType geometryType = find(getTemporalType(firstBlock)); return geometryType.calculateNumberOfBlocksUsedForTemporal(firstBlock); } private static TemporalType find(int temporalType)
		 {
			 if ( temporalType < TYPES.length && temporalType >= 0 ) { return TYPES[temporalType]; } else { return TEMPORAL_INVALID; }
		 }
		 private static void checkValidNanoOfDay( long nanoOfDay )
