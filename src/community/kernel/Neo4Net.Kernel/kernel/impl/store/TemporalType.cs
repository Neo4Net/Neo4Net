...
//       TEMPORAL_DURATION(6, "Duration") { public org.Neo4Net.values.storable.Value decodeForTemporal(long[] valueBlocks, int offset) { int nanos = (int)(valueBlocks[offset] >>> 32); long months = valueBlocks[1 + offset]; long days = valueBlocks[2 + offset]; long seconds = valueBlocks[3 + offset]; return org.Neo4Net.values.storable.DurationValue.duration(months, days, seconds, nanos); } public int calculateNumberOfBlocksUsedForTemporal(long firstBlock) { return BLOCKS_DURATION; } public org.Neo4Net.values.storable.ArrayValue decodeArray(org.Neo4Net.values.storable.Value dataValue) { if(dataValue instanceof org.Neo4Net.values.storable.LongArray) { org.Neo4Net.values.storable.LongArray numbers = (org.Neo4Net.values.storable.LongArray) dataValue; DurationValue[] durations = new org.Neo4Net.values.storable.DurationValue[(int)(numbers.length() / BLOCKS_DURATION)]; for(int i = 0; i < durations.length; i++) { durations[i] = org.Neo4Net.values.storable.DurationValue.duration(numbers.longValue(i * BLOCKS_DURATION), numbers.longValue(i * BLOCKS_DURATION + 1), numbers.longValue(i * BLOCKS_DURATION + 2), numbers.longValue(i * BLOCKS_DURATION + 3)); } return org.Neo4Net.values.storable.Values.durationArray(durations); } else { throw new InvalidRecordException("Array with unexpected type. Actual:" + dataValue.getClass().getSimpleName() + ". Expected: LongArray."); } } };
		 ...
		 public static Neo4Net.Values.Storable.ArrayValue decodeTemporalArray( TemporalHeader header, byte[] data ) { byte[] dataHeader = PropertyType.ARRAY.readDynamicRecordHeader( data ); byte[] dataBody = new byte[data.length - dataHeader.length]; System.arraycopy( data, dataHeader.length, dataBody, 0, dataBody.length ); Neo4Net.Values.Storable.Value dataValue = DynamicArrayStore.getRightArray( Neo4Net.Collections.Helpers.Pair.of( dataHeader, dataBody ) ); return find( header.temporalType ).decodeArray( dataValue ); } private final int temporalType);
