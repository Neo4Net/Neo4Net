...
namespace Neo4Net.Kernel.Impl.Index.Schema
{
	...
	public class NativeDistinctValuesProgressor<KEY, VALUE> : NativeIndexProgressor<KEY, VALUE> where KEY : NativeIndexKey<KEY> where VALUE : NativeIndexValue
	{
		 ...
		 internal NativeDistinctValuesProgressor( IRawCursor<Hit<KEY, VALUE>, IOException> seeker, Neo4Net.Kernel.Api.StorageEngine.schema.IndexProgressor_NodeValueClient client, ICollection<RawCursor<Hit<KEY, VALUE>, IOException>> toRemoveFromOnClose, IndexLayout<KEY, VALUE> layout, IComparer<KEY> comparator ) : base( seeker, client, toRemoveFromOnClose )
		 {
			  this._layout = layout;
			  _prev = layout.newKey();
