...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 {
			  Data.get();
			  ResponseEntity responseEntity = Gen().withHeader(Neo4Net.Server.rest.repr.StreamingFormat_Fields.STREAM_HEADER, "true").expectedType(APPLICATION_JSON_TYPE).expectedStatus(200).get(DataUri);
			  ...
			  string expectedMediaType = Neo4Net.Server.rest.repr.StreamingFormat_Fields.MediaType.ToString();
