...
//ORIGINAL LINE: private <RECORD> RecordScanner<RECORD> recordScanner(String name, org.Neo4Net.helpers.collection.BoundedIterable<RECORD> store, RecordProcessor<RECORD> processor, Stage stage, QueueDistribution distribution, @SuppressWarnings("rawtypes") IterableStore... warmupStores)
		 ...
		 private StoreProcessorTask<RECORD> Create<RECORD>( string name, RecordStore<RECORD> input, QueueDistribution distribution ) where RECORD : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
		 ...
		 private StoreProcessorTask<RECORD> Create<RECORD>( string name, RecordStore<RECORD> input, StoreProcessor processor, QueueDistribution distribution ) where RECORD : Neo4Net.Kernel.Impl.Store.Records.AbstractBaseRecord
		 {
