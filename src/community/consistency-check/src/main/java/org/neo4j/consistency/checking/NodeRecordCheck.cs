...
//ORIGINAL LINE: @SafeVarargs static NodeRecordCheck forSparseNodes(RecordField<org.Neo4Net.kernel.impl.store.record.NodeRecord,org.Neo4Net.consistency.report.ConsistencyReport_NodeConsistencyReport>... additional)
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @SafeVarargs static NodeRecordCheck forDenseNodes(RecordField<org.Neo4Net.kernel.impl.store.record.NodeRecord,org.Neo4Net.consistency.report.ConsistencyReport_NodeConsistencyReport>... additional)
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @SafeVarargs NodeRecordCheck(RecordField<org.Neo4Net.kernel.impl.store.record.NodeRecord, org.Neo4Net.consistency.report.ConsistencyReport_NodeConsistencyReport>... fields)
		 ...
		 {
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: NEXT_GROUP { @Override public void checkConsistency(org.Neo4Net.kernel.impl.store.record.NodeRecord node, CheckerEngine<org.Neo4Net.kernel.impl.store.record.NodeRecord, org.Neo4Net.consistency.report.ConsistencyReport_NodeConsistencyReport> engine, org.Neo4Net.consistency.store.RecordAccess records) { if(!org.Neo4Net.kernel.impl.store.record.Record.NO_NEXT_RELATIONSHIP.is(node.getNextRel())) { engine.comparativeCheck(records.relationshipGroup(node.getNextRel()), this); } } @Override public long valueFrom(org.Neo4Net.kernel.impl.store.record.NodeRecord node) { return node.getNextRel(); } @Override public void checkReference(org.Neo4Net.kernel.impl.store.record.NodeRecord record, org.Neo4Net.kernel.impl.store.record.RelationshipGroupRecord group, CheckerEngine<org.Neo4Net.kernel.impl.store.record.NodeRecord, org.Neo4Net.consistency.report.ConsistencyReport_NodeConsistencyReport> engine, org.Neo4Net.consistency.store.RecordAccess records) { if(!group.inUse()) { engine.report().relationshipGroupNotInUse(group); } else { if(group.getOwningNode() != record.getId()) { engine.report().relationshipGroupHasOtherOwner(group); } } } }
			  NEXT_GROUP
			  {
