...
namespace Neo4Net.Kernel.Api.Exceptions
{
   ...
   ///
   /// This class is not part of the public Neo4Net API, and backwards compatibility for using it as a Java class is not
   ...
   public interface IStatus
   {
      ...
      1. For unexpected events, name with a leading noun followed by a short problem term in the past tense. For example,
         IEntityNotFound or LockSessionExpired. As a variant, names may omit the leading noun; in this case, the current
   ...
   public sealed class Status_Transaction : Status
   {
      ...
      public static readonly Status_Transaction InstanceStateChanged = new Status_Transaction("InstanceStateChanged", InnerEnum.InstanceStateChanged, TransientError, "Transactions rely on assumptions around the state of the Neo4Net instance they " + "execute on. For instance, transactions in a cluster may expect that " + "they are executing on an instance that can perform writes. However, " + "instances may change state while the transaction is running. This causes " + "assumptions the instance has made about how to execute the transaction " + "to be violated - meaning the transaction must be rolled " + "back. If you see this error, you should retry your operation in a new transaction.");
   ...
   public sealed class Status_Statement : Status
   {
      ...
      public static readonly Status_Statement IEntityNotFound = new Status_Statement("EntityNotFound", InnerEnum.EntityNotFound, ClientError, "The statement refers to a non-existent IEntity.");
   ...
   public sealed class Status_General : Status
   {
      ...
      public static readonly Status_General OutOfMemoryError = new Status_General("OutOfMemoryError", InnerEnum.OutOfMemoryError, TransientError, "There is not enough memory to perform the current task. Please try increasing " + "'dbms.memory.heap.max_size' in the Neo4Net configuration (normally in 'conf/Neo4Net.conf' or, if you " + "you are using Neo4Net Desktop, found through the user interface) or if you are running an embedded " + "installation increase the heap by using '-Xmx' command line flag, and then restart the database.");
      public static readonly Status_General StackOverFlowError = new Status_General("StackOverFlowError", InnerEnum.StackOverFlowError, TransientError, "There is not enough stack size to perform the current task. This is generally considered to be a " + "database error, so please contact Neo4Net support. You could try increasing the stack size: " + "for example to set the stack size to 2M, add `dbms.jvm.additional=-Xss2M' to " + "in the Neo4Net configuration (normally in 'conf/Neo4Net.conf' or, if you are using " + "Neo4Net Desktop, found through the user interface) or if you are running an embedded installation " + "just add -Xss2M as command line flag.");
   ...
   public class Status_Code
   {
      ...
      internal Status_Code<C> private (Status_Classification classification, C categoryAndTitle, string description) where C : Enum<C>, Status
		  {
