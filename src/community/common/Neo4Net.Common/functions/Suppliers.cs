...
namespace Neo4Net.Functions
{
   ...
   public sealed class Suppliers
   {
      ...
      {
         return new LazyAnonymousInnerClass<T>(supplier);
      }

      private class LazyAnonymousInnerClass<T> : Lazy<T>
      {
         private System.Func<T> _supplier;

         //$!!$i tac nternal volatile T _instance;
         //$!!$ tac internal volatile object _instance;
         ...
         public bool Initialized { get { return _instance != null; } }
      }
      ...
      private class FuncAnonymousInnerClass<V, T> : System.Func<T>
      {
      ...
      internal class ThrowingCapturingSupplier<T, E> : ThrowingSupplier<bool, E> where E : Exception
      {
         ...
         internal ThrowingCapturingSupplier<T1, T2> private (ThrowingSupplier<T1> input, ThrowingPredicate<T2> predicate) where T1 : private E where T2 : E
			  {
					this.Input = input;
