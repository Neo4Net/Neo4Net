...
//ORIGINAL LINE: @Test void shouldRemainSilentWhenEverythingIsInOrder()
		 ...
		 private static NodeInUseWithCorrectLabelsCheck<LabelScanDocument, Org.Neo4j.Consistency.report.ConsistencyReport_LabelScanConsistencyReport> Checker( long[] expectedLabels, bool checkStoreToIndex )
		 {
			  return new NodeInUseWithCorrectLabelsCheck<LabelScanDocument, Org.Neo4j.Consistency.report.ConsistencyReport_LabelScanConsistencyReport>( expectedLabels, COMPLETE_ALL_TOKENS, checkStoreToIndex );
		 }

		 internal interface Engine : CheckerEngine<LabelScanDocument, Org.Neo4j.Consistency.report.ConsistencyReport_LabelScanConsistencyReport>
		 {
		 }
	}
