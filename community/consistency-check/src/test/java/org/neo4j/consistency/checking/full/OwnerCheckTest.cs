...
//ORIGINAL LINE: @Test void shouldNotDecorateCheckerWhenInactive()
		 internal virtual void ShouldNotDecorateCheckerWhenInactive()
		 {
			  ...
			  PrimitiveRecordCheck<NodeRecord, Org.Neo4j.Consistency.report.ConsistencyReport_NodeConsistencyReport> checker = dummyNodeCheck();

			  // when
			  RecordCheck<NodeRecord, Org.Neo4j.Consistency.report.ConsistencyReport_NodeConsistencyReport> decorated = decorator.DecorateNodeChecker( checker );
...
//ORIGINAL LINE: @Test void shouldNotReportAnythingForNodesWithDifferentPropertyChains()
		 internal virtual void ShouldNotReportAnythingForNodesWithDifferentPropertyChains()
		 {
			  ...
			  RecordCheck<NodeRecord, Org.Neo4j.Consistency.report.ConsistencyReport_NodeConsistencyReport> nodeChecker = decorator.DecorateNodeChecker( dummyNodeCheck() );
			  ...
			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_NodeConsistencyReport report1 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_NodeConsistencyReport ), nodeChecker, node1, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_NodeConsistencyReport report2 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_NodeConsistencyReport ), nodeChecker, node2, records );
...
//ORIGINAL LINE: @Test void shouldNotReportAnythingForRelationshipsWithDifferentPropertyChains()
		 internal virtual void ShouldNotReportAnythingForRelationshipsWithDifferentPropertyChains()
		 {
			  // given
			  OwnerCheck decorator = new OwnerCheck( true );
			  RecordCheck<RelationshipRecord, Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport> relationshipChecker = decorator.DecorateRelationshipChecker( dummyRelationshipChecker() );
			  ...
			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport report1 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport ), relationshipChecker, relationship1, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport report2 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport ), relationshipChecker, relationship2, records );
...
//ORIGINAL LINE: @Test void shouldReportRelationshipWithSamePropertyChainAsNode()
		 internal virtual void ShouldReportRelationshipWithSamePropertyChainAsNode()
		 {
			  ...
			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_NodeConsistencyReport nodeReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_NodeConsistencyReport ), nodeChecker, node, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport relationshipReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport ), relationshipChecker, relationship, records );
...
//ORIGINAL LINE: @Test void shouldReportRelationshipWithReferenceToTheGraphGlobalChain()
		 internal virtual void ShouldReportRelationshipWithReferenceToTheGraphGlobalChain()
		 {
			  ...
			  OwnerCheck decorator = new OwnerCheck( true );
			  RecordCheck<RelationshipRecord, Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport> relationshipChecker = decorator.DecorateRelationshipChecker( dummyRelationshipChecker() );
			  RecordCheck<NeoStoreRecord, Org.Neo4j.Consistency.report.ConsistencyReport_NeoStoreConsistencyReport> neoStoreCheck = decorator.DecorateNeoStoreChecker( dummyNeoStoreCheck() );
			  ...
			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_NeoStoreConsistencyReport masterReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_NeoStoreConsistencyReport ), neoStoreCheck, master, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport relationshipReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport ), relationshipChecker, relationship, records );
...
//ORIGINAL LINE: @Test void shouldReportOrphanPropertyChain()
		 internal virtual void ShouldReportOrphanPropertyChain()
		 {
			  ...
			  RecordAccessStub records = new RecordAccessStub();
			  OwnerCheck decorator = new OwnerCheck( true );
			  RecordCheck<PropertyRecord, Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport> checker = decorator.DecoratePropertyChecker( dummyPropertyChecker() );

			  PropertyRecord record = inUse( new PropertyRecord( 42 ) );
			  Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport report = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport ), checker, record, records );
...
//ORIGINAL LINE: @Test void shouldNotReportOrphanIfOwnedByNode()
		 internal virtual void ShouldNotReportOrphanIfOwnedByNode()
		 {
			  ...
			  PropertyRecord record = inUse( new PropertyRecord( 42 ) );
			  Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport report = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport ), decorator.DecoratePropertyChecker( dummyPropertyChecker() ), record, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_NodeConsistencyReport nodeReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_NodeConsistencyReport ), decorator.DecorateNodeChecker( dummyNodeCheck() ), inUse(new NodeRecord(10, false, NONE, 42)), records );
...
//ORIGINAL LINE: @Test void shouldNotReportOrphanIfOwnedByRelationship()
		 internal virtual void ShouldNotReportOrphanIfOwnedByRelationship()
		 {
			  ...
			  Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport relationshipReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipConsistencyReport ), decorator.DecorateRelationshipChecker( dummyRelationshipChecker() ), relationship, records );
...
//ORIGINAL LINE: @Test void shouldNotReportOrphanIfOwnedByNeoStore()
		 internal virtual void ShouldNotReportOrphanIfOwnedByNeoStore()
		 {
			  ...
			  master.NextProp = 42;
			  Org.Neo4j.Consistency.report.ConsistencyReport_NeoStoreConsistencyReport masterReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_NeoStoreConsistencyReport ), decorator.DecorateNeoStoreChecker( dummyNeoStoreCheck() ), master, records );
...
//ORIGINAL LINE: @Test void shouldReportDynamicRecordOwnedByTwoOtherDynamicRecords()
		 internal virtual void ShouldReportDynamicRecordOwnedByTwoOtherDynamicRecords()
		 {
			  ...
			  RecordCheck<DynamicRecord, Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport> checker = decorator.DecorateDynamicChecker( RecordType.STRING_PROPERTY, dummyDynamicCheck( configureDynamicStore( 50 ), DynamicStore.STRING ) );
			  ...
			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport report1 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport ), checker, record1, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport report2 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport ), checker, record2, records );
...
//ORIGINAL LINE: @Test void shouldReportDynamicStringRecordOwnedByTwoPropertyRecords()
		 internal virtual void ShouldReportDynamicStringRecordOwnedByTwoPropertyRecords()
		 {
			  ...
			  RecordCheck<PropertyRecord, Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport> propChecker = decorator.DecoratePropertyChecker( dummyPropertyChecker() );
			  ...
			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport report1 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport ), propChecker, property1, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport report2 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport ), propChecker, property2, records );
...
//ORIGINAL LINE: @Test void shouldReportDynamicRecordOwnedByPropertyAndOtherDynamic()
		 internal virtual void ShouldReportDynamicRecordOwnedByPropertyAndOtherDynamic()
		 {
			  ...
			  RecordCheck<DynamicRecord, Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport> dynChecker = decorator.DecorateDynamicChecker( RecordType.STRING_PROPERTY, dummyDynamicCheck( configureDynamicStore( 50 ), DynamicStore.STRING ) );
			  RecordCheck<PropertyRecord, Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport> propChecker = decorator.DecoratePropertyChecker( dummyPropertyChecker() );
			  ...
			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport propReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport ), propChecker, property, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport dynReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport ), dynChecker, dynamic, records );
...
//ORIGINAL LINE: @Test void shouldReportDynamicArrayRecordOwnedByOtherDynamicAndProperty()
		 internal virtual void ShouldReportDynamicArrayRecordOwnedByOtherDynamicAndProperty()
		 {
			  ...
			  RecordCheck<DynamicRecord, Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport> dynChecker = decorator.DecorateDynamicChecker( RecordType.ARRAY_PROPERTY, dummyDynamicCheck( configureDynamicStore( 50 ), DynamicStore.ARRAY ) );
			  RecordCheck<PropertyRecord, Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport> propChecker = decorator.DecoratePropertyChecker( dummyPropertyChecker() );
...
//ORIGINAL LINE: @Test void shouldReportDynamicRecordOwnedByTwoRelationshipLabels()
		 internal virtual void ShouldReportDynamicRecordOwnedByTwoRelationshipLabels()
		 {
			  ...
			  RecordCheck<RelationshipTypeTokenRecord, Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipTypeConsistencyReport> checker = decorator.DecorateRelationshipTypeTokenChecker( dummyRelationshipLabelCheck() );
			  ...
			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipTypeConsistencyReport report1 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipTypeConsistencyReport ), checker,record1, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipTypeConsistencyReport report2 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipTypeConsistencyReport ), checker,record2, records );
...
//ORIGINAL LINE: @Test void shouldReportDynamicRecordOwnedByRelationshipLabelAndOtherDynamicRecord()
		 internal virtual void ShouldReportDynamicRecordOwnedByRelationshipLabelAndOtherDynamicRecord()
		 {
			  ...
			  RecordCheck<DynamicRecord, Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport> dynChecker = decorator.DecorateDynamicChecker( RecordType.RELATIONSHIP_TYPE_NAME, dummyDynamicCheck( configureDynamicStore( 50 ), DynamicStore.RELATIONSHIP_TYPE ) );

			  RecordCheck<RelationshipTypeTokenRecord, Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipTypeConsistencyReport> labelCheck = decorator.DecorateRelationshipTypeTokenChecker( dummyRelationshipLabelCheck() );
			  ...
			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipTypeConsistencyReport labelReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipTypeConsistencyReport ), labelCheck, label, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport dynReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport ), dynChecker, dynamic, records );
...
//ORIGINAL LINE: @Test void shouldReportDynamicRecordOwnedByTwoPropertyKeys()
		 internal virtual void ShouldReportDynamicRecordOwnedByTwoPropertyKeys()
		 {
			  ...
			  RecordCheck<PropertyKeyTokenRecord, Org.Neo4j.Consistency.report.ConsistencyReport_PropertyKeyTokenConsistencyReport> checker = decorator.DecoratePropertyKeyTokenChecker( dummyPropertyKeyCheck() );
			  ...
			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_PropertyKeyTokenConsistencyReport report1 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_PropertyKeyTokenConsistencyReport ), checker,record1, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_PropertyKeyTokenConsistencyReport report2 = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_PropertyKeyTokenConsistencyReport ), checker,record2, records );
...
//ORIGINAL LINE: @Test void shouldReportDynamicRecordOwnedByPropertyKeyAndOtherDynamicRecord()
		 internal virtual void ShouldReportDynamicRecordOwnedByPropertyKeyAndOtherDynamicRecord()
		 {
			  ...
			  RecordCheck<DynamicRecord, Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport> dynChecker = decorator.DecorateDynamicChecker( RecordType.PROPERTY_KEY_NAME, dummyDynamicCheck( configureDynamicStore( 50 ), DynamicStore.PROPERTY_KEY ) );

			  RecordCheck<PropertyKeyTokenRecord, Org.Neo4j.Consistency.report.ConsistencyReport_PropertyKeyTokenConsistencyReport> keyCheck = decorator.DecoratePropertyKeyTokenChecker( dummyPropertyKeyCheck() );
			  ...
			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_PropertyKeyTokenConsistencyReport keyReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_PropertyKeyTokenConsistencyReport ), keyCheck, key, records );
			  Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport dynReport = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport ), dynChecker, dynamic, records );
...
//ORIGINAL LINE: @Test void shouldReportOrphanedDynamicStringRecord()
		 internal virtual void ShouldReportOrphanedDynamicStringRecord()
		 {
			  ...
			  RecordCheck<DynamicRecord, Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport> stringCheck = owners.DecorateDynamicChecker( RecordType.STRING_PROPERTY, dummyDynamicCheck( configureDynamicStore( 60 ), DynamicStore.STRING ) );
			  DynamicRecord record = @string( inUse( new DynamicRecord( 42 ) ) );

			  // when
			  Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport report = check( typeof( Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport ), stringCheck, record, records );
...
//ORIGINAL LINE: @Test void shouldReportOrphanedDynamicArrayRecord()
		 internal virtual void ShouldReportOrphanedDynamicArrayRecord()
		 {
			  ...
			  RecordCheck<DynamicRecord, Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport> stringCheck = owners.DecorateDynamicChecker( RecordType.ARRAY_PROPERTY, dummyDynamicCheck( configureDynamicStore( 60 ), DynamicStore.ARRAY ) );
			  DynamicRecord record = @string( inUse( new DynamicRecord( 42 ) ) );
...
//ORIGINAL LINE: @Test void shouldReportOrphanedDynamicRelationshipLabelRecord()
		 internal virtual void ShouldReportOrphanedDynamicRelationshipLabelRecord()
		 {
			  ...
			  RecordCheck<DynamicRecord, Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport> stringCheck = owners.DecorateDynamicChecker( RecordType.RELATIONSHIP_TYPE_NAME, dummyDynamicCheck( configureDynamicStore( 60 ), DynamicStore.RELATIONSHIP_TYPE ) );
			  DynamicRecord record = @string( inUse( new DynamicRecord( 42 ) ) );
...
//ORIGINAL LINE: @Test void shouldReportOrphanedDynamicPropertyKeyRecord()
		 internal virtual void ShouldReportOrphanedDynamicPropertyKeyRecord()
		 {
			  ...
			  RecordCheck<DynamicRecord, Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport> stringCheck = owners.DecorateDynamicChecker( RecordType.PROPERTY_KEY_NAME, dummyDynamicCheck( configureDynamicStore( 60 ), DynamicStore.PROPERTY_KEY ) );
			  DynamicRecord record = @string( inUse( new DynamicRecord( 42 ) ) );
