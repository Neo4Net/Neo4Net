...
//	import static org.neo4j.consistency.store.RecordReference_SkippingReference.skipReference;
	...
	internal abstract class DynamicOwner<RECORD> : Owner where RECORD : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord
	{
		 internal static readonly ComparativeRecordChecker<DynamicRecord, AbstractBaseRecord, Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport> OrphanCheck = ( record, ignored, engine, records ) => engine.report().orphanDynamicRecord();
		 ...
		 internal class Property : DynamicOwner<PropertyRecord>, ComparativeRecordChecker<PropertyRecord, AbstractBaseRecord, Org.Neo4j.Consistency.report.ConsistencyReport_PropertyConsistencyReport>
		 {
		 ...
		 internal class Dynamic : DynamicOwner<DynamicRecord>, ComparativeRecordChecker<DynamicRecord, AbstractBaseRecord, Org.Neo4j.Consistency.report.ConsistencyReport_DynamicConsistencyReport>
		 {
			  internal readonly long Id;
			  internal readonly RecordType Type;
		 ...
		 internal abstract class NameOwner<RECORD, REPORT> : DynamicOwner<RECORD>, ComparativeRecordChecker<RECORD, AbstractBaseRecord, REPORT> where RECORD : Org.Neo4j.Kernel.impl.store.record.TokenRecord where REPORT : Org.Neo4j.Consistency.report.ConsistencyReport_NameConsistencyReport
		 {
...
//ORIGINAL LINE: @SuppressWarnings("ConstantConditions") @Override public void checkReference(RECORD name, org.neo4j.kernel.impl.store.record.AbstractBaseRecord record, org.neo4j.consistency.checking.CheckerEngine<RECORD, REPORT> engine, org.neo4j.consistency.store.RecordAccess records)
		 ...
		 internal class PropertyKey : NameOwner<PropertyKeyTokenRecord, Org.Neo4j.Consistency.report.ConsistencyReport_PropertyKeyTokenConsistencyReport>
		 {
		 ...
		 internal class LabelToken : NameOwner<LabelTokenRecord, Org.Neo4j.Consistency.report.ConsistencyReport_LabelTokenConsistencyReport>
		 {
			  internal readonly int Id;
		 ...
		 internal class RelationshipTypeToken : NameOwner<RelationshipTypeTokenRecord, Org.Neo4j.Consistency.report.ConsistencyReport_RelationshipTypeConsistencyReport>
		 {
			  internal readonly int Id;
