...
namespace Org.Neo4j.Consistency.report
{
	...
	using DocumentedUtils = Org.Neo4j.Kernel.Impl.Annotations.DocumentedUtils;
...
//	import static org.neo4j.helpers.Exceptions.stringify;
	...
	public class ConsistencyReporter : ConsistencyReport_Reporter
	{
		 ...
		 public virtual REPORT Report<RECORD, REPORT>( RECORD record, Type cls, RecordType recordType ) where RECORD : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord where REPORT : ConsistencyReport
		 {
				 cls = typeof( REPORT );
		 ...
		 public abstract class ReportInvocationHandler <RECORD, REPORT> : CheckerEngine<RECORD, REPORT>, InvocationHandler where RECORD : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord where REPORT : ConsistencyReport
		 {
...
//ORIGINAL LINE: public <REFERRED extends org.neo4j.kernel.impl.store.record.AbstractBaseRecord> void comparativeCheck(org.neo4j.consistency.store.RecordReference<REFERRED> reference, org.neo4j.consistency.checking.ComparativeRecordChecker<RECORD, ? super REFERRED, REPORT> checker)
		 ...
		 public class ReportHandler <RECORD, REPORT> : ReportInvocationHandler<RECORD, REPORT> where RECORD : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord where REPORT : ConsistencyReport
		 {
