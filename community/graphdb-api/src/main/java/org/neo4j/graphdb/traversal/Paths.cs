...
namespace Org.Neo4j.Graphdb.traversal
{
	...
	public class Paths
	{
		 ...
		 public interface PathDescriptor<T> where T : Org.Neo4j.Graphdb.Path
		 {
			  /// <summary>
		 ...
		 public class DefaultPathDescriptor<T> : PathDescriptor<T> where T : Org.Neo4j.Graphdb.Path
		 {
		 ...
		 public static string PathToString<T>( T path, PathDescriptor<T> builder ) where T : Org.Neo4j.Graphdb.Path
		 {
...
//ORIGINAL LINE: public static <T extends org.neo4j.graphdb.Path> PathDescriptor<T> descriptorForIdAndProperties(final boolean nodeId, final boolean relId, final String... propertyKeys)
		 public static PathDescriptor<T> DescriptorForIdAndProperties<T>( bool nodeId, bool relId, params string[] propertyKeys ) where T : Org.Neo4j.Graphdb.Path
		 {
