...
namespace Org.Neo4j.Graphdb
{
	...
	public interface Transaction : AutoCloseable
	{
		 ...
		 /// <param name="entity"> the entity to acquire a lock for. If another transaction
		 /// currently holds a write lock to that entity this call will wait until
		 /// it's released.
		 /// </param>
		 /// <returns> a <seealso cref="Lock"/> which optionally can be used to release this
		 /// lock earlier than when the transaction finishes. If not released
		 /// (with <seealso cref="Lock.release()"/> it's going to be released with the
