...
//ORIGINAL LINE: final org.neo4j.test.Barrier_Control barrier = new org.neo4j.test.Barrier_Control();
		 ...
		 private class NamedFunctionAnonymousInnerClass : NamedFunction<GraphDatabaseService, long>
		 {
			 ...
			 public NamedFunctionAnonymousInnerClass( RelationshipCountsTest outerInstance, GraphDatabaseService graphDb, Org.Neo4j.Test.Barrier_Control barrier ) : base( "create-relationships" )
			 {
...
//ORIGINAL LINE: final org.neo4j.test.Barrier_Control barrier = new org.neo4j.test.Barrier_Control();
		 ...
		 private class NamedFunctionAnonymousInnerClass2 : NamedFunction<GraphDatabaseService, long>
		 {
			 ...
			 public NamedFunctionAnonymousInnerClass2( RelationshipCountsTest outerInstance, GraphDatabaseService graphDb, Relationship rel, Transaction tx, Org.Neo4j.Test.Barrier_Control barrier ) : base( "create-relationships" )
			 {
				 this.outerInstance = outerInstance;
				 this._graphDb = graphDb;
				 this._rel = rel;
...
//ORIGINAL LINE: @Test public void shouldUpdateRelationshipWithLabelCountsWhenRemovingLabelAndDeletingRelationship()
		 ...
		 private long CountsForRelationship( Label start, RelationshipType type, Label end )
		 {
			  ...
			  using ( Statement ignore = ktx.AcquireStatement() )
			  {
					...
					if ( start == null )
					{
						 startId = Org.Neo4j.@internal.Kernel.Api.Read_Fields.ANY_LABEL;
					}
					else
					{
						 if ( Org.Neo4j.@internal.Kernel.Api.TokenRead_Fields.NO_TOKEN == ( startId = tokenRead.NodeLabel( start.Name() ) ) )
						 {
							  return 0;
						 }
					}
					...
					if ( type == null )
					{
						 typeId = Org.Neo4j.@internal.Kernel.Api.Read_Fields.ANY_RELATIONSHIP_TYPE;
					}
					else
					{
						 if ( Org.Neo4j.@internal.Kernel.Api.TokenRead_Fields.NO_TOKEN == ( typeId = tokenRead.RelationshipType( type.Name() ) ) )
						 {
							  return 0;
						 }
					}
					...
					if ( end == null )
					{
						 endId = Org.Neo4j.@internal.Kernel.Api.Read_Fields.ANY_LABEL;
					}
					else
					{
						 if ( Org.Neo4j.@internal.Kernel.Api.TokenRead_Fields.NO_TOKEN == ( endId = tokenRead.NodeLabel( end.Name() ) ) )
						 {
							  return 0;
						 }
					}
