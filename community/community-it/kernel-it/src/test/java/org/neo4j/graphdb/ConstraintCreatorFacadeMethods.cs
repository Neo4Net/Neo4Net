...
//       UNIQUE(new FacadeMethod<>("ConstraintCreator assertPropertyIsUnique()", self -> self.assertPropertyIsUnique("property"))),
		 public static readonly ConstraintCreatorFacadeMethods Create = new ConstraintCreatorFacadeMethods( "Create", InnerEnum.Create, new FacadeMethod<>( "ConstraintDefinition create()", Org.Neo4j.Graphdb.schema.ConstraintCreator::create ) );
		 ...
		 internal ConstraintCreatorFacadeMethods( string name, InnerEnum innerEnum, FacadeMethod<Org.Neo4j.Graphdb.schema.ConstraintCreator> facadeMethod )
		 {
		 ...
		 public void Accept( Org.Neo4j.Graphdb.schema.ConstraintCreator constraintCreator )
		 {
