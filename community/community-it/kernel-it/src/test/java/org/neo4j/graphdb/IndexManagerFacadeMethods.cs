...
//       FOR_NODES_WITH_CONFIG(new FacadeMethod<>("Index<Node> forNodes( String indexName, Map<String, String> customConfiguration )", self -> self.forNodes("foo", null))),
		 public static readonly IndexManagerFacadeMethods NodeIndexNames = new IndexManagerFacadeMethods( "NodeIndexNames", InnerEnum.NodeIndexNames, new FacadeMethod<>( "String[] nodeIndexNames()", Org.Neo4j.Graphdb.index.IndexManager::nodeIndexNames ) );
//JAVA TO C# CONVERTER TODO TASK: Enum value-specific class bodies are not converted by Java to C# Converter:
...
//       FOR_RELATIONSHIPS_WITH_CONFIG(new FacadeMethod<>("RelationshipIndex forRelationships( String indexName, Map<String, String> customConfiguration )", self -> self.forRelationships("foo", null))),
		 public static readonly IndexManagerFacadeMethods RelationshipIndexNames = new IndexManagerFacadeMethods( "RelationshipIndexNames", InnerEnum.RelationshipIndexNames, new FacadeMethod<>( "String[] relationshipIndexNames()", Org.Neo4j.Graphdb.index.IndexManager::relationshipIndexNames ) );
		 ...
		 internal IndexManagerFacadeMethods( string name, InnerEnum innerEnum, FacadeMethod<Org.Neo4j.Graphdb.index.IndexManager> facadeMethod )
		 {
		 ...
		 public void Accept( Org.Neo4j.Graphdb.index.IndexManager indexManager )
		 {
