...
//ORIGINAL LINE: final java.util.concurrent.atomic.AtomicReference<java.util.Map<String,Object>> metaDataRef = new java.util.concurrent.atomic.AtomicReference<>();
		 ...
		 private class LoginContextAnonymousInnerClass : LoginContext
		 {
			 ...
			 {
				  return new SecurityContext( _subject, Org.Neo4j.@internal.Kernel.Api.security.AccessMode_Static.Write );
			 }
		 }
...
//ORIGINAL LINE: java.util.concurrent.Future<?> relationshipCreator = executor.submit(() ->
		 ...
		 private Org.Neo4j.Graphdb.@event.TransactionEventHandler_Adapter<object> GetBeforeCommitHandler( System.Action<TransactionData> dataConsumer )
		 {
			  return new TransactionEventHandler_AdapterAnonymousInnerClass( this, dataConsumer );
		 }

		 private class TransactionEventHandler_AdapterAnonymousInnerClass : Org.Neo4j.Graphdb.@event.TransactionEventHandler_Adapter<object>
		 {
			 private readonly TransactionEventsIT _outerInstance;
...
//JAVA TO C# CONVERTER NOTE: Fields cannot have the same name as methods:
		 ...
		 private class TransactionIdCommitTimeTracker : Org.Neo4j.Graphdb.@event.TransactionEventHandler_Adapter<object>
		 {
