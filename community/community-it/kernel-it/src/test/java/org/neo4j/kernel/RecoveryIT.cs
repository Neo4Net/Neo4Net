...
namespace Org.Neo4j.Kernel
{
	...
	using DatabaseCreator = Org.Neo4j.Graphdb.factory.GraphDatabaseBuilder.DatabaseCreator;
	...
	using ExistingTargetStrategy = Org.Neo4j.Kernel.impl.storemigration.ExistingTargetStrategy;
	using FileOperation = Org.Neo4j.Kernel.impl.storemigration.FileOperation;
...
//ORIGINAL LINE: public org.neo4j.graphdb.GraphDatabaseService newDatabase(@Nonnull Config config)
		 ...
		 private static void AssertSameStoreContents( EphemeralFileSystemAbstraction fs1, EphemeralFileSystemAbstraction fs2, DatabaseLayout databaseLayout )
		 {
			  ...
			  try (ThreadPoolJobScheduler jobScheduler = new ThreadPoolJobScheduler(); PageCache pageCache1 = new ConfiguringPageCacheFactory(fs1, defaults(), PageCacheTracer.NULL, Org.Neo4j.Io.pagecache.tracing.cursor.PageCursorTracerSupplier_Fields.Null, NullLog.Instance, contextSupplier, jobScheduler)
									.OrCreatePageCache;
						 PageCache pageCache2 = ( new ConfiguringPageCacheFactory( fs2, defaults(), PageCacheTracer.NULL, Org.Neo4j.Io.pagecache.tracing.cursor.PageCursorTracerSupplier_Fields.Null, NullLog.Instance, contextSupplier, jobScheduler ) ).OrCreatePageCache;
		 ...
		 private static void AssertSameStoreContents<RECORD>( RecordStore<RECORD> store1, RecordStore<RECORD> store2 ) where RECORD : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord
		 {
		 ...
		 {
			  ( ( GraphDatabaseAPI )db ).DependencyResolver.resolveDependency( typeof( StorageEngine ) ).flushAndForce( Org.Neo4j.Io.pagecache.IOLimiter_Fields.Unlimited );
		 }
