...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 {
			 public Configuration_OverriddenAnonymousInnerClass( Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.Configuration actual ) : base( actual )
			 {
			 }
		 ...
		 private IList<InputEntity> RandomNodeData()
		 {
			  ...
			  for ( int i = 0; i < 300; i++ )
			  {
					...
					node.Id( System.Guid.randomUUID().ToString(), Org.Neo4j.@unsafe.Impl.Batchimport.input.Group_Fields.Global );
...
//ORIGINAL LINE: private void println(java.io.Writer writer, String string) throws java.io.IOException
		 ...
		 private IList<InputEntity> RandomRelationshipData( IList<InputEntity> nodeData )
		 {
			  ...
			  for ( int i = 0; i < 1000; i++ )
			  {
					...
					relationship.StartId( nodeData[Random.Next( nodeData.Count )].id(), Org.Neo4j.@unsafe.Impl.Batchimport.input.Group_Fields.Global );
					relationship.EndId( nodeData[Random.Next( nodeData.Count )].id(), Org.Neo4j.@unsafe.Impl.Batchimport.input.Group_Fields.Global );
