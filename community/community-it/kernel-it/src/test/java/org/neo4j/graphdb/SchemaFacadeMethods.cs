...
//       GET_INDEXES_BY_LABEL(new FacadeMethod<>("Iterable<IndexDefinition> getIndexes( Label label )", s -> s.getIndexes(LABEL))),
		 public static readonly SchemaFacadeMethods GetIndexes = new SchemaFacadeMethods( "GetIndexes", InnerEnum.GetIndexes, new FacadeMethod<>( "Iterable<IndexDefinition> getIndexes()", Org.Neo4j.Graphdb.schema.Schema::getIndexes ) );
//JAVA TO C# CONVERTER TODO TASK: Enum value-specific class bodies are not converted by Java to C# Converter:
...
//       GET_CONSTRAINTS_BY_LABEL(new FacadeMethod<>("Iterable<ConstraintDefinition> getConstraints( Label label )", s -> s.getConstraints(LABEL))),
		 public static readonly SchemaFacadeMethods GetConstraints = new SchemaFacadeMethods( "GetConstraints", InnerEnum.GetConstraints, new FacadeMethod<>( "Iterable<ConstraintDefinition> getConstraints()", Org.Neo4j.Graphdb.schema.Schema::getConstraints ) );
//JAVA TO C# CONVERTER TODO TASK: Enum value-specific class bodies are not converted by Java to C# Converter:
...
//       AWAIT_INDEXES_ONLINE(new FacadeMethod<>("void awaitIndexesOnline( long duration, TimeUnit unit )", s -> s.awaitIndexesOnline(1L, java.util.concurrent.TimeUnit.SECONDS)));
		 ...
		 private readonly FacadeMethod<Org.Neo4j.Graphdb.schema.Schema> facadeMethod;

		 internal SchemaFacadeMethods( string name, InnerEnum innerEnum, FacadeMethod<Org.Neo4j.Graphdb.schema.Schema> facadeMethod )
		 {
		 ...
		 public void Accept( Org.Neo4j.Graphdb.schema.Schema schema )
		 {
