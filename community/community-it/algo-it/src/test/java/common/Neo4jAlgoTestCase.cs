...
//ORIGINAL LINE: try (org.neo4j.graphdb.ResourceIterator<? extends org.neo4j.graphdb.Path> iterator = org.neo4j.helpers.collection.Iterators.asResourceIterator(paths.iterator()))
		 ...
		 public virtual void AssertPaths<T1>( IEnumerable<T1> paths, params string[] pathDefinitions ) where T1 : Org.Neo4j.Graphdb.Path
		 {
			  AssertPaths( paths, new List<string>( Arrays.asList( pathDefinitions ) ) );
		 }

		 public virtual void AssertPathsWithPaths<T1>( IEnumerable<T1> actualPaths, params Path[] expectedPaths ) where T1 : Org.Neo4j.Graphdb.Path
		 {
