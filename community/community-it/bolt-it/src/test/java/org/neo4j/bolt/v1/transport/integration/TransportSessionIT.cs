...
namespace Org.Neo4j.Bolt.v1.transport.integration
{
	...
	using SeverityLevel = Org.Neo4j.Graphdb.SeverityLevel;
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldBeAbleToRunQueryAfterAckFailure()
		 {
			  ...
			  assertThat( Connection, Util.eventuallyReceivesSelectedProtocolVersion() );
			  assertThat( Connection, Util.eventuallyReceives( msgSuccess(), msgFailure(Org.Neo4j.Kernel.Api.Exceptions.Status_Statement.SyntaxError, string.Format("Invalid input 'Q': expected <init> (line 1, column 1 (offset: 0))%n" + "\"QINVALID\"%n" + " ^")), msgIgnored() ) );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldFailNicelyOnNullKeysInMap()
		 {
			  ...
			  // Then
			  assertThat( Connection, Util.eventuallyReceivesSelectedProtocolVersion() );
			  assertThat( Connection, Util.eventuallyReceives( msgSuccess(), msgFailure(Org.Neo4j.Kernel.Api.Exceptions.Status_Request.Invalid, "Value `null` is not supported as key in maps, must be a non-nullable string."), msgIgnored() ) );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldFailNicelyWhenDroppingUnknownIndex()
		 {
			  ...
			  // Then
			  assertThat( Connection, Util.eventuallyReceivesSelectedProtocolVersion() );
			  assertThat( Connection, Util.eventuallyReceives( msgSuccess(), msgFailure(Org.Neo4j.Kernel.Api.Exceptions.Status_Schema.IndexDropFailed, "Unable to drop index on :Movie12345(id): No such INDEX ON :Movie12345(id)."), msgIgnored() ) );
		 }
	}
