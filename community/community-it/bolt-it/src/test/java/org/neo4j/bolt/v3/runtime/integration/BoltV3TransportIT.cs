...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldBeAbleToRunQueryAfterReset()
		 {
			  ...
			  assertThat( Connection, Util.eventuallyReceives( msgFailure( Org.Neo4j.Kernel.Api.Exceptions.Status_Statement.SyntaxError, string.Format( "Invalid input 'Q': expected <init> (line 1, column 1 (offset: 0))%n" + "\"QINVALID\"%n" + " ^" ) ), msgIgnored() ) );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldFailNicelyOnNullKeysInMap()
		 {
			  ...
			  // Then
			  assertThat( Connection, Util.eventuallyReceives( msgFailure( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.Invalid, "Value `null` is not supported as key in maps, must be a non-nullable string." ), msgIgnored() ) );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldFailNicelyWhenDroppingUnknownIndex()
		 {
			  ...
			  // Then
			  assertThat( Connection, Util.eventuallyReceives( msgFailure( Org.Neo4j.Kernel.Api.Exceptions.Status_Schema.IndexDropFailed, "Unable to drop index on :Movie12345(id): No such INDEX ON :Movie12345(id)." ), msgIgnored() ) );
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldSendFailureMessageForBeginWithInvalidBookmark()
		 {
			  ...
			  assertThat( Connection, Util.eventuallyReceives( msgFailure( Org.Neo4j.Kernel.Api.Exceptions.Status_Transaction.InvalidBookmark, bookmarkString ) ) );
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldSendFailureMessageForBeginWithInvalidTransactionTimeout()
		 {
			  ...
			  assertThat( Connection, Util.eventuallyReceives( msgFailure( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.Invalid, txTimeout ) ) );
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldSendFailureMessageForBeginWithInvalidTransactionMetadata()
		 {
			  ...
			  assertThat( Connection, Util.eventuallyReceives( msgFailure( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.Invalid, txMetadata ) ) );
		 }
