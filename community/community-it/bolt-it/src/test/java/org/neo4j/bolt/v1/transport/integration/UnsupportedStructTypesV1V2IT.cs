...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldFailWhenNullKeyIsSentWithInit()
		 {
			  ...
			  assertThat( Connection, Util.eventuallyReceives( msgFailure( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.Invalid, "Value `null` is not supported as key in maps, must be a non-nullable string." ) ) );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldFailWhenDuplicateKeyIsSentWithInit()
		 {
			  ...
			  assertThat( Connection, Util.eventuallyReceives( msgFailure( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.Invalid, "Duplicate map key `key1`." ) ) );
			  assertThat( Connection, eventuallyDisconnects() );
		 }
