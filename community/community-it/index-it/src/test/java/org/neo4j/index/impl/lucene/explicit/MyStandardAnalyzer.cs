...
namespace Org.Neo4j.Index.impl.lucene.@explicit
{
	...
	public sealed class MyStandardAnalyzer : AnalyzerWrapper
	{
		 private readonly Analyzer _actual;

		 public MyStandardAnalyzer() : base(GLOBAL_REUSE_STRATEGY)
		 {
			  CharArraySet stopWords = CharArraySet.copy( new HashSet<>( Arrays.asList( "just", "some", "words" ) ) );
			  _actual = new StandardAnalyzer( stopWords );
		 }
