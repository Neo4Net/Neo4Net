...
//ORIGINAL LINE: @SuppressWarnings("unchecked") private <T extends org.neo4j.graphdb.PropertyContainer> void makeSureAdditionsCanBeRead(org.neo4j.graphdb.index.Index<T> index, EntityCreator<T> entityCreator)
		 private void MakeSureAdditionsCanBeRead<T>( Index<T> index, EntityCreator<T> entityCreator ) where T : Org.Neo4j.Graphdb.PropertyContainer
		 {
			  string key = "name";
...
//ORIGINAL LINE: @SuppressWarnings("unchecked") private <T extends org.neo4j.graphdb.PropertyContainer> void doSomeRandomUseCaseTestingWithExactIndex(org.neo4j.graphdb.index.Index<T> index, EntityCreator<T> creator)
		 private void DoSomeRandomUseCaseTestingWithExactIndex<T>( Index<T> index, EntityCreator<T> creator ) where T : Org.Neo4j.Graphdb.PropertyContainer
		 {
			  string name = "name";
...
//ORIGINAL LINE: @SuppressWarnings("unchecked") private <T extends org.neo4j.graphdb.PropertyContainer> void doSomeRandomTestingWithFulltextIndex(org.neo4j.graphdb.index.Index<T> index, EntityCreator<T> creator)
		 private void DoSomeRandomTestingWithFulltextIndex<T>( Index<T> index, EntityCreator<T> creator ) where T : Org.Neo4j.Graphdb.PropertyContainer
		 {
			  T entity1 = creator.Create();
			  T entity2 = creator.Create();
...
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
			  Index<Node> index = NodeIndex( MapUtil.stringMap( Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER, "lucene", "analyzer", typeof( CustomAnalyzer ).FullName, "to_lower_case", "true" ) );
			  Node node = GraphDb.createNode();
			  string key = "name";
...
//ORIGINAL LINE: @Test public void makeSureCustomAnalyzerCanBeUsed2()
		 public virtual void MakeSureCustomAnalyzerCanBeUsed2()
		 {
			  CustomAnalyzer.Called = false;
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
			  Index<Node> index = NodeIndex( "w-custom-analyzer-2", MapUtil.stringMap( Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER, "lucene", "analyzer", typeof( CustomAnalyzer ).FullName, "to_lower_case", "true", "type", "fulltext" ) );
			  Node node = GraphDb.createNode();
			  string key = "name";
			  string value = "The value";
			  index.Add( node, key, value );
...
//ORIGINAL LINE: @SuppressWarnings("unchecked") private <T extends org.neo4j.graphdb.PropertyContainer> void testAbandonedIds(EntityCreator<T> creator, org.neo4j.graphdb.index.Index<T> index)
		 private void TestAbandonedIds<T>( EntityCreator<T> creator, Index<T> index ) where T : Org.Neo4j.Graphdb.PropertyContainer
		 {
...
//ORIGINAL LINE: @Test public void makeSureYouCanGetEntityTypeFromIndex()
		 ...
		 {
			  Index<Node> nodeIndex = nodeIndex( MapUtil.stringMap( Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER, "lucene", "type", "exact" ) );
			  Index<Relationship> relIndex = RelationshipIndex( MapUtil.stringMap( Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER, "lucene", "type", "exact" ) );
...
//ORIGINAL LINE: @Test public void makeSureConfigurationCanBeModified()
		 public virtual void MakeSureConfigurationCanBeModified()
		 {
			  ...
			  try
			  {
					GraphDb.index().setConfiguration(index, Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER, "something");
			  ...
			  try
			  {
					GraphDb.index().removeConfiguration(index, Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER);
					fail( "Shouldn't be able to modify provider" );
			  }
			  catch ( System.ArgumentException )
			  {
...
//ORIGINAL LINE: @Test public void makeSureSlightDifferencesInIndexConfigCanBeSupplied()
		 ...
		 {
			  IDictionary<string, string> config = MapUtil.stringMap( Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER, "lucene", "type", "fulltext" );
			  string name = CurrentIndexName();
...
//ORIGINAL LINE: @Test public void testSimilarity()
		 ...
		 {
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
			  Index<Node> index = NodeIndex( MapUtil.stringMap( Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER, "lucene", "type", "fulltext", "similarity", typeof( DefaultSimilarity ).FullName ) );
			  Node node = GraphDb.createNode();
			  index.Add( node, "key", "value" );
			  RestartTx();
...
//ORIGINAL LINE: @SuppressWarnings("unchecked") private <T extends org.neo4j.graphdb.PropertyContainer> void testRemoveWithoutKey(EntityCreator<T> creator, org.neo4j.graphdb.index.Index<T> index)
		 private void TestRemoveWithoutKey<T>( EntityCreator<T> creator, Index<T> index ) where T : Org.Neo4j.Graphdb.PropertyContainer
		 {
			  string key1 = "key1";
			  string key2 = "key2";
			  string value = "value";
...
//ORIGINAL LINE: @SuppressWarnings("unchecked") private <T extends org.neo4j.graphdb.PropertyContainer> void testRemoveWithoutKeyValue(EntityCreator<T> creator, org.neo4j.graphdb.index.Index<T> index)
		 private void TestRemoveWithoutKeyValue<T>( EntityCreator<T> creator, Index<T> index ) where T : Org.Neo4j.Graphdb.PropertyContainer
		 {
			  string key1 = "key1";
...
//ORIGINAL LINE: @SuppressWarnings("unchecked") private <T extends org.neo4j.graphdb.PropertyContainer> void testRemoveWithoutKeyFulltext(EntityCreator<T> creator, org.neo4j.graphdb.index.Index<T> index)
		 private void TestRemoveWithoutKeyFulltext<T>( EntityCreator<T> creator, Index<T> index ) where T : Org.Neo4j.Graphdb.PropertyContainer
		 {
			  string key1 = "key1";
			  string key2 = "key2";
...
//ORIGINAL LINE: @SuppressWarnings("unchecked") private <T extends org.neo4j.graphdb.PropertyContainer> void testRemoveWithoutKeyValueFulltext(EntityCreator<T> creator, org.neo4j.graphdb.index.Index<T> index)
		 private void TestRemoveWithoutKeyValueFulltext<T>( EntityCreator<T> creator, Index<T> index ) where T : Org.Neo4j.Graphdb.PropertyContainer
		 {
			  string value = "value";
			  string key1 = "key1";
			  string value1 = value + " one";
