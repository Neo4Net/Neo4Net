...
namespace Org.Neo4j.Consistency.checking.full
{
	...
	using Applier = Org.Neo4j.Consistency.checking.GraphStoreFixture.Applier;
	using IdGenerator = Org.Neo4j.Consistency.checking.GraphStoreFixture.IdGenerator;
	using TransactionDataBuilder = Org.Neo4j.Consistency.checking.GraphStoreFixture.TransactionDataBuilder;
	...
	using DynamicRecordAllocator = Org.Neo4j.Kernel.impl.store.DynamicRecordAllocator;
...
//ORIGINAL LINE: final Reference<int> inconsistentName = new Reference<>();
		 ...
		 private class TransactionAnonymousInnerClass21 : GraphStoreFixture.Transaction
		 {
			 ...
			 private Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.Reference<int> _inconsistentName;

			 public TransactionAnonymousInnerClass21( FullCheckIntegrationTest outerInstance, Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.Reference<int> inconsistentName )
			 {
				 this.outerInstance = outerInstance;
...
//ORIGINAL LINE: final Reference<int> inconsistentName = new Reference<>();
		 ...
		 private class TransactionAnonymousInnerClass22 : GraphStoreFixture.Transaction
		 {
			 ...
			 public TransactionAnonymousInnerClass22( FullCheckIntegrationTest outerInstance, Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.Reference<int> inconsistentName )
			 {
				 this.outerInstance = outerInstance;
				 this._inconsistentName = inconsistentName;
			 }
...
//ORIGINAL LINE: final Reference<int> inconsistentKey = new Reference<>();
		 ...
		 private class TransactionAnonymousInnerClass23 : GraphStoreFixture.Transaction
		 {
			 ...
			 private Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.Reference<int> _inconsistentKey;

			 public TransactionAnonymousInnerClass23( FullCheckIntegrationTest outerInstance, Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.Reference<int> inconsistentKey )
			 {
				 this.outerInstance = outerInstance;
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 private class TransactionAnonymousInnerClass25 : GraphStoreFixture.Transaction
		 {
			 ...
			 protected internal override void transactionData( GraphStoreFixture.TransactionDataBuilder tx, GraphStoreFixture.IdGenerator next )
			 {
				  ...
				  long group = next.RelationshipGroup();
				  tx.Create( inUse( new NodeRecord( node, true, group, NO_NEXT_PROPERTY.intValue() ) ) );
				  tx.Create( WithOwner( Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.WithNext( inUse( new RelationshipGroupRecord( group, _outerInstance.C ) ), group + 1 ), node ) );
			 }
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 private class TransactionAnonymousInnerClass26 : GraphStoreFixture.Transaction
		 {
			 ...
			 protected internal override void transactionData( GraphStoreFixture.TransactionDataBuilder tx, GraphStoreFixture.IdGenerator next )
			 {
				  ...
				  tx.Create( WithOwner( Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.WithNext( inUse( new RelationshipGroupRecord( firstGroupId, _outerInstance.T ) ), otherGroupId ), node ) );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 private class TransactionAnonymousInnerClass28 : GraphStoreFixture.Transaction
		 {
			 ...
			 protected internal override void transactionData( GraphStoreFixture.TransactionDataBuilder tx, GraphStoreFixture.IdGenerator next )
			 {
				  ...
				  tx.Create( Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.WithNext( inUse( new RelationshipRecord( relA, otherNode, otherNode, _outerInstance.C ) ), relB ) );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 private class TransactionAnonymousInnerClass30 : GraphStoreFixture.Transaction
		 {
			 ...
			 protected internal override void transactionData( GraphStoreFixture.TransactionDataBuilder tx, GraphStoreFixture.IdGenerator next )
			 {
				  ...
				  tx.Create( inUse( new NodeRecord( otherNode, false, NO_NEXT_RELATIONSHIP.intValue(), NO_NEXT_PROPERTY.intValue() ) ) );
				  tx.Create( Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.WithNext( WithOwner( inUse( new RelationshipGroupRecord( groupA, _outerInstance.C ) ), node ), groupB ) );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 private class TransactionAnonymousInnerClass34 : GraphStoreFixture.Transaction
		 {
			 ...
			 protected internal override void transactionData( GraphStoreFixture.TransactionDataBuilder tx, GraphStoreFixture.IdGenerator next )
			 {
				  ...
				  tx.Create( WithOwner( WithRelationship( Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.WithNext( new RelationshipGroupRecord( groupA, _outerInstance.C ), groupB ), Direction.OUTGOING, rel ), nodeA ) );
...
//ORIGINAL LINE: private void shouldReportCircularPropertyRecordChain(org.neo4j.consistency.RecordType expectedInconsistentRecordType, EntityCreator entityCreator) throws Exception
		 ...
		 private class TransactionAnonymousInnerClass41 : GraphStoreFixture.Transaction
		 {
			 ...
			 private Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.EntityCreator _entityCreator;

			 public TransactionAnonymousInnerClass41( FullCheckIntegrationTest outerInstance, Org.Neo4j.Consistency.checking.full.FullCheckIntegrationTest.EntityCreator entityCreator )
			 {
				 this.outerInstance = outerInstance;
