...
namespace Org.Neo4j.Bolt.v1.runtime
{
	...
	using BoltConnectionAuthFatality = Org.Neo4j.Bolt.runtime.BoltConnectionAuthFatality;
...
//ORIGINAL LINE: @Test public void shouldSetPendingErrorOnMarkFailedIfNoHandler()
		 public virtual void ShouldSetPendingErrorOnMarkFailedIfNoHandler()
		 {
			  ...
			  BoltChannel boltChannel = mock( typeof( BoltChannel ) );
			  BoltStateMachine machine = new BoltStateMachineV1( spi, boltChannel, Clock.systemUTC() );
			  Neo4jError error = Neo4jError.from( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.NoThreadsAvailable, "no threads" );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 {
			  TestMarkFailedOnNextMessage( ( machine, handler ) => machine.handleExternalFailure( Neo4jError.from( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.Invalid, "invalid" ), handler ) );
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldSetPendingIgnoreOnMarkFailedIfAlreadyFailedAndNoHandler()
		 {
			  ...
			  Neo4jError error2 = Neo4jError.from( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.NoThreadsAvailable, "no threads" );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 {
			  TestMarkFailedShouldYieldIgnoredIfAlreadyFailed( ( machine, handler ) => machine.handleExternalFailure( Neo4jError.from( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.Invalid, "invalid" ), handler ) );
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldSucceedOnResetOnFailedState()
		 {
			  ...
			  machine.MarkFailed( Neo4jError.from( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.NoThreadsAvailable, "No Threads Available" ) );
			  machine.Process( PullAllMessage.INSTANCE, recorder );
			  ...
			  assertThat( recorder.NextResponse(), failedWithStatus(Org.Neo4j.Kernel.Api.Exceptions.Status_Request.NoThreadsAvailable) );
			  // ...successfully
			  assertThat( recorder.NextResponse(), succeeded() );
		 }
