...
namespace Org.Neo4j.Bolt.v1.messaging
{
	...
	using PackInput = Org.Neo4j.Bolt.v1.packstream.PackInput;
	using PackOutput = Org.Neo4j.Bolt.v1.packstream.PackOutput;
	using PackStream = Org.Neo4j.Bolt.v1.packstream.PackStream;
	using PackType = Org.Neo4j.Bolt.v1.packstream.PackType;
	using PrimitiveLongIntKeyValueArray = Org.Neo4j.Collection.primitive.PrimitiveLongIntKeyValueArray;
...
//	import static org.neo4j.values.storable.Values.byteArray;
	...
	/// </summary>
	public class Neo4jPackV1 : Neo4jPack
	{
		 ...
		 public override Org.Neo4j.Bolt.messaging.Neo4jPack_Unpacker NewUnpacker( PackInput input )
		 {
		 ...
		 protected internal class PackerV1 : PackStream.Packer, AnyValueWriter<IOException>, Org.Neo4j.Bolt.messaging.Neo4jPack_Packer
		 {
...
//ORIGINAL LINE: public void pack(org.neo4j.values.AnyValue value) throws java.io.IOException
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: public void writePoint(org.neo4j.values.storable.CoordinateReferenceSystem crs, double[] coordinate) throws java.io.IOException
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: void throwUnsupportedTypeError(String type) throws org.neo4j.bolt.messaging.BoltIOException
			  internal virtual void ThrowUnsupportedTypeError( string type )
			  {
					throw new BoltIOException( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.Invalid, type + " is not supported as a return type in Bolt protocol version 1. " + "Please make sure driver supports at least protocol version 2. " + "Driver upgrade is most likely required." );
			  }
		 }

		 protected internal class UnpackerV1 : PackStream.Unpacker, Org.Neo4j.Bolt.messaging.Neo4jPack_Unpacker
		 {
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: public org.neo4j.values.AnyValue unpack() throws java.io.IOException
			  public override AnyValue Unpack()
			  {
					...
					default:
						 throw new BoltIOException( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, "Unknown value type: " + valType );
					}
			  }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: protected org.neo4j.values.AnyValue unpackStruct(char signature, long size) throws java.io.IOException
			  protected internal virtual AnyValue UnpackStruct( char signature, long size )
			  {
					...
					if ( structType == null )
					{
						 throw new BoltIOException( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, string.Format( "Struct types of 0x{0} are not recognized.", signature.ToString( "x" ) ) );
					}

					throw new BoltIOException( Org.Neo4j.Kernel.Api.Exceptions.Status_Statement.TypeError, string.Format( "{0} values cannot be unpacked with this version of bolt.", structType.description() ) );
			  }

//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: public org.neo4j.values.virtual.MapValue unpackMap() throws java.io.IOException
					...
					if ( size == UNKNOWN_SIZE )
					{
						 ...
						 while ( more )
						 {
							  ...
							  case PackType.STRING:
									...
									{
										 throw new BoltIOException( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.Invalid, "Duplicate map key `" + key + "`." );
									}
									break;
							  case PackType.NULL:
									throw new BoltIOException( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.Invalid, "Value `null` is not supported as key in maps, must be a non-nullable string." );
							  default:
									throw new BoltIOException( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, "Bad key type: " + keyType );
							  }
						 }
					}
					else
