...
namespace Org.Neo4j.Bolt.runtime
{
	...
	public class Neo4jError
	{
		 ...
		 public static Status CodeFromString( string codeStr )
		 {
			  ...
			  {
			  case "Schema":
					return Org.Neo4j.Kernel.Api.Exceptions.Status_Schema.valueOf( error );
			  case "LegacyIndex":
					return Org.Neo4j.Kernel.Api.Exceptions.Status_LegacyIndex.valueOf( error );
			  case "General":
					return Org.Neo4j.Kernel.Api.Exceptions.Status_General.valueOf( error );
			  case "Statement":
					return Org.Neo4j.Kernel.Api.Exceptions.Status_Statement.valueOf( error );
			  case "Transaction":
					return Org.Neo4j.Kernel.Api.Exceptions.Status_Transaction.valueOf( error );
			  case "Request":
					return Org.Neo4j.Kernel.Api.Exceptions.Status_Request.valueOf( error );
			  case "Network":
					return Org.Neo4j.Kernel.Api.Exceptions.Status_Network.valueOf( error );
			  case "Security":
					return Org.Neo4j.Kernel.Api.Exceptions.Status_Security.valueOf( error );
			  default:
					return Org.Neo4j.Kernel.Api.Exceptions.Status_General.UnknownError;
			  }
		 }
		 ...
		 private static Neo4jError FromThrowable( Exception any, bool isFatal )
		 {
			  for ( Exception cause = any; cause != null; cause = cause.InnerException )
			  {
					...
					{
						 return new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_General.DatabaseUnavailable, cause, isFatal );
					}
					if ( _cause is Org.Neo4j.Kernel.Api.Exceptions.Status_HasStatus )
					{
						 return new Neo4jError( ( ( Org.Neo4j.Kernel.Api.Exceptions.Status_HasStatus ) _cause ).status(), any.Message, any, isFatal );
					}
					if ( _cause is System.OutOfMemoryException )
					{
						 return new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_General.OutOfMemoryError, _cause, isFatal );
					}
					if ( _cause is StackOverflowError )
					{
						 return new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_General.StackOverFlowError, _cause, isFatal );
					}
			  }
			  ...
			  return new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_General.UnknownError, any != null ? any.Message : null, any, isFatal );
		 }
