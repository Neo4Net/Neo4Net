...
//	import static org.junit.jupiter.api.Assertions.assertThrows;
	...
	{
		 private static readonly string _onePage = Org.Neo4j.Io.pagecache.PageCache_Fields.PAGE_SIZE + "";
		 private static readonly string _eightPages = ( 8 * Org.Neo4j.Io.pagecache.PageCache_Fields.PAGE_SIZE ) + "";
...
//ORIGINAL LINE: @Test void allocatedPointerMustNotBeNull()
		 internal virtual void AllocatedPointerMustNotBeNull()
		 {
			  ...
			  long address = mman.AllocateAligned( Org.Neo4j.Io.pagecache.PageCache_Fields.PAGE_SIZE, 8 );
...
//ORIGINAL LINE: @Test void allocatingMustIncreaseMemoryUsedAndDecreaseAvailableMemory()
		 internal virtual void AllocatingMustIncreaseMemoryUsedAndDecreaseAvailableMemory()
		 {
			  ...
			  assertThat( mman.AvailableMemory(), @is((long) Org.Neo4j.Io.pagecache.PageCache_Fields.PAGE_SIZE) );
			  ...
			  assertThat( mman.AvailableMemory(), @is(lessThanOrEqualTo(Org.Neo4j.Io.pagecache.PageCache_Fields.PAGE_SIZE - 32L)) );

			  // Allocate another 32 bytes of unaligned memory.
			  mman.AllocateAligned( 32, 1 );
			  assertThat( mman.UsedMemory(), @is(greaterThanOrEqualTo(64L)) );
			  assertThat( mman.AvailableMemory(), @is(lessThanOrEqualTo(Org.Neo4j.Io.pagecache.PageCache_Fields.PAGE_SIZE - 64L)) );
			  ...
			  assertThat( mman.AvailableMemory(), @is(lessThanOrEqualTo(Org.Neo4j.Io.pagecache.PageCache_Fields.PAGE_SIZE - 97L)) );
		 }
