...
namespace Org.Neo4j.Tooling
{
	...
	using IllegalMultilineFieldException = Org.Neo4j.Csv.Reader.IllegalMultilineFieldException;
	using GraphDatabaseSettings = Org.Neo4j.Graphdb.factory.GraphDatabaseSettings;
	using Args = Org.Neo4j.Helpers.Args;
	using Org.Neo4j.Helpers.Args;
	...
	using DuplicateInputIdException = Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.@string.DuplicateInputIdException;
	...
	using MissingRelationshipDataException = Org.Neo4j.@unsafe.Impl.Batchimport.input.MissingRelationshipDataException;
	...
	using SpectrumExecutionMonitor = Org.Neo4j.@unsafe.Impl.Batchimport.staging.SpectrumExecutionMonitor;
...
//JAVA TO C# CONVERTER TODO TASK: This Java 'import static' statement cannot be converted to C#:
//	import static org.neo4j.@unsafe.impl.batchimport.Configuration_Fields.DEFAULT_MAX_MEMORY_PERCENT;
//JAVA TO C# CONVERTER TODO TASK: This Java 'import static' statement cannot be converted to C#:
...
//	import static org.neo4j.@unsafe.impl.batchimport.input.csv.DataFactories.defaultFormatRelationshipFileHeader;
	...
	public class ImportTool
	{
		 ...
		 internal sealed class Options
		 {
			  ...
			  public static readonly Options MultilineFields = new Options( "MultilineFields", InnerEnum.MultilineFields, "multiline-fields", Org.Neo4j.Csv.Reader.Configuration_Fields.Default.multilineFields(), "<true/false>", "Whether or not fields from input source can span multiple lines, i.e. contain newline characters." );

			  public static readonly Options TrimStrings = new Options( "TrimStrings", InnerEnum.TrimStrings, "trim-strings", Org.Neo4j.Csv.Reader.Configuration_Fields.Default.trimStrings(), "<true/false>", "Whether or not strings should be trimmed for whitespaces." );
			  ...
			  public static readonly Options IgnoreEmptyStrings = new Options( "IgnoreEmptyStrings", InnerEnum.IgnoreEmptyStrings, "ignore-empty-strings", Org.Neo4j.Csv.Reader.Configuration_Fields.Default.emptyQuotedStringsAsNull(), "<true/false>", "Whether or not empty string fields, i.e. \"\" from input source are ignored, i.e. treated as null." );
			  public static readonly Options IdType = new Options( "IdType", InnerEnum.IdType, "id-type", Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.IdType.String, "<id-type>", "One out of " + java.util.Arrays.ToString( Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.IdType.values() ) + " and specifies how ids in node/relationship " + "input files are treated.\n" + Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.IdType.String + ": arbitrary strings for identifying nodes.\n" + Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.IdType.Integer + ": arbitrary integer values for identifying nodes.\n" + Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.IdType.Actual + ": (advanced) actual node ids. The default option is `" + Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.IdType.String + "`.", true );
			  ...
			  public static readonly Options DatabaseConfig = new Options( "DatabaseConfig", InnerEnum.DatabaseConfig, "db-config", null, "<path/to/" + Org.Neo4j.Kernel.configuration.Config.DEFAULT_CONFIG_FILE_NAME + ">", "(advanced) Option is deprecated and replaced by 'additional-config'. " );
			  public static readonly Options AdditionalConfig = new Options( "AdditionalConfig", InnerEnum.AdditionalConfig, "additional-config", null, "<path/to/" + Org.Neo4j.Kernel.configuration.Config.DEFAULT_CONFIG_FILE_NAME + ">", "(advanced) File specifying database-specific configuration. For more information consult " + "manual about available configuration options for a neo4j configuration file. " + "Only configuration affecting store at time of creation will be read. " + "Examples of supported config are:\n" + Org.Neo4j.Graphdb.factory.GraphDatabaseSettings.DenseNodeThreshold.name() + "\n" + Org.Neo4j.Graphdb.factory.GraphDatabaseSettings.StringBlockSize.name() + "\n" + Org.Neo4j.Graphdb.factory.GraphDatabaseSettings.ArrayBlockSize.name(), true );
			  public static readonly Options LegacyStyleQuoting = new Options( "LegacyStyleQuoting", InnerEnum.LegacyStyleQuoting, "legacy-style-quoting", Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.Configuration.DEFAULT_LEGACY_STYLE_QUOTING, "<true/false>", "Whether or not backslash-escaped quote e.g. \\\" is interpreted as inner quote." );
			  public static readonly Options ReadBufferSize = new Options( "ReadBufferSize", InnerEnum.ReadBufferSize, "read-buffer-size", Org.Neo4j.Csv.Reader.Configuration_Fields.Default.bufferSize(), "<bytes, e.g. 10k, 4M>", "Size of each buffer for reading input data. It has to at least be large enough to hold the " + "biggest single value in the input data." );
...
//ORIGINAL LINE: public static void main(String[] incomingArguments, boolean defaultSettingsSuitableForTests) throws java.io.IOException
		 ...
		 {
			  Console.Error.WriteLine( format( "WARNING: neo4j-import is deprecated and support for it will be removed in a future%n" + "version of Neo4j; please use neo4j-admin import instead." ) );
			  ...
			  try
			  {
					  using ( FileSystemAbstraction fs = new DefaultFileSystemAbstraction() )
					  {
						...
						dbConfig.augment( GraphDatabaseSettings.neo4j_home, storeDir.CanonicalFile.ParentFile.AbsolutePath );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: public static org.neo4j.helpers.Args useArgumentsFromFileArgumentIfPresent(org.neo4j.helpers.Args args) throws java.io.IOException
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: public static void doImport(java.io.PrintStream out, java.io.PrintStream err, java.io.InputStream in, org.neo4j.io.layout.DatabaseLayout databaseLayout, java.io.File badFile, org.neo4j.io.fs.FileSystemAbstraction fs, java.util.Collection<org.neo4j.helpers.Args.Option<java.io.File[]>> nodesFiles, java.util.Collection<org.neo4j.helpers.Args.Option<java.io.File[]>> relationshipsFiles, boolean enableStacktrace, org.neo4j.unsafe.impl.batchimport.input.Input input, org.neo4j.kernel.configuration.Config dbConfig, java.io.OutputStream badOutput, org.neo4j.unsafe.impl.batchimport.Configuration configuration, boolean detailedProgress) throws java.io.IOException
		 public static void DoImport( PrintStream @out, PrintStream err, Stream @in, DatabaseLayout databaseLayout, File badFile, FileSystemAbstraction fs, ICollection<Args.Option<File[]>> nodesFiles, ICollection<Args.Option<File[]>> relationshipsFiles, bool enableStacktrace, Input input, Config dbConfig, Stream badOutput, Org.Neo4j.@unsafe.Impl.Batchimport.Configuration configuration, bool detailedProgress )
		 {
			  bool success;
...
//ORIGINAL LINE: final org.neo4j.scheduler.JobScheduler jobScheduler = life.add(createScheduler());
		 ...
		 internal static void PrintOverview( File storeDir, ICollection<Args.Option<File[]>> nodesFiles, ICollection<Args.Option<File[]>> relationshipsFiles, Org.Neo4j.@unsafe.Impl.Batchimport.Configuration configuration, PrintStream @out )
		 {
			  @out.println( "Neo4j version: " + Version.Neo4jVersion );
		 ...
		 public static Org.Neo4j.@unsafe.Impl.Batchimport.Configuration ImportConfiguration( Number processors, bool defaultSettingsSuitableForTests, Config dbConfig, File storeDir, bool? defaultHighIO )
		 {
		 ...
		 public static Org.Neo4j.@unsafe.Impl.Batchimport.Configuration ImportConfiguration( Number processors, bool defaultSettingsSuitableForTests, Config dbConfig, long? maxMemory, File storeDir, bool allowCacheOnHeap, bool? defaultHighIO )
		 {
...
//ORIGINAL LINE: public static Iterable<org.neo4j.unsafe.impl.batchimport.input.csv.DataFactory> relationshipData(final java.nio.charset.Charset encoding, java.util.Collection<org.neo4j.helpers.Args.Option<java.io.File[]>> relationshipsFiles)
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are ignored unless the option to convert to C# 7.2 'in' parameters is selected:
//ORIGINAL LINE: public static Iterable<org.neo4j.unsafe.impl.batchimport.input.csv.DataFactory> nodeData(final java.nio.charset.Charset encoding, java.util.Collection<org.neo4j.helpers.Args.Option<java.io.File[]>> nodesFiles)
		 ...
		 private static void PrintUsage( PrintStream @out )
		 {
			  ...
			  foreach ( string line in Args.splitLongLine( "neo4j-import is used to create a new Neo4j database " + "from data in CSV files. " + "See the chapter \"Import Tool\" in the Neo4j Manual for details on the CSV file format " + "- a special kind of header is required.", 80 ) )
			  {
					@out.println( "\t" + line );
			  }
			  @out.println( "Usage:" );
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are ignored unless the option to convert to C# 7.2 'in' parameters is selected:
//ORIGINAL LINE: public static org.neo4j.unsafe.impl.batchimport.input.csv.Configuration csvConfiguration(org.neo4j.helpers.Args args, final boolean defaultSettingsSuitableForTests)
		 public static Configuration CsvConfiguration( Args args, bool defaultSettingsSuitableForTests )
		 {
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final org.neo4j.unsafe.impl.batchimport.input.csv.Configuration defaultConfiguration = COMMAS;
			  Configuration defaultConfiguration = COMMAS;
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final System.Nullable<char> specificDelimiter = args.interpretOption(Options.DELIMITER.key(), org.neo4j.kernel.impl.util.Converters.optional(), CHARACTER_CONVERTER);
			  char? specificDelimiter = args.InterpretOption( Options.Delimiter.key(), Converters.optional(), _characterConverter );
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final System.Nullable<char> specificArrayDelimiter = args.interpretOption(Options.ARRAY_DELIMITER.key(), org.neo4j.kernel.impl.util.Converters.optional(), CHARACTER_CONVERTER);
			  char? specificArrayDelimiter = args.InterpretOption( Options.ArrayDelimiter.key(), Converters.optional(), _characterConverter );
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final System.Nullable<char> specificQuote = args.interpretOption(Options.QUOTE.key(), org.neo4j.kernel.impl.util.Converters.optional(), CHARACTER_CONVERTER);
...
//ORIGINAL LINE: final Number bufferSize = args.has(Options.READ_BUFFER_SIZE.key()) ? parseLongWithUnit(args.get(Options.READ_BUFFER_SIZE.key(), null)) : null;
		 ...
		 private class Configuration_DefaultAnonymousInnerClass : Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.Configuration_Default
		 {
			 private bool _defaultSettingsSuitableForTests;
