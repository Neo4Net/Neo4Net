...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 private class ForkedProcessorStepAnonymousInnerClass : ForkedProcessorStep<int[]>
		 {
			 ...
			 public ForkedProcessorStepAnonymousInnerClass( ForkedProcessorStepTest outerInstance, Org.Neo4j.@unsafe.Impl.Batchimport.staging.StageControl control, Configuration config, AtomicIntegerArray reference ) : base( control, "Processor", config )
			 {
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 private class ForkedProcessorStepAnonymousInnerClass2 : ForkedProcessorStep<Void>
		 {
			 ...
			 public ForkedProcessorStepAnonymousInnerClass2( ForkedProcessorStepTest outerInstance, Org.Neo4j.@unsafe.Impl.Batchimport.staging.SimpleStageControl control, Configuration config, Exception testPanic ) : base( control, "Processor", config )
			 {
				 this.outerInstance = outerInstance;
...
//ORIGINAL LINE: java.util.List<Step<?>> steps = asList(execution.steps());
		 ...
		 private class StressStage : Stage
		 {
			  ...
			  private class PullingProducerStepAnonymousInnerClass : PullingProducerStep
			  {
				  ...
				  public PullingProducerStepAnonymousInnerClass( StressStage outerInstance, Org.Neo4j.@unsafe.Impl.Batchimport.staging.StageControl control, Configuration config, int batches ) : base( control, config )
				  {
					  this.outerInstance = outerInstance;
			  ...
			  private class ProcessorStepAnonymousInnerClass : ProcessorStep<long>
			  {
				  ...
				  public ProcessorStepAnonymousInnerClass( StressStage outerInstance, Org.Neo4j.@unsafe.Impl.Batchimport.staging.StageControl control, Configuration config ) : base( control, "Yeah", config, 3 )
				  {
					  this.outerInstance = outerInstance;
				  }
...
//ORIGINAL LINE: protected void process(System.Nullable<long> batch, BatchSender sender) throws Throwable
			  ...
			  private class ForkedProcessorStepAnonymousInnerClass3 : ForkedProcessorStep<long>
			  {
				  ...
				  public ForkedProcessorStepAnonymousInnerClass3( StressStage outerInstance, Org.Neo4j.@unsafe.Impl.Batchimport.staging.StageControl control, Configuration config ) : base( control, "Subject", config )
				  {
					  this.outerInstance = outerInstance;
				  }
