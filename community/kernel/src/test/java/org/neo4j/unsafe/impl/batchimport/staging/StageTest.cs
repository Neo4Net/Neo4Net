...
//ORIGINAL LINE: for (Step<?> step : execution.steps())
		 ...
		 private class PullingProducerStepAnonymousInnerClass : PullingProducerStep
		 {
			 ...
			 public PullingProducerStepAnonymousInnerClass( StageTest outerInstance, Org.Neo4j.@unsafe.Impl.Batchimport.staging.StageControl control, Configuration config, long items ) : base( control, config )
			 {
				 this.outerInstance = outerInstance;
				 this._items = items;
