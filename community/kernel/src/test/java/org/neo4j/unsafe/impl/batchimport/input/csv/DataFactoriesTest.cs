...
namespace Org.Neo4j.@unsafe.Impl.Batchimport.input.csv
{
	...
	using MultiReadable = Org.Neo4j.Csv.Reader.MultiReadable;
	using Readables = Org.Neo4j.Csv.Reader.Readables;
	using IOFunctions = Org.Neo4j.Function.IOFunctions;
...
//ORIGINAL LINE: @Test public void shouldFailOnUnexpectedRelationshipHeaderType()
		 ...
		 private static readonly Org.Neo4j.Csv.Reader.Configuration SEEKER_CONFIG = new Configuration_OverriddenAnonymousInnerClass();

		 private class Configuration_OverriddenAnonymousInnerClass : Org.Neo4j.Csv.Reader.Configuration_Overridden
		 {
			 public Configuration_OverriddenAnonymousInnerClass() : base(new org.neo4j.csv.reader.Configuration_Default())
			 {
			 }
...
//ORIGINAL LINE: private static Configuration withBufferSize(Configuration config, final int bufferSize)
		 ...
		 private class Configuration_OverriddenAnonymousInnerClass : Configuration_Overridden
		 {
			 ...
			 public Configuration_OverriddenAnonymousInnerClass( Org.Neo4j.@unsafe.Impl.Batchimport.Configuration config, int bufferSize ) : base( config )
			 {
