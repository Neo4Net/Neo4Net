...
//ORIGINAL LINE: final java.util.concurrent.atomic.AtomicInteger processCounter = new java.util.concurrent.atomic.AtomicInteger();
		 ...
		 private class StageAnonymousInnerClass : Stage
		 {
			 ...
			 private Org.Neo4j.@unsafe.Impl.Batchimport.Configuration _config;
			 private IEnumerator<RelationshipGroupRecord> _groups;
			 private AtomicInteger _processCounter;

			 public StageAnonymousInnerClass( ReadGroupsFromCacheStepTest outerInstance, Org.Neo4j.@unsafe.Impl.Batchimport.Configuration config, IEnumerator<RelationshipGroupRecord> groups, AtomicInteger processCounter ) : base( this.GetType().Name, null, config, 0 )
			 {
