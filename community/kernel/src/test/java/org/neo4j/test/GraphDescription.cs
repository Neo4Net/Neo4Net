...
//ORIGINAL LINE: public static TestData.Producer<java.util.Map<String, org.neo4j.graphdb.Node>> createGraphFor(final GraphHolder holder, final boolean destroy)
		 ...
		 {
			 private Org.Neo4j.Test.GraphHolder _holder;
			 private bool _destroy;

			 public ProducerAnonymousInnerClass( Org.Neo4j.Test.GraphHolder holder, bool destroy )
			 {
				 this._holder = holder;
		 ...
		 private static T Init<T>( T entity, string name, PROP[] properties, AutoIndexer<T> autoindex, bool auto ) where T : Org.Neo4j.Graphdb.PropertyContainer
		 {
			  ...
			  foreach ( PROP prop in properties )
			  {
					...
					{
					case Org.Neo4j.Test.GraphDescription.PropType.InnerEnum.ARRAY:
