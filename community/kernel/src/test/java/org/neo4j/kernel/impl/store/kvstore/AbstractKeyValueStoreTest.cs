...
namespace Org.Neo4j.Kernel.impl.store.kvstore
{
	...
	using Resources = Org.Neo4j.Test.rule.Resources;
...
//ORIGINAL LINE: private static DataProvider data(final Entry... data)
		 ...
		 {
			 private Org.Neo4j.Kernel.impl.store.kvstore.AbstractKeyValueStoreTest.Entry[] _data;

			 public DataProviderAnonymousInnerClass( Org.Neo4j.Kernel.impl.store.kvstore.AbstractKeyValueStoreTest.Entry[] data )
			 {
				 this._data = data;
			 }
