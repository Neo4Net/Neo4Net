...
namespace Org.Neo4j.Kernel.impl.coreapi
{
	...
	using StubStorageCursors = Org.Neo4j.Storageengine.Api.StubStorageCursors;
...
//ORIGINAL LINE: @Test public void shouldAccessUsernameFromAuthSubject()
		 public virtual void ShouldAccessUsernameFromAuthSubject()
		 {
			  ...
			  when( _transaction.securityContext() ).thenReturn(new SecurityContext(authSubject, Org.Neo4j.@internal.Kernel.Api.security.AccessMode_Static.Full));
...
//ORIGINAL LINE: final org.neo4j.kernel.impl.api.KernelTransactionImplementation transaction = mock(org.neo4j.kernel.impl.api.KernelTransactionImplementation.class);
		 ...
		 private IList<long> IdList<T1>( IEnumerable<T1> entities ) where T1 : Org.Neo4j.Graphdb.PropertyContainer
		 {
