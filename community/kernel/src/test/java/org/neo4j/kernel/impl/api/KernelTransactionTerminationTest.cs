...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 {
			  RunTwoThreads(tx => tx.markForTermination(Org.Neo4j.Kernel.Api.Exceptions.Status_Transaction.TransactionMarkedAsFailed), tx =>
			  {
						  Close( tx );
...
//           TERMINATE { void executeOn(org.neo4j.kernel.api.KernelTransaction tx) { tx.markForTermination(org.neo4j.kernel.api.exceptions.Status_Transaction.TransactionMarkedAsFailed); } };
			  ...
			  internal abstract void executeOn( Org.Neo4j.Kernel.api.KernelTransaction tx );
...
//ORIGINAL LINE: abstract void closeNotTerminated(TestKernelTransaction tx) throws org.neo4j.internal.kernel.api.exceptions.TransactionFailureException;
		 ...
		 private class TestKernelTransaction : KernelTransactionImplementation
		 {
			  ...
			  {
					Initialize( 42, 42, new SimpleStatementLocks( new NoOpClient() ), Org.Neo4j.@internal.Kernel.Api.Transaction_Type.Implicit, AUTH_DISABLED, 0L, 1L );
					Monitor.reset();
					return this;
			  }
			  ...
			  {
					assertEquals( Org.Neo4j.Kernel.Api.Exceptions.Status_Transaction.TransactionMarkedAsFailed, ReasonIfTerminated.get() );
