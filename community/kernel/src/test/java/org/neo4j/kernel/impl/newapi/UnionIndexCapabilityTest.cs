...
//ORIGINAL LINE: @Test public void shouldCreateUnionOfIndexLimitations()
		 public virtual void ShouldCreateUnionOfIndexLimitations()
		 {
			  ...
			  // given
			  union = UnionOfIndexLimitations( Org.Neo4j.@internal.Kernel.Api.IndexCapability_Fields.LimitiationNone, Org.Neo4j.@internal.Kernel.Api.IndexCapability_Fields.LimitiationNone );

			  // then
			  assertEquals( Collections.emptySet(), asSet(union.Limitations()) );

			  // given
			  union = UnionOfIndexLimitations( Org.Neo4j.@internal.Kernel.Api.IndexCapability_Fields.LimitiationNone, array( IndexLimitation.SLOW_CONTAINS ) );
		 ...
		 private IndexCapability MockedIndexCapability()
		 {
			  ...
			  when( mock.Limitations() ).thenReturn(Org.Neo4j.@internal.Kernel.Api.IndexCapability_Fields.LimitiationNone);
			  return mock;
		 }
