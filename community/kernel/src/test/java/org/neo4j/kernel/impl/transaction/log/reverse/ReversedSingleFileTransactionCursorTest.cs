 ...
 */
namespace Org.Neo4j.Kernel.impl.transaction.log.reverse
{
	...
	using UnsupportedLogVersionException = Org.Neo4j.Kernel.impl.transaction.log.entry.UnsupportedLogVersionException;
...
//ORIGINAL LINE: @Rule public final org.junit.rules.RuleChain rules = org.junit.rules.RuleChain.outerRule(random).around(fs).around(directory).around(life).around(expectedException);
		 ...
		 private long _txId = Org.Neo4j.Kernel.impl.transaction.log.TransactionIdStore_Fields.BASE_TX_ID;
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldHandleVerySmallTransactions()
		 {
			  ...
			  // then
			  AssertTransactionRange( readTransactions, _txId, Org.Neo4j.Kernel.impl.transaction.log.TransactionIdStore_Fields.BASE_TX_ID );
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldDetectAndPreventChannelReadingMultipleLogVersions()
		 {
			  ...
			  try
			  {
					  ...
					  {
						new ReversedSingleFileTransactionCursor( channel, new VersionAwareLogEntryReader<Org.Neo4j.Kernel.impl.transaction.log.ReadableClosablePositionAwareChannel>(), false, _monitor );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ReadCorruptedTransactionLog()
		 {
			  ...
			  AssertTransactionRange( committedTransactionRepresentations, readableTransactions + Org.Neo4j.Kernel.impl.transaction.log.TransactionIdStore_Fields.BASE_TX_ID, Org.Neo4j.Kernel.impl.transaction.log.TransactionIdStore_Fields.BASE_TX_ID );
		 }
...
//ORIGINAL LINE: private ReversedSingleFileTransactionCursor txCursor(boolean failOnCorruptedLogFiles) throws java.io.IOException
		 private ReversedSingleFileTransactionCursor TxCursor( bool failOnCorruptedLogFiles )
		 {
			  ...
			  try
			  {
					return new ReversedSingleFileTransactionCursor( fileReader, new VersionAwareLogEntryReader<Org.Neo4j.Kernel.impl.transaction.log.ReadableClosablePositionAwareChannel>(), failOnCorruptedLogFiles, _monitor );
			  }
