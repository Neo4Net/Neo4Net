...
namespace Org.Neo4j.Kernel.impl.transaction.state
{
	...
	using CloneableInPublic = Org.Neo4j.Helpers.CloneableInPublic;
...
//ORIGINAL LINE: private static <T extends org.neo4j.helpers.CloneableInPublic> Iterable<T> cloned(Iterable<T> items, final Class<T> itemClass)
		 private static IEnumerable<T> Cloned<T>( IEnumerable<T> items, Type itemClass ) where T : Org.Neo4j.Helpers.CloneableInPublic
		 {
				 itemClass = typeof( T );
