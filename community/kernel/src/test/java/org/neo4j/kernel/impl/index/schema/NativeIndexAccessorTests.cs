...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ForceShouldCheckpointTree()
		 {
			  ...
			  // when
			  _accessor.force( Org.Neo4j.Io.pagecache.IOLimiter_Fields.Unlimited );
			  _accessor.Dispose();
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldNotSeeFilteredEntries()
		 {
			  ...
			  Org.Neo4j.Storageengine.Api.schema.IndexProgressor_NodeValueClient filterClient = filterClient( iter, filter );
...
//ORIGINAL LINE: private org.neo4j.storageengine.api.schema.IndexProgressor_NodeValueClient filterClient(final NodeValueIterator iter, final org.neo4j.internal.kernel.api.IndexQuery filter)
		 private Org.Neo4j.Storageengine.Api.schema.IndexProgressor_NodeValueClient FilterClient( NodeValueIterator iter, IndexQuery filter )
		 {
			  return new IndexProgressor_NodeValueClientAnonymousInnerClass( this, iter, filter );
		 }

		 private class IndexProgressor_NodeValueClientAnonymousInnerClass : Org.Neo4j.Storageengine.Api.schema.IndexProgressor_NodeValueClient
		 {
			 private readonly NativeIndexAccessorTests<KEY, VALUE> _outerInstance;

			 private Org.Neo4j.Kernel.Impl.Index.Schema.NodeValueIterator _iter;
			 private IndexQuery _filter;

			 public IndexProgressor_NodeValueClientAnonymousInnerClass( NativeIndexAccessorTests<KEY, VALUE> outerInstance, Org.Neo4j.Kernel.Impl.Index.Schema.NodeValueIterator iter, IndexQuery filter )
			 {
