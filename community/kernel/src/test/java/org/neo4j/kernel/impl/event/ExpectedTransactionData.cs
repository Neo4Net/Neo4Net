...
//	import static org.neo4j.kernel.impl.util.AutoCreatingHashMap.nested;
	...
	internal class ExpectedTransactionData
	{
		 ...
		 private PropertyEntryImpl<E> Property<E>( E entity, string key, object value, object valueBeforeTx ) where E : Org.Neo4j.Graphdb.PropertyContainer
		 {
		 ...
		 private IDictionary<KEY, IDictionary<string, PropertyEntryImpl<KEY>>> Clone<KEY>( IDictionary<KEY, IDictionary<string, PropertyEntryImpl<KEY>>> map ) where KEY : Org.Neo4j.Graphdb.PropertyContainer
		 {
		 ...
		 internal virtual void CheckAssigned<T>( IDictionary<T, IDictionary<string, PropertyEntryImpl<T>>> map, PropertyEntry<T> entry ) where T : Org.Neo4j.Graphdb.PropertyContainer
		 {
