...
namespace Org.Neo4j.Kernel.impl.storageengine.impl.recordstorage
{
	...
	using InMemoryVersionableReadableClosablePositionAwareChannel = Org.Neo4j.Kernel.impl.transaction.log.InMemoryVersionableReadableClosablePositionAwareChannel;
	...
	using PrepareTrackingRecordFormats = Org.Neo4j.Kernel.impl.transaction.state.PrepareTrackingRecordFormats;
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 private class CommandVisitor_AdapterAnonymousInnerClass : Org.Neo4j.Kernel.Impl.Api.CommandVisitor_Adapter
		 {
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldLockUpdatedNodes()
		 {
			  ...
			  verify( locks, times( 1 ) ).acquireNodeLock( nodes[0], Org.Neo4j.Kernel.impl.locking.LockService_LockType.WriteLock );
			  // add label, NodeCommand == 1 update
			  verify( locks, times( 1 ) ).acquireNodeLock( nodes[1], Org.Neo4j.Kernel.impl.locking.LockService_LockType.WriteLock );
			  // add property, NodeCommand and PropertyCommand == 2 updates
			  verify( locks, times( 2 ) ).acquireNodeLock( nodes[2], Org.Neo4j.Kernel.impl.locking.LockService_LockType.WriteLock );
			  // update property, in place, PropertyCommand == 1 update
			  verify( locks, times( 1 ) ).acquireNodeLock( nodes[3], Org.Neo4j.Kernel.impl.locking.LockService_LockType.WriteLock );
			  // remove property, updates the Node and the Property == 2 updates
			  verify( locks, times( 2 ) ).acquireNodeLock( nodes[4], Org.Neo4j.Kernel.impl.locking.LockService_LockType.WriteLock );
			  // delete node, single NodeCommand == 1 update
			  verify( locks, times( 1 ) ).acquireNodeLock( nodes[5], Org.Neo4j.Kernel.impl.locking.LockService_LockType.WriteLock );
			  // create and add-label goes into the NodeCommand, add property is a PropertyCommand == 2 updates
			  verify( locks, times( 2 ) ).acquireNodeLock( nodes[6], Org.Neo4j.Kernel.impl.locking.LockService_LockType.WriteLock );
		 }
...
//ORIGINAL LINE: final java.util.concurrent.atomic.AtomicBoolean foundRelationshipGroupInUse = new java.util.concurrent.atomic.AtomicBoolean();
		 ...
		 private class CommandVisitor_AdapterAnonymousInnerClass2 : Org.Neo4j.Kernel.Impl.Api.CommandVisitor_Adapter
		 {
			 private readonly TransactionRecordStateTest _outerInstance;
