...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldPropagateTimeoutException()
		 {
			  ...
			  {
					// then
					assertEquals( Org.Neo4j.Kernel.Api.Exceptions.Status_Transaction.InstanceStateChanged, ex.Status() );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldNotWaitIfTheDatabaseIsUnavailable()
		 {
			  ...
			  {
					// then
					assertEquals( Org.Neo4j.Kernel.Api.Exceptions.Status_General.DatabaseUnavailable, ex.Status() );
			  }
