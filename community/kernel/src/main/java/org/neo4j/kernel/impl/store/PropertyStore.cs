...
namespace Org.Neo4j.Kernel.impl.store
{
	...
	using UnsupportedFormatCapabilityException = Org.Neo4j.Kernel.impl.store.format.UnsupportedFormatCapabilityException;
...
//ORIGINAL LINE: public <FAILURE extends Exception> void accept(RecordStore_Processor<FAILURE> processor, org.neo4j.kernel.impl.store.record.PropertyRecord record) throws FAILURE
		 ...
		 private RecordStore<DynamicRecord> DynamicStoreForValueType( PropertyType type )
		 {
			  ...
			  {
			  case Org.Neo4j.Kernel.impl.store.PropertyType.InnerEnum.ARRAY:
				  return _arrayStore;
			  case Org.Neo4j.Kernel.impl.store.PropertyType.InnerEnum.STRING:
				  return _stringStore;
			  default:
				  return null;
			  }
