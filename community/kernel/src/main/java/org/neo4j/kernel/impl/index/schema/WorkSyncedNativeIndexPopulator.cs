...
//ORIGINAL LINE: void process(java.util.Collection<? extends org.neo4j.kernel.api.index.IndexEntryUpdate<?>> indexEntryUpdates) throws Exception
			  internal virtual void Process<T1>( ICollection<T1> indexEntryUpdates ) where T1 : Org.Neo4j.Kernel.Api.Index.IndexEntryUpdate<T1>
			  {
...
//ORIGINAL LINE: private final java.util.Collection<? extends org.neo4j.kernel.api.index.IndexEntryUpdate<?>> updates;
			  ...
			  internal IndexUpdateWork<T1>( WorkSyncedNativeIndexPopulator<KEY, VALUE> outerInstance, ICollection<T1> updates ) where T1 : Org.Neo4j.Kernel.Api.Index.IndexEntryUpdate<T1>
			  {
				  this._outerInstance = outerInstance;
