...
namespace Org.Neo4j.Kernel.@internal
{
	...
	public class KernelEventHandlers : LifecycleAdapter
	{
		 ...
		 public virtual KernelEventHandler RegisterKernelEventHandler( KernelEventHandler handler )
		 {
			  ...
			  {
					Org.Neo4j.Graphdb.@event.KernelEventHandler_ExecutionOrder order = handler.OrderComparedTo( registeredHandler );
					int index = this._kernelEventHandlers.IndexOf( registeredHandler );
					if ( order == Org.Neo4j.Graphdb.@event.KernelEventHandler_ExecutionOrder.Before )
					{
						 this._kernelEventHandlers.Insert( index, handler );
						 return handler;
					}
					else if ( order == Org.Neo4j.Graphdb.@event.KernelEventHandler_ExecutionOrder.After )
					{
