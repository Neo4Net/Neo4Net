...
namespace Org.Neo4j.Kernel.impl.transaction.command
{
	...
	/// </para>
	/// </summary>
	public class NeoStoreTransactionApplier : Org.Neo4j.Kernel.Impl.Api.TransactionApplier_Adapter
	{
		 ...
		 public override bool VisitPropertyCommand( Command.PropertyCommand command )
		 {
			  ...
			  if ( command.NodeId != -1 )
			  {
					_lockGroup.add( _lockService.acquireNodeLock( command.NodeId, Org.Neo4j.Kernel.impl.locking.LockService_LockType.WriteLock ) );
			  }
			  else if ( command.RelId != -1 )
			  {
					_lockGroup.add( _lockService.acquireRelationshipLock( command.RelId, Org.Neo4j.Kernel.impl.locking.LockService_LockType.WriteLock ) );
			  }
		 ...
		 private RECORD SelectRecordByCommandVersion<RECORD>( BaseCommand<RECORD> command ) where RECORD : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord
		 {
			  switch ( _version )
			  {
