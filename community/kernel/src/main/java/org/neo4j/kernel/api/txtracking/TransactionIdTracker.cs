 ...
 */
namespace Org.Neo4j.Kernel.api.txtracking
{
...
//ORIGINAL LINE: public void awaitUpToDate(long oldestAcceptableTxId, java.time.Duration timeout) throws org.neo4j.internal.kernel.api.exceptions.TransactionFailureException
		 public virtual void AwaitUpToDate( long oldestAcceptableTxId, Duration timeout )
		 {
			  ...
			  {
					throw new TransactionFailureException( Org.Neo4j.Kernel.Api.Exceptions.Status_General.DatabaseUnavailable, "Database unavailable" );
			  }
			  ...
			  catch ( Exception e ) when ( e is InterruptedException || e is TimeoutException )
			  {
					...
					throw new TransactionFailureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Transaction.InstanceStateChanged, e, "Database not up to the requested version: %d. Latest database version is %d", oldestAcceptableTxId, TransactionIdStore().LastClosedTransactionId );
			  }
		 }
