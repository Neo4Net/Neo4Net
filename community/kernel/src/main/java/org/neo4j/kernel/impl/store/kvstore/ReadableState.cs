...
//ORIGINAL LINE: static <Key> ReadableState<Key> store(final KeyFormat<Key> keys, final KeyValueStoreFile store)
		 ...
		 {
			 private Org.Neo4j.Kernel.impl.store.kvstore.KeyFormat<Key> _keys;
			 private Org.Neo4j.Kernel.impl.store.kvstore.KeyValueStoreFile _store;

			 public ReadableStateAnonymousInnerClass( Org.Neo4j.Kernel.impl.store.kvstore.KeyFormat<Key> keys, Org.Neo4j.Kernel.impl.store.kvstore.KeyValueStoreFile store )
			 {
				 this._keys = keys;
				 this._store = store;
			 }
...
//ORIGINAL LINE: static <Key> ReadableState<Key> empty(final KeyFormat<Key> keys, final long version)
		 ...
		 private class ReadableStateAnonymousInnerClass2 : ReadableState<Key>
		 {
			 ...
			 public ReadableStateAnonymousInnerClass2( Org.Neo4j.Kernel.impl.store.kvstore.KeyFormat<Key> keys, long version )
			 {
				 this._keys = keys;
