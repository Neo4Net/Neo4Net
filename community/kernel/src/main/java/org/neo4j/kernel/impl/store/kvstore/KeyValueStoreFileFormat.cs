...
//ORIGINAL LINE: HeaderField<?>[] headerFields = headerFieldsForFormat(formatSpecifier);
		 ...
		 private class MetadataCollectorAnonymousInnerClass : MetadataCollector
		 {
			 ...
			 public MetadataCollectorAnonymousInnerClass<T1>( KeyValueStoreFileFormat outerInstance, Org.Neo4j.Kernel.impl.store.kvstore.HeaderField<T1>[] headerFields, Org.Neo4j.Kernel.impl.store.kvstore.BigEndianByteArrayBuffer specifier ) : base( pageSize / ( keySize + valueSize ), headerFields, specifier )
			 {
...
//ORIGINAL LINE: private KeyValueStoreFile open(org.neo4j.io.fs.FileSystemAbstraction fs, java.io.File path, org.neo4j.io.pagecache.PageCache pages) throws java.io.IOException
		 ...
		 {
			  int pageSize = pages == null ? Org.Neo4j.Io.pagecache.PageCache_Fields.PAGE_SIZE : pages.PageSize();
