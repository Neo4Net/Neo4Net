...
namespace Org.Neo4j.Kernel.Impl.Api
{
	...
	using AuxiliaryTransactionStateCloseException = Org.Neo4j.Kernel.api.txstate.auxiliary.AuxiliaryTransactionStateCloseException;
	using AuxiliaryTransactionStateHolder = Org.Neo4j.Kernel.api.txstate.auxiliary.AuxiliaryTransactionStateHolder;
	...
	using DefaultCursors = Org.Neo4j.Kernel.Impl.Newapi.DefaultCursors;
	using IndexTxStateUpdater = Org.Neo4j.Kernel.Impl.Newapi.IndexTxStateUpdater;
	using KernelToken = Org.Neo4j.Kernel.Impl.Newapi.KernelToken;
...
//	import static org.neo4j.storageengine.api.TransactionApplicationMode.INTERNAL;
	...
	public class KernelTransactionImplementation : KernelTransaction, TxStateHolder, ExecutionStatistics
	{
		 ...
		 private readonly IList<Org.Neo4j.Kernel.api.KernelTransaction_CloseListener> _closeListeners = new List<Org.Neo4j.Kernel.api.KernelTransaction_CloseListener>( 2 );
		 ...
		 private Org.Neo4j.@internal.Kernel.Api.Transaction_Type _type;
		 ...
		 /// </summary>
		 public virtual KernelTransactionImplementation Initialize( long lastCommittedTx, long lastTimeStamp, StatementLocks statementLocks, Org.Neo4j.@internal.Kernel.Api.Transaction_Type type, SecurityContext frozenSecurityContext, long transactionTimeout, long userTransactionId )
		 {
			  this._type = type;
...
//ORIGINAL LINE: private void dropCreatedConstraintIndexes() throws org.neo4j.internal.kernel.api.exceptions.TransactionFailureException
		 ...
		 {
			  foreach ( Org.Neo4j.Kernel.api.KernelTransaction_CloseListener closeListener in _closeListeners )
			  {
		 ...
		 public override long CloseTransaction()
		 {
			  ...
			  try
			  {
					if ( _failure || !_success || Terminated )
					{
						 ...
						 return Org.Neo4j.@internal.Kernel.Api.Transaction_Fields.ROLLBACK;
					}
					else
					{
						 return Commit();
...
//ORIGINAL LINE: private void failOnNonExplicitRollbackIfNeeded() throws org.neo4j.internal.kernel.api.exceptions.TransactionFailureException
		 private void FailOnNonExplicitRollbackIfNeeded()
		 {
			  ...
			  if ( _success )
			  {
					...
					throw new TransactionFailureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Transaction.TransactionMarkedAsFailed, "Transaction rolled back even if marked as successful" );
			  }
		 }
...
//ORIGINAL LINE: private long commit() throws org.neo4j.internal.kernel.api.exceptions.TransactionFailureException
		 ...
		 {
			  bool success = false;
			  long txId = Org.Neo4j.@internal.Kernel.Api.Transaction_Fields.READ_ONLY;
			  ...
			  try
			  {
					  using ( CommitEvent commitEvent = _transactionEvent.beginCommitEvent() )
					  {
						...
						if ( HasDataChanges() )
						{
							 try
							 {
								  ...
								  if ( _hooksState != null && _hooksState.failed() )
								  {
										...
										throw new TransactionFailureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Transaction.TransactionHookFailed, cause, "" );
								  }
							 }
							 finally
							 {
						...
						if ( HasChanges() )
						{
							 ...
							 Org.Neo4j.Kernel.impl.locking.Locks_Client commitLocks = _statementLocks.pessimistic();
...
//ORIGINAL LINE: private void rollback() throws org.neo4j.internal.kernel.api.exceptions.TransactionFailureException
		 private void Rollback()
		 {
			  try
			  {
					...
					{
						 throw new TransactionFailureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Transaction.TransactionRollbackFailed, e, "Could not drop created constraint indexes" );
					}
		 ...
		 private class TxStateVisitor_AdapterAnonymousInnerClass : Org.Neo4j.Storageengine.Api.txstate.TxStateVisitor_Adapter
		 {
...
//JAVA TO C# CONVERTER TODO TASK: Method reference arbitrary object instance method syntax is not converted by Java to C# Converter:
		 ...
		 public override Org.Neo4j.@internal.Kernel.Api.Locks Locks()
		 {
		 ...
		 public override Org.Neo4j.@internal.Kernel.Api.Procedures Procedures()
		 {
		 ...
		 private void AfterRollback()
		 {
			  try
			  {
					MarkAsClosed( Org.Neo4j.@internal.Kernel.Api.Transaction_Fields.ROLLBACK );
					if ( _beforeHookInvoked )
					{
