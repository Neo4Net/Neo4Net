	...
	internal class RecordRelationshipTraversalCursor : RecordRelationshipCursor, StorageRelationshipTraversalCursor
	{
		 ...
		 private void TraverseDenseNode()
		 {
			  while ( _next == NO_ID )
			  {
					...
					switch ( _groupState )
					{
					case Org.Neo4j.Kernel.impl.storageengine.impl.recordstorage.RecordRelationshipTraversalCursor.GroupState.Incoming:
					...
					case Org.Neo4j.Kernel.impl.storageengine.impl.recordstorage.RecordRelationshipTraversalCursor.GroupState.Outgoing:
						 _next = _group.outgoingRawId();
						 _groupState = GroupState.Loop;
						 break;

					case Org.Neo4j.Kernel.impl.storageengine.impl.recordstorage.RecordRelationshipTraversalCursor.GroupState.Loop:
