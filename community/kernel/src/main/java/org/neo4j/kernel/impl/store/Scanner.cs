...
//JAVA TO C# CONVERTER WARNING: Java wildcard generics have no direct equivalent in .NET:
		 public static ResourceIterable<R> Scan<R>( RecordStore<R> store, params System.Predicate<object>[] filters ) where R : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord
		 {
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are ignored unless the option to convert to C# 7.2 'in' parameters is selected:
//JAVA TO C# CONVERTER WARNING: Java wildcard generics have no direct equivalent in .NET:
		 public static ResourceIterable<R> Scan<R>( RecordStore<R> store, bool forward, params System.Predicate<object>[] filters ) where R : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord
		 {
			  return () => new Scan<R>(store, forward, filters);
		 }

		 private class Scan<R> : PrefetchingResourceIterator<R> where R : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord
		 {
