...
//ORIGINAL LINE: void apply(org.neo4j.kernel.api.proc.Context ctx, Object object) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
			  internal virtual void Apply( Org.Neo4j.Kernel.api.proc.Context ctx, object @object )
			  {
					try
					{
					...
					{
						 throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureCallFailed, e, "Unable to inject component to field `%s`, please ensure it is public and non-final: %s", Field.Name, e.Message );
					}
			  }
		 }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: java.util.List<FieldSetter> setters(Class cls) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: private FieldSetter createInjector(Class cls, Field field) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
...
//ORIGINAL LINE: ComponentRegistry.Provider<?> provider = components.providerFor(field.getType());
					...
					if ( provider == null )
					{
						 throw new ComponentInjectionException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Unable to set up injection for procedure `%s`, the field `%s` " + "has type `%s` which is not a known injectable component.", cls.Name, field.Name, field.Type );
					}
			  ...
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Unable to set up injection for `%s`, failed to access field `%s`: %s", e, cls.Name, field.Name, e.Message );
			  }
		 }

//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
//ORIGINAL LINE: private void assertValidForInjection(Class cls, Field field) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
		 private void AssertValidForInjection( Type cls, System.Reflection.FieldInfo field )
		 {
			  if ( !field.isAnnotationPresent( typeof( Context ) ) )
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Field `%s` on `%s` is not annotated as a @" + typeof( Context ).Name + " and is not static. If you want to store state along with your procedure," + " please use a static field.", field.Name, cls.Name );
			  }

			  if ( !Modifier.isPublic( field.Modifiers ) || Modifier.isFinal( field.Modifiers ) )
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Field `%s` on `%s` must be non-final and public.", field.Name, cls.Name );
