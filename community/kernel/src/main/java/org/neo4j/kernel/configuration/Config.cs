...
namespace Org.Neo4j.Kernel.configuration
{
	...
	using Org.Neo4j.Graphdb.config;
	using Org.Neo4j.Graphdb.config;
	using SettingValidator = Org.Neo4j.Graphdb.config.SettingValidator;
	...
	using BufferingLog = Org.Neo4j.Logging.BufferingLog;
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are ignored unless the option to convert to C# 7.2 'in' parameters is selected:
			  public virtual Builder WithConfigClasses<T1>( ICollection<T1> loadableConfigs ) where T1 : Org.Neo4j.Configuration.LoadableConfig
			  {
