...
namespace Org.Neo4j.Kernel.impl.proc
{
	...
	using ComponentInjectionException = Org.Neo4j.Kernel.Api.Exceptions.ComponentInjectionException;
	...
	using FailedLoadAggregatedFunction = Org.Neo4j.Kernel.api.proc.FailedLoadAggregatedFunction;
	using FailedLoadFunction = Org.Neo4j.Kernel.api.proc.FailedLoadFunction;
	using FailedLoadProcedure = Org.Neo4j.Kernel.api.proc.FailedLoadProcedure;
	using OutputMapper = Org.Neo4j.Kernel.impl.proc.OutputMappers.OutputMapper;
...
//ORIGINAL LINE: java.util.List<org.neo4j.kernel.api.proc.CallableUserFunction> compileFunction(Class fcnDefinition) throws org.neo4j.internal.kernel.api.exceptions.KernelException
		 internal virtual IList<CallableUserFunction> CompileFunction( Type fcnDefinition )
		 {
			  ...
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, e, "Failed to compile function defined in `%s`: %s", fcnDefinition.Name, e.Message );
			  }
		 }
...
//ORIGINAL LINE: java.util.List<org.neo4j.kernel.api.proc.CallableProcedure> compileProcedure(Class procDefinition, String warning, boolean fullAccess) throws org.neo4j.internal.kernel.api.exceptions.KernelException
		 internal virtual IList<CallableProcedure> CompileProcedure( Type procDefinition, string warning, bool fullAccess )
		 {
			  ...
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, e, "Failed to compile procedure defined in `%s`: %s", procDefinition.Name, e.Message );
			  }
		 }
...
//ORIGINAL LINE: private org.neo4j.kernel.api.proc.CallableProcedure compileProcedure(Class procDefinition, MethodHandle constructor, Method method, String warning, boolean fullAccess, org.neo4j.internal.kernel.api.procs.QualifiedName procName) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
		 private CallableProcedure CompileProcedure( Type procDefinition, MethodHandle constructor, System.Reflection.MethodInfo method, string warning, bool fullAccess, QualifiedName procName )
		 {
			  ...
			  if ( method.isAnnotationPresent( typeof( PerformsWrites ) ) )
			  {
					...
					{
						 throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Conflicting procedure annotation, cannot use PerformsWrites and mode" );
					}
					else
					{
						 mode = Mode.WRITE;
		 ...
		 private CallableUserAggregationFunction CompileAggregationFunction( Type definition, MethodHandle constructor, System.Reflection.MethodInfo method, QualifiedName funcName )
		 {
			  ...
			  foreach ( System.Reflection.MethodInfo m in aggregator.GetMethods( BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance ) )
			  {
					if ( m.isAnnotationPresent( typeof( UserAggregationUpdate ) ) )
					{
						 if ( update != null )
						 {
							  throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Class '%s' contains multiple methods annotated with '@%s'.", aggregator.Name, typeof( UserAggregationUpdate ).Name );
						 }
						 update = m;
					...
					if ( m.isAnnotationPresent( typeof( UserAggregationResult ) ) )
					{
						 if ( result != null )
						 {
							  throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Class '%s' contains multiple methods annotated with '@%s'.", aggregator.Name, typeof( UserAggregationResult ).Name );
						 }
						 result = m;
					}
			  }
			  ...
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Class '%s' must contain methods annotated with both '@%s' as well as '@%s'.", aggregator.Name, typeof( UserAggregationResult ).Name, typeof( UserAggregationUpdate ).Name );
			  }
			  if ( update.ReturnType != typeof( void ) )
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Update method '%s' in %s has type '%s' but must have return type 'void'.", update.Name, aggregator.Name, update.ReturnType.SimpleName );

			  }
			  if ( !Modifier.isPublic( method.Modifiers ) )
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Aggregation method '%s' in %s must be public.", method.Name, definition.Name );
			  }
			  if ( !Modifier.isPublic( aggregator.Modifiers ) )
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Aggregation class '%s' must be public.", aggregator.Name );
			  }
			  if ( !Modifier.isPublic( update.Modifiers ) )
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Aggregation update method '%s' in %s must be public.", update.Name, aggregator.Name );
			  }
			  if ( !Modifier.isPublic( result.Modifiers ) )
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Aggregation result method '%s' in %s must be public.", result.Name, aggregator.Name );
			  }
...
//ORIGINAL LINE: private MethodHandle constructor(Class procDefinition) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
		 private MethodHandle Constructor( Type procDefinition )
		 {
			  ...
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, e, "Unable to find a usable public no-argument constructor in the class `%s`. " + "Please add a valid, public constructor, recompile the class and try again.", procDefinition.Name );
			  }
		 }
...
//ORIGINAL LINE: protected Object[] mapToObjects(String type, org.neo4j.internal.kernel.api.procs.QualifiedName name, java.util.List<org.neo4j.internal.kernel.api.procs.FieldSignature> inputSignature, org.neo4j.values.AnyValue[] input) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
			  protected internal virtual object[] MapToObjects( string type, QualifiedName name, IList<FieldSignature> inputSignature, AnyValue[] input )
			  {
					...
					{
						 throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureCallFailed, "%s `%s` takes %d arguments but %d was provided.", type, name, inputSignature.Count, input.Length );
					}
...
//ORIGINAL LINE: public org.neo4j.collection.RawIterator<Object[],org.neo4j.internal.kernel.api.exceptions.ProcedureException> apply(org.neo4j.kernel.api.proc.Context ctx, Object[] input, org.neo4j.kernel.api.ResourceTracker resourceTracker) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
			  public override RawIterator<object[], ProcedureException> Apply( Context ctx, object[] input, ResourceTracker resourceTracker )
			  {
					...
					try
					{
						 ...
						 if ( inputSignature.Count != input.Length )
						 {
							  throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureCallFailed, "Procedure `%s` takes %d arguments but %d was provided.", SignatureConflict.name(), inputSignature.Count, input.Length );
						 }
						 ...
						 {
							  SecurityContext securityContext = ctx.Get( Org.Neo4j.Kernel.api.proc.Context_Fields.SecurityContext );
...
//JAVA TO C# CONVERTER TODO TASK: Method reference constructor syntax is not converted by Java to C# Converter:
			  ...
			  internal virtual ProcedureException NewProcedureException( Exception throwable )
			  {
					...
					if ( throwable is Org.Neo4j.Kernel.Api.Exceptions.Status_HasStatus )
					{
						 return new ProcedureException( ( ( Org.Neo4j.Kernel.Api.Exceptions.Status_HasStatus ) throwable ).Status(), throwable, throwable.Message );
					}
					else
					{
						 Exception cause = ExceptionUtils.getRootCause( throwable );
						 return new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureCallFailed, throwable, "Failed to invoke procedure `%s`: %s", SignatureConflict.name(), "Caused by: " + (cause != null ? cause : throwable) );
					}
			  }
		 }
...
//ORIGINAL LINE: public org.neo4j.values.AnyValue apply(org.neo4j.kernel.api.proc.Context ctx, org.neo4j.values.AnyValue[] input) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
			  public override AnyValue Apply( Context ctx, AnyValue[] input )
			  {
					...
					catch ( Exception throwable )
					{
						 if ( throwable is Org.Neo4j.Kernel.Api.Exceptions.Status_HasStatus )
						 {
							  throw new ProcedureException( ( ( Org.Neo4j.Kernel.Api.Exceptions.Status_HasStatus ) throwable ).Status(), throwable, throwable.Message, throwable );
						 }
						 else
						 {
							  Exception cause = ExceptionUtils.getRootCause( throwable );
							  throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureCallFailed, throwable, "Failed to invoke function `%s`: %s", SignatureConflict.name(), "Caused by: " + (cause != null ? cause : throwable) );
						 }
					}
			  }
		 }
...
//ORIGINAL LINE: public org.neo4j.internal.kernel.api.procs.UserAggregator create(org.neo4j.kernel.api.proc.Context ctx) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
			  public override UserAggregator Create( Context ctx )
			  {
					...
					catch ( Exception throwable )
					{
						 if ( throwable is Org.Neo4j.Kernel.Api.Exceptions.Status_HasStatus )
						 {
							  throw new ProcedureException( ( ( Org.Neo4j.Kernel.Api.Exceptions.Status_HasStatus ) throwable ).Status(), throwable, throwable.Message );
						 }
						 else
						 {
							  Exception cause = ExceptionUtils.getRootCause( throwable );
...
//ORIGINAL LINE: public void update(Object[] input) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
				  public void update( object[] input )
				  {
						try
						{
							 ...
							 {
								  throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureCallFailed, "Function `%s` takes %d arguments but %d was provided.", _outerInstance.signature.name(), _expectedNumberOfInputs, input.Length );
							 }
							 // Some input fields are not supported by Cypher and need to be mapped
						...
						catch ( Exception throwable )
						{
							 ...
							 {
								  Exception cause = ExceptionUtils.getRootCause( throwable );
								  throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureCallFailed, throwable, "Failed to invoke function `%s`: %s", _outerInstance.signature.name(), "Caused by: " + (cause != null ? cause : throwable) );
							 }
						}
				  }
...
//ORIGINAL LINE: private static void rejectEmptyNamespace(org.neo4j.internal.kernel.api.procs.QualifiedName name) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
		 private static void RejectEmptyNamespace( QualifiedName name )
		 {
			  ...
			  {
					throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "It is not allowed to define functions in the root namespace please use a namespace, " + "e.g. `@UserFunction(\"org.example.com.%s\")", name.Name() );
			  }
		 }
