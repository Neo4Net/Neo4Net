...
namespace Org.Neo4j.Kernel.impl.traversal
{
	...
	using BranchOrderingPolicies = Org.Neo4j.Graphdb.traversal.BranchOrderingPolicies;
	...
	{
		 internal static readonly System.Func<Resource> NoStatement = () => Org.Neo4j.Graphdb.Resource_Fields.Empty;
...
//JAVA TO C# CONVERTER WARNING: Java wildcard generics have no direct equivalent in .NET:
		 ...
		 public MonoDirectionalTraversalDescription<T1>( System.Func<T1> statementProvider ) where T1 : Org.Neo4j.Graphdb.Resource : this( PathExpanders.allTypesAndDirections(), Uniqueness.NODE_GLOBAL, null, Evaluators.all(), InitialBranchState.NO_STATE, BranchOrderingPolicies.PREORDER_DEPTH_FIRST, null, null, statementProvider )
		 {
		 }
...
//ORIGINAL LINE: public org.neo4j.graphdb.traversal.Traverser traverse(final Iterable<org.neo4j.graphdb.Node> iterableStartNodes)
		 ...
		 {
			  return evaluator( new Org.Neo4j.Graphdb.traversal.Evaluator_AsPathEvaluator( evaluator ) );
		 }
