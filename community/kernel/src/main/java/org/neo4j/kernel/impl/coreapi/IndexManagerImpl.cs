...
namespace Org.Neo4j.Kernel.impl.coreapi
{
	...
	using RelationshipAutoIndexer = Org.Neo4j.Graphdb.index.RelationshipAutoIndexer;
	...
	public class IndexManagerImpl : IndexManager
	{
		 ...
		 public override IDictionary<string, string> GetConfiguration<T1>( Index<T1> index ) where T1 : Org.Neo4j.Graphdb.PropertyContainer
		 {
			  try
			  {
		 ...
		 public override string SetConfiguration<T1>( Index<T1> index, string key, string value ) where T1 : Org.Neo4j.Graphdb.PropertyContainer
		 {
		 ...
		 public override string RemoveConfiguration<T1>( Index<T1> index, string key ) where T1 : Org.Neo4j.Graphdb.PropertyContainer
		 {
			  // Configuration changes should be done transactionally. However this
			  // has always been done non-transactionally, so it's not a regression.
