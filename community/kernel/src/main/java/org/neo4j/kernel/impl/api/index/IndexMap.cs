...
namespace Org.Neo4j.Kernel.Impl.Api.index
{
	...
	public sealed class IndexMap : ICloneable
	{
		 ...
		 /// <returns> set of SchemaDescriptors describing the potentially affected indexes </returns>
		 private ISet<T> GetRelatedDescriptors<T>( SchemaDescriptorLookupSet<T> set, long[] changedLabels, long[] unchangedLabels, int[] sortedProperties, bool propertyListIsComplete ) where T : Org.Neo4j.@internal.Kernel.Api.schema.SchemaDescriptorSupplier
		 {
			  if ( set.Empty )
			  {
