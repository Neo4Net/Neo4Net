...
//       TEMPORAL(14) { public org.neo4j.values.storable.Value value(org.neo4j.kernel.impl.store.record.PropertyBlock block, PropertyStore store) { return TemporalType.decode(block); } public int calculateNumberOfBlocksUsed(long firstBlock) { return TemporalType.calculateNumberOfBlocksUsed(firstBlock); } };
		 ...
		 private static int payloadSize = Org.Neo4j.Kernel.impl.store.format.standard.PropertyRecordFormat.DEFAULT_PAYLOAD_SIZE;

		 private readonly int type;

		 PropertyType( int type ) { this.type = type; } public int intValue() { return type; } public sbyte byteValue() { return(sbyte) type; } public abstract Org.Neo4j.Values.Storable.Value value(Org.Neo4j.Kernel.impl.store.record.PropertyBlock block, PropertyStore store);
