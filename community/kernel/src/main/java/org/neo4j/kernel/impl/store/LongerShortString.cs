...
//       UPPERHEX(12, 4) { int encTranslate(byte b) { if(b >= '0' && b <= '9') { return b - '0'; } if(b >= 'A' && b <= 'F') { return b - 'A' + 10; } throw cannotEncode(b); } int encPunctuation(byte b) { throw cannotEncode(b); } char decTranslate(byte codePoint) { if(codePoint < 10) { return(char)(codePoint + '0'); } return(char)(codePoint + 'A' - 10); } };
		  ...
		  */
		 public static bool Encode( int keyId, string @string, Org.Neo4j.Kernel.impl.store.record.PropertyBlock target, int payloadSize )
		 {
		 ...
		 private static bool EncodeWithCharSet( int keyId, string @string, Org.Neo4j.Kernel.impl.store.record.PropertyBlock target, int payloadSize, int stringLength )
		 {
...
//ORIGINAL LINE: private static boolean tryEncode(int encodings, int keyId, org.neo4j.kernel.impl.store.record.PropertyBlock target, int payloadSize, byte[] data, final int length)
		 private static bool TryEncode( int encodings, int keyId, Org.Neo4j.Kernel.impl.store.record.PropertyBlock target, int payloadSize, sbyte[] data, int length )
		 {
...
//ORIGINAL LINE: final LongerShortString[] retainUri = {NUMERICAL, DATE, UPPER, LOWER, EMAIL, ALPHANUM, ALPHASYM, EUROPEAN, LOWERHEX, UPPERHEX};
		 ...
		 private static void WriteHeader( Org.Neo4j.Kernel.impl.util.Bits bits, int keyId, int encoding, int stringLength )
		 {
		 ...
		 public static Org.Neo4j.Values.Storable.TextValue Decode( Org.Neo4j.Kernel.impl.store.record.PropertyBlock block )
		 {
			  return Decode( block.ValueBlocks, 0, block.ValueBlocks.Length );
		 }

		 public static Org.Neo4j.Values.Storable.TextValue Decode( long[] blocks, int offset, int length )
		 {
		 ...
		 private static Org.Neo4j.Kernel.impl.util.Bits NewBits( LongerShortString encoding, int length )
		 {
			  return Bits.bits( CalculateNumberOfBlocksUsed( encoding, length ) << 3 ); //*8
		 }

		 private static Org.Neo4j.Kernel.impl.util.Bits NewBitsForStep8( int length )
		 {
			  return Bits.bits( CalculateNumberOfBlocksUsedForStep8( length ) << 3 ); //*8
		 }

		 private static bool EncodeLatin1( int keyId, string @string, Org.Neo4j.Kernel.impl.store.record.PropertyBlock target )
		 {
		 ...
		 public static bool WriteLatin1Characters( string @string, Org.Neo4j.Kernel.impl.util.Bits bits )
		 {
			  int length = @string.Length;
			  for ( int i = 0; i < length; i++ )
			  {
		 ...
		 private static bool EncodeUTF8( int keyId, string @string, Org.Neo4j.Kernel.impl.store.record.PropertyBlock target, int payloadSize )
		 {
...
//ORIGINAL LINE: private boolean doEncode(int keyId, byte[] data, org.neo4j.kernel.impl.store.record.PropertyBlock target, int payloadSize, final int length)
		 private bool DoEncode( int keyId, sbyte[] data, Org.Neo4j.Kernel.impl.store.record.PropertyBlock target, int payloadSize, int length )
		 {
...
//ORIGINAL LINE: private void translateData(org.neo4j.kernel.impl.util.Bits bits, byte[] data, int length, final int step)
		 private void TranslateData( Org.Neo4j.Kernel.impl.util.Bits bits, sbyte[] data, int length, int step )
		 {
			  for ( int i = 0; i < length; i++ )
			  {
		 ...
		 private static Org.Neo4j.Values.Storable.TextValue DecodeLatin1( long[] blocks, int offset, int stringLength )
		 {
			  char[] result = new char[stringLength];
			  int block = offset;
			  int maskShift = HEADER_SIZE;
		 ...
		 private static Org.Neo4j.Values.Storable.TextValue DecodeUTF8( long[] blocks, int offset, int stringLength )
		 {
