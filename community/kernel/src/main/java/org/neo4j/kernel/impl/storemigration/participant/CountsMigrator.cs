...
namespace Org.Neo4j.Kernel.impl.storemigration.participant
{
	...
	using StoreFailureException = Org.Neo4j.Kernel.impl.store.StoreFailureException;
		 ...
		 private void RebuildCountsFromScratch( DatabaseLayout sourceStructure, DatabaseLayout migrationStructure, long lastTxId, ProgressReporter progressMonitor, string expectedStoreVersion, PageCache pageCache, LogProvider logProvider )
		 {
			  ...
			  using ( NeoStores neoStores = storeFactory.OpenNeoStores( StoreType.NODE, StoreType.RELATIONSHIP, StoreType.LABEL_TOKEN, StoreType.RELATIONSHIP_TYPE_TOKEN ) )
			  {
					...
					using ( Lifespan life = new Lifespan() )
					{
						 ...
						 CountsComputer initializer = new CountsComputer( lastTxId, nodeStore, relationshipStore, highLabelId, highRelationshipTypeId, NumberArrayFactory.auto( pageCache, migrationStructure.DatabaseDirectory(), true, Org.Neo4j.@unsafe.Impl.Batchimport.cache.NumberArrayFactory_Fields.NoMonitor ), progressMonitor );
