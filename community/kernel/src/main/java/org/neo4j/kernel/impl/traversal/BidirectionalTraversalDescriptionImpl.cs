...
//ORIGINAL LINE: final System.Func<? extends org.neo4j.graphdb.Resource> statementFactory;
		 ...
		 public BidirectionalTraversalDescriptionImpl<T1>( System.Func<T1> statementFactory ) where T1 : Org.Neo4j.Graphdb.Resource : this( new MonoDirectionalTraversalDescription(), new MonoDirectionalTraversalDescription(), STANDARD, Evaluators.all(), ALTERNATING, statementFactory, int.MaxValue )
		 {
		 }
		 ...
		 {
			  return collisionEvaluator( new Org.Neo4j.Graphdb.traversal.Evaluator_AsPathEvaluator( collisionEvaluator ) );
		 }
