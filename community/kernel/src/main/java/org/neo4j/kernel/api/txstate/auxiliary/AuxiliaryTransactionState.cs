 ...
 */
namespace Org.Neo4j.Kernel.api.txstate.auxiliary
{
	using KernelTransactionImplementation = Org.Neo4j.Kernel.Impl.Api.KernelTransactionImplementation;
	using RecordState = Org.Neo4j.Kernel.impl.storageengine.impl.recordstorage.RecordState;

	/// <summary>
	/// An opaque instance of auxiliary transaction state, which can be closed, and may or may not have record changes.
	/// <para>
	/// The implemnentation of this interface is opaque to the <seealso cref="KernelTransactionImplementation"/>, and is entirely controlled by the
