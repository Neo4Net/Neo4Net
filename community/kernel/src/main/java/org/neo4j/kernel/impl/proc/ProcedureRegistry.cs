...
//ORIGINAL LINE: private void validateSignature(String descriptiveName, java.util.List<org.neo4j.internal.kernel.api.procs.FieldSignature> fields, String fieldType) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
		 private void ValidateSignature( string descriptiveName, IList<FieldSignature> fields, string fieldType )
		 {
			  ...
			  foreach ( FieldSignature field in fields )
			  {
					...
					{
						 throw new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureRegistrationFailed, "Procedure `%s` cannot be registered, because it contains a duplicated " + fieldType + " field, '%s'. " + "You need to rename or remove one of the duplicate fields.", descriptiveName, field.Name() );
					}
			  }
		 }
...
//ORIGINAL LINE: public org.neo4j.internal.kernel.api.procs.UserAggregator createAggregationFunction(org.neo4j.kernel.api.proc.Context ctx, int id) throws org.neo4j.internal.kernel.api.exceptions.ProcedureException
		 ...
		 {
			  return new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureNotFound, "There is no procedure with the name `%s` registered for this database instance. " + "Please ensure you've spelled the procedure name correctly and that the " + "procedure is properly deployed.", name );
		 }

		 private ProcedureException NoSuchProcedure( int id )
		 {
			  return new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureNotFound, "There is no procedure with the internal id `%d` registered for this database instance.", id );
		 }

		 private ProcedureException NoSuchFunction( QualifiedName name )
		 {
			  return new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureNotFound, "There is no function with the name `%s` registered for this database instance. " + "Please ensure you've spelled the function name correctly and that the " + "function is properly deployed.", name );
		 }

		 private ProcedureException NoSuchFunction( int id )
		 {
			  return new ProcedureException( Org.Neo4j.Kernel.Api.Exceptions.Status_Procedure.ProcedureNotFound, "There is no function with the internal id `%d` registered for this database instance.", id );
		 }
