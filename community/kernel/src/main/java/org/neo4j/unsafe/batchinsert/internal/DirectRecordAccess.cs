...
namespace Org.Neo4j.@unsafe.Batchinsert.@internal
{
	...
	/// </summary>
	public class DirectRecordAccess<RECORD, ADDITIONAL> : RecordAccess<RECORD, ADDITIONAL> where RECORD : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord
	{
		 private readonly RecordStore<RECORD> _store;
		 private readonly Org.Neo4j.Kernel.impl.transaction.state.RecordAccess_Loader<RECORD, ADDITIONAL> _loader;
		 ...
		 public DirectRecordAccess( RecordStore<RECORD> store, Org.Neo4j.Kernel.impl.transaction.state.RecordAccess_Loader<RECORD, ADDITIONAL> loader )
		 {
			  this._store = store;
			  this._loader = loader;
		 }

		 public override Org.Neo4j.Kernel.impl.transaction.state.RecordAccess_RecordProxy<RECORD, ADDITIONAL> GetOrLoad( long key, ADDITIONAL additionalData )
		 {
		 ...
		 private Org.Neo4j.Kernel.impl.transaction.state.RecordAccess_RecordProxy<RECORD, ADDITIONAL> PutInBatch( long key, DirectRecordProxy proxy )
		 {
		 ...
		 public override Org.Neo4j.Kernel.impl.transaction.state.RecordAccess_RecordProxy<RECORD, ADDITIONAL> Create( long key, ADDITIONAL additionalData )
		 {
			  return Proxy( key, _loader.newUnused( key, additionalData ), additionalData, true );
		 }

		 public override Org.Neo4j.Kernel.impl.transaction.state.RecordAccess_RecordProxy<RECORD, ADDITIONAL> GetIfLoaded( long key )
		 {
			  return _batch[key];
		 }
		 ...
		 public override Org.Neo4j.Kernel.impl.transaction.state.RecordAccess_RecordProxy<RECORD, ADDITIONAL> SetRecord( long key, RECORD record, ADDITIONAL additionalData )
		 {
			  throw new System.NotSupportedException( "Not supported" );
		 }
		 ...
		 public override IEnumerable<Org.Neo4j.Kernel.impl.transaction.state.RecordAccess_RecordProxy<RECORD, ADDITIONAL>> Changes()
		 {
			  return new IterableWrapperAnonymousInnerClass( this, _batch.Values );
		 }

		 private class IterableWrapperAnonymousInnerClass : IterableWrapper<Org.Neo4j.Kernel.impl.transaction.state.RecordAccess_RecordProxy<RECORD, ADDITIONAL>, DirectRecordProxy>
		 {
			 ...
			 protected internal override Org.Neo4j.Kernel.impl.transaction.state.RecordAccess_RecordProxy<RECORD, ADDITIONAL> underlyingObjectToObject( DirectRecordProxy @object )
			 {
				  return @object;
			 }
		 }
...
//ORIGINAL LINE: private DirectRecordProxy proxy(final long key, final RECORD record, final ADDITIONAL additionalData, boolean created)
		 ...
		 private class DirectRecordProxy : Org.Neo4j.Kernel.impl.transaction.state.RecordAccess_RecordProxy<RECORD, ADDITIONAL>
		 {
			 private readonly DirectRecordAccess<RECORD, ADDITIONAL> _outerInstance;
