...
namespace Org.Neo4j.@unsafe.Impl.Batchimport
{
	...
	using RelationshipTypeCount = Org.Neo4j.@unsafe.Impl.Batchimport.DataStatistics.RelationshipTypeCount;
	...
	using PageCacheArrayFactoryMonitor = Org.Neo4j.@unsafe.Impl.Batchimport.cache.PageCacheArrayFactoryMonitor;
	...
	using EstimationSanityChecker = Org.Neo4j.@unsafe.Impl.Batchimport.input.EstimationSanityChecker;
	...
	using ExecutionSupervisors = Org.Neo4j.@unsafe.Impl.Batchimport.staging.ExecutionSupervisors;
...
//ORIGINAL LINE: final org.eclipse.collections.api.set.primitive.IntSet typesToLinkThisRound = relationshipTypeDistribution.types(startingFromType, upToType);
		 ...
		 public virtual void BuildCountsStore()
		 {
			  ...
			  using ( Org.Neo4j.Kernel.Impl.Api.CountsAccessor_Updater countsUpdater = _neoStore.CountsStore.reset( _neoStore.LastCommittedTransactionId ) )
			  {
...
//ORIGINAL LINE: public void close() throws java.io.IOException
		 ...
		 private static long TotalMemoryUsageOf( params Org.Neo4j.@unsafe.Impl.Batchimport.cache.MemoryStatsVisitor_Visitable[] users )
		 {
			  GatheringMemoryStatsVisitor total = new GatheringMemoryStatsVisitor();
			  foreach ( Org.Neo4j.@unsafe.Impl.Batchimport.cache.MemoryStatsVisitor_Visitable user in users )
			  {
					if ( user != null )
					{
