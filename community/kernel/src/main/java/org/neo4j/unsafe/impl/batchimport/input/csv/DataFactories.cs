...
//ORIGINAL LINE: public static DataFactory data(final Decorator decorator, final java.nio.charset.Charset charset, final java.io.File... files)
		 ...
		 {
			 private Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.Decorator _decorator;
			 private Charset _charset;
			 private File[] _files;

			 public DataAnonymousInnerClass( Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.Decorator decorator, Charset charset, File[] files )
			 {
...
//ORIGINAL LINE: public static DataFactory data(final Decorator decorator, final System.Func<org.neo4j.csv.reader.CharReadable> readable)
		 ...
		 private class DataAnonymousInnerClass2 : Data
		 {
			 ...
			 public DataAnonymousInnerClass2( Org.Neo4j.@unsafe.Impl.Batchimport.input.csv.Decorator decorator, System.Func<CharReadable> readable )
			 {
				 this._decorator = decorator;
...
//ORIGINAL LINE: org.neo4j.csv.reader.Extractor<?> idExtractor = idType.extractor(extractors);
						 ...
						 for ( int i = 0; !mark.EndOfLine && dataSeeker.Seek( mark, delimiter ); i++ )
						 {
							  ...
							  {
									columns.Add( new Header.Entry( null, Type.Ignore, Org.Neo4j.@unsafe.Impl.Batchimport.input.Group_Fields.Global, null, null ) );
							  }
							  else
							  {
