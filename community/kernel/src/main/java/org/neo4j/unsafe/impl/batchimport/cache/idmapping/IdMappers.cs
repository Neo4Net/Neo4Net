 ...
 */
namespace Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping
{
	...
	using EncodingIdMapper = Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.@string.EncodingIdMapper;
	using LongCollisionValues = Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.@string.LongCollisionValues;
	using LongEncoder = Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.@string.LongEncoder;
	using Radix = Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.@string.Radix;
	using StringCollisionValues = Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.@string.StringCollisionValues;
	using StringEncoder = Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.@string.StringEncoder;
...
//	import static org.neo4j.@unsafe.impl.batchimport.cache.idmapping.@string.TrackerFactories.dynamic;
	...
	public class IdMappers
	{
		 private class ActualIdMapper : IdMapper
		 {
			  ...
			  public override Org.Neo4j.@unsafe.Impl.Batchimport.cache.MemoryStatsVisitor_Visitable MemoryEstimation( long numberOfNodes )
			  {
					return Org.Neo4j.@unsafe.Impl.Batchimport.cache.MemoryStatsVisitor_Fields.None;
			  }
