...
namespace Org.Neo4j.@unsafe.Impl.Batchimport.input.csv
{
	...
	using CSVHeaderInformation = Org.Neo4j.Values.Storable.CSVHeaderInformation;
...
//ORIGINAL LINE: private final org.neo4j.csv.reader.Extractor<?> extractor;
...
//JAVA TO C# CONVERTER NOTE: Fields cannot have the same name as methods:
			  ...
			  public override string ToString()
			  {
					...
					{
						 return ( !string.ReferenceEquals( NameConflict, null ) ? NameConflict : "" ) + ":" + ( TypeConflict == Type.Property ? ExtractorConflict.name().ToLower() : TypeConflict.name() ) + (Group() != Org.Neo4j.@unsafe.Impl.Batchimport.input.Group_Fields.Global ? "(" + Group().name() + ")" : "");
					}
					else
					{
						 return ( !string.ReferenceEquals( NameConflict, null ) ? NameConflict : "" ) + ":" + ( TypeConflict == Type.Property ? ExtractorConflict.name().ToLower() + "[" + OptionalParameterConflict + "]" : TypeConflict.name() ) + (Group() != Org.Neo4j.@unsafe.Impl.Batchimport.input.Group_Fields.Global ? "(" + Group().name() + ")" : "");
					}
			  }

//JAVA TO C# CONVERTER WARNING: Java wildcard generics have no direct equivalent in .NET:
//ORIGINAL LINE: public org.neo4j.csv.reader.Extractor<?> extractor()
			  ...
			  {
					return GroupConflict != null ? GroupConflict : Org.Neo4j.@unsafe.Impl.Batchimport.input.Group_Fields.Global;
			  }
