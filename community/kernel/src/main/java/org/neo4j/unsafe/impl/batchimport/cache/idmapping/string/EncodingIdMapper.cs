...
//	import static org.neo4j.@unsafe.impl.batchimport.cache.idmapping.@string.ParallelSort.DEFAULT;
	...
	public class EncodingIdMapper : IdMapper
	{
		 ...
		 private long BinarySearch( object inputId, int groupId )
		 {
			  ...
			  if ( returnVal == Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.IdMapper_Fields.ID_NOT_FOUND )
			  {
					low = 0;
		 ...
		 private class DetectWorker : ThreadStart
		 {
			 private readonly EncodingIdMapper _outerInstance;
			  ...
			  internal virtual void Detect( SameGroupDetector sameGroupDetector, long i )
			  {
					...
					if ( dataIndexA == Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.IdMapper_Fields.ID_NOT_FOUND || dataIndexB == Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.IdMapper_Fields.ID_NOT_FOUND )
					{
					...
					case EQ:
						 ...
						 if ( collision != Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.IdMapper_Fields.ID_NOT_FOUND )
						 {
...
//ORIGINAL LINE: private void detectDuplicateInputIds(Radix radix, org.neo4j.unsafe.impl.batchimport.input.Collector collector, org.neo4j.helpers.progress.ProgressListener progress) throws InterruptedException
		 ...
		 private long BinarySearch( long x, object inputId, long low, long high, int groupId )
		 {
			  while ( low <= high )
			  {
					...
					if ( dataIndex == Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.IdMapper_Fields.ID_NOT_FOUND )
					{
						 return Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.IdMapper_Fields.ID_NOT_FOUND;
					}
					...
					case EQ:
						 ...
						 return GroupOf( dataIndex ) == groupId ? dataIndex : Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.IdMapper_Fields.ID_NOT_FOUND;
					case LT:
						 low = mid + 1;
						 break;
					default:
		 ...
		 {
			  long lowestFound = Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.IdMapper_Fields.ID_NOT_FOUND; // lowest data index means "first put"
					...
					if ( groupId == group )
					{
						 ...
						 if ( IsCollision( eId ) )
						 {
							  ...
							  { // We found a data value for our group, but there are collisions within this group.
									...
									{
										 // :)
										 lowestFound = lowestFound == Org.Neo4j.@unsafe.Impl.Batchimport.cache.idmapping.IdMapper_Fields.ID_NOT_FOUND ? nodeId : min( lowestFound, nodeId );
