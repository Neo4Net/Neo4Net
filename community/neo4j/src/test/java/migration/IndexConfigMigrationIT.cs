...
//ORIGINAL LINE: @ExtendWith(TestDirectoryExtension.class) class IndexConfigMigrationIT
	internal class IndexConfigMigrationIT
	{
		 private sealed class MinMaxSetting
		 {
			  ...
			  internal MinMaxSetting( string name, InnerEnum innerEnum, Org.Neo4j.Graphdb.config.Setting<double> setting, string settingValue )
			  {
		 ...
		 private sealed class FulltextIndexDescription
		 {
			  ...
			  internal FulltextIndexDescription( string name, InnerEnum innerEnum, string indexName, bool nodeIndex, string tokenName, IDictionary<string, Org.Neo4j.Values.Storable.Value> configMap )
			  {
