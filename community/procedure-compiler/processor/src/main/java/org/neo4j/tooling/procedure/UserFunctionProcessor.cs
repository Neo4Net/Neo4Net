...
namespace Org.Neo4j.Tooling.procedure
{
	...
	using UserFunctionVisitor = Org.Neo4j.Tooling.procedure.visitors.UserFunctionVisitor;
		 ...
		 {
		  {
				...
				return new UserFunctionVisitor( new FunctionVisitor<Org.Neo4j.Procedure.UserFunction>( _supportedAnnotationType, typeUtils, elementUtils, typeMirrorUtils, CustomNameExtractor(), processingEnvironment.Options.containsKey(IGNORE_CONTEXT_WARNINGS_OPTION) ) );
		  });
		 }
