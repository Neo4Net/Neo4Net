...
namespace Org.Neo4j.Server.rest.transactional
{
	...
	using RFC1123 = Org.Neo4j.Server.rest.repr.util.RFC1123;
...
//ORIGINAL LINE: private void writePlanIdentifiers(org.neo4j.graphdb.ExecutionPlanDescription planDescription) throws java.io.IOException
		 ...
		 public virtual void Errors<T1>( IEnumerable<T1> errors ) where T1 : Org.Neo4j.Server.rest.transactional.error.Neo4jError
		 {
			  try
			  {
					EnsureDocumentOpen();
