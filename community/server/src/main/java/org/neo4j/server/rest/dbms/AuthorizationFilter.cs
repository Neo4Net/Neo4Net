...
//ORIGINAL LINE: protected static org.neo4j.function.ThrowingConsumer<javax.servlet.http.HttpServletResponse, java.io.IOException> unauthorizedAccess(final String message)
		 ...
		 {
			  return Error( 403, map( "errors", singletonList( map( "code", Org.Neo4j.Kernel.Api.Exceptions.Status_Security.Forbidden.code().serialize(), "message", string.Format("Unauthorized access violation: {0}.", message) ) ) ) );
		 }
