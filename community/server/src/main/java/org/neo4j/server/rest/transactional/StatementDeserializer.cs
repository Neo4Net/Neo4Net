...
//ORIGINAL LINE: private boolean beginsWithCorrectTokens() throws java.io.IOException
		 private bool BeginsWithCorrectTokens()
		 {
			  ...
			  for ( int i = 0; i < expectedTokens.Count; i++ )
			  {
					...
					{
						 AddError( new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, new DeserializationException( string.Format( "Unable to deserialize request. " + "Expected first field to be '{0}', but was '{1}'.", expectedField, _input.Text ) ) ) );
						 return false;
					}
...
//ORIGINAL LINE: if (!expectedTokens.equals(foundTokens))
			  ...
			  {
					AddError( new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, new DeserializationException( string.Format( "Unable to deserialize request. " + "Expected {0}, found {1}.", expectedTokens, foundTokens ) ) ) );
					return false;
			  }
			  return true;
		 }
