...
namespace Org.Neo4j.Server.rest.dbms
{
	...
	using AuthProviderFailedException = Org.Neo4j.Graphdb.security.AuthProviderFailedException;
	using AuthProviderTimeoutException = Org.Neo4j.Graphdb.security.AuthProviderTimeoutException;
	...
	using XForwardUtil = Org.Neo4j.Server.web.XForwardUtil;
...
//ORIGINAL LINE: private org.neo4j.internal.kernel.api.security.LoginContext authenticate(String username, String password) throws org.neo4j.kernel.api.security.exception.InvalidAuthTokenException
		 ...
		 private static readonly ThrowingConsumer<HttpServletResponse, IOException> _noHeader = Error( 401, map( "errors", singletonList( map( "code", Org.Neo4j.Kernel.Api.Exceptions.Status_Security.Unauthorized.code().serialize(), "message", "No authentication header supplied." ) ) ) );

		 private static readonly ThrowingConsumer<HttpServletResponse, IOException> _badHeader = Error( 400, map( "errors", singletonList( map( "code", Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat.code().serialize(), "message", "Invalid authentication header." ) ) ) );

		 private static readonly ThrowingConsumer<HttpServletResponse, IOException> _invalidCredential = Error( 401, map( "errors", singletonList( map( "code", Org.Neo4j.Kernel.Api.Exceptions.Status_Security.Unauthorized.code().serialize(), "message", "Invalid username or password." ) ) ) );

		 private static readonly ThrowingConsumer<HttpServletResponse, IOException> _tooManyAttempts = Error( 429, map( "errors", singletonList( map( "code", Org.Neo4j.Kernel.Api.Exceptions.Status_Security.AuthenticationRateLimit.code().serialize(), "message", "Too many failed authentication requests. Please wait 5 seconds and try again." ) ) ) );

		 private static readonly ThrowingConsumer<HttpServletResponse, IOException> _authProviderFailed = Error( 502, map( "errors", singletonList( map( "code", Org.Neo4j.Kernel.Api.Exceptions.Status_Security.AuthProviderFailed.code().serialize(), "message", "An auth provider request failed." ) ) ) );

		 private static readonly ThrowingConsumer<HttpServletResponse, IOException> _authProviderTimeout = Error( 504, map( "errors", singletonList( map( "code", Org.Neo4j.Kernel.Api.Exceptions.Status_Security.AuthProviderTimeout.code().serialize(), "message", "An auth provider request timed out." ) ) ) );
...
//ORIGINAL LINE: private static org.neo4j.function.ThrowingConsumer<javax.servlet.http.HttpServletResponse, java.io.IOException> invalidAuthToken(final String message)
		 ...
		 {
			  return Error( 401, map( "errors", singletonList( map( "code", Org.Neo4j.Kernel.Api.Exceptions.Status_Security.Unauthorized.code().serialize(), "message", message ) ) ) );
		 }
...
//ORIGINAL LINE: private static org.neo4j.function.ThrowingConsumer<javax.servlet.http.HttpServletResponse, java.io.IOException> passwordChangeRequired(final String username, final String baseURL)
		 private static ThrowingConsumer<HttpServletResponse, IOException> PasswordChangeRequired( string username, string baseURL )
		 {
			  ...
			  return Error( 403, map( "errors", singletonList( map( "code", Org.Neo4j.Kernel.Api.Exceptions.Status_Security.Forbidden.code().serialize(), "message", "User is required to change their password." ) ), "password_change", path.ToString() ) );
		 }
