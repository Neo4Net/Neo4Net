...
namespace Org.Neo4j.Server.rest.web
{
	...
	using BatchOperationResults = Org.Neo4j.Server.rest.batch.BatchOperationResults;
	using NonStreamingBatchOperations = Org.Neo4j.Server.rest.batch.NonStreamingBatchOperations;
...
//ORIGINAL LINE: public void write(int i) throws java.io.IOException
		 ...
		 {
			  if ( "true".Equals( httpHeaders.RequestHeaders.getFirst( Org.Neo4j.Server.rest.repr.StreamingFormat_Fields.STREAM_HEADER ), StringComparison.OrdinalIgnoreCase ) )
			  {
					return true;
			  }
