...
//ORIGINAL LINE: @Test public void shouldRejectMapWithADifferentFieldBeforeStatement()
		 public virtual void ShouldRejectMapWithADifferentFieldBeforeStatement()
		 {
			  ...
			  AssertYieldsErrors( json, new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, new DeserializationException( "Unable to deserialize request. Expected first field to be 'statements', but was 'timeout'." ) ) );
		 }
...
//ORIGINAL LINE: @Test public void shouldNotThrowButReportErrorOnInvalidInput()
		 ...
		 {
			  AssertYieldsErrors( "{}", new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, new DeserializationException( "Unable to " + "deserialize request. " + "Expected [START_OBJECT, FIELD_NAME, START_ARRAY], " + "found [START_OBJECT, END_OBJECT, null]." ) ) );

			  AssertYieldsErrors( "{ \"statements\":\"WAIT WAT A STRING NOO11!\" }", new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, new DeserializationException( "Unable to " + "deserialize request. Expected [START_OBJECT, FIELD_NAME, START_ARRAY], found [START_OBJECT, " + "FIELD_NAME, VALUE_STRING]." ) ) );

			  AssertYieldsErrors( "[{]}", new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, new DeserializationException( "Unable to deserialize request: Unexpected close marker ']': " + "expected '}' " + "(for OBJECT starting at [Source: TestInputStream; line: 1, column: 1])\n " + "at [Source: TestInputStream; line: 1, column: 4]" ) ) );

			  AssertYieldsErrors( "{ \"statements\" : \"ITS A STRING\" }", new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, new DeserializationException( "Unable to deserialize request. " + "Expected [START_OBJECT, FIELD_NAME, START_ARRAY], " + "found [START_OBJECT, FIELD_NAME, VALUE_STRING]." ) ) );

			  AssertYieldsErrors( "{ \"statements\" : [ { \"statement\" : [\"dd\"] } ] }", new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, new DeserializationException( "Unable to deserialize request: Can not deserialize instance of" + " java.lang.String out of START_ARRAY token\n at [Source: TestInputStream; line: 1, " + "column: 22]" ) ) );

			  AssertYieldsErrors( "{ \"statements\" : [ { \"statement\" : \"stmt\", \"parameters\" : [\"AN ARRAY!!\"] } ] }", new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, new DeserializationException( "Unable to deserialize request: Can not deserialize instance of" + " java.util.LinkedHashMap out of START_ARRAY token\n at [Source: TestInputStream; " + "line: 1, column: 42]" ) ) );
		 }
