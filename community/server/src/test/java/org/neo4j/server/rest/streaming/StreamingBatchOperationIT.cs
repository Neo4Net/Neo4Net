 ...
 */
namespace Org.Neo4j.Server.rest.streaming
{
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ReferToItemsCreatedEarlierInTheSameBatchJobStreaming()
		 {
			  ...
			  string entity = GenConflict.get().expectedType(APPLICATION_JSON_TYPE).withHeader(Org.Neo4j.Server.rest.repr.StreamingFormat_Fields.STREAM_HEADER, "true").expectedStatus(200).payload(jsonString).post(BatchUri()).entity();
