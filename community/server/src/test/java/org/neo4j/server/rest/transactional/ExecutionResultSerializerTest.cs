...
namespace Org.Neo4j.Server.rest.transactional
{
	...
	using NotificationCode = Org.Neo4j.Graphdb.impl.notification.NotificationCode;
	...
	using GraphMock = Org.Neo4j.Test.mockito.mock.GraphMock;
	using Link = Org.Neo4j.Test.mockito.mock.Link;
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldSerializeResponseWithCommitUriAndResultsAndErrors()
		 {
			  ...
			  serializer.TransactionCommitUri( URI.create( "commit/uri/1" ) );
			  serializer.StatementResult( executionResult, false );
			  serializer.Errors( asList( new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_Request.InvalidFormat, new Exception( "cause1" ) ) ) );
			  serializer.Finish();
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldErrorWhenSerializingUnknownGeometryType()
		 {
			  ...
			  {
					serializer.Errors( asList( new Neo4jError( Org.Neo4j.Kernel.Api.Exceptions.Status_Statement.ExecutionFailed, e ) ) );
			  }
