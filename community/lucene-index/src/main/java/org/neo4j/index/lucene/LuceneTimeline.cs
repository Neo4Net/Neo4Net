...
//	import static org.neo4j.index.lucene.ValueContext.numeric;
	...
	public class LuceneTimeline<T> : TimelineIndex<T> where T : Org.Neo4j.Graphdb.PropertyContainer
	{
		 ...
		 private void AssertIsLuceneIndex( GraphDatabaseService db, Index<T> index )
		 {
			  ...
			  if ( !config[Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER].Equals( "lucene" ) ) // Not so hard coded please
			  {
