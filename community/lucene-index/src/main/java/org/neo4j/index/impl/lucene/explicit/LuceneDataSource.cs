...
//ORIGINAL LINE: public void shutdown() throws java.io.IOException
		 ...
		 private IndexReference[] AllIndexes
		 {
			 get
			 {
				 lock ( this )
				 {
					  Org.Neo4j.Index.impl.lucene.@explicit.IndexClockCache.ValueCollection indexReferences = _indexSearchers.values();
...
//ORIGINAL LINE: org.neo4j.graphdb.ResourceIterator<java.io.File> listStoreFiles() throws java.io.IOException
		 ...
		 private void MakeSureAllIndexesAreInstantiated()
		 {
			  foreach ( string name in _indexStore.getNames( typeof( Node ) ) )
			  {
					...
					if ( config[Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER].Equals( LuceneIndexImplementation.SERVICE_NAME ) )
					{
...
//ORIGINAL LINE: abstract org.apache.lucene.store.Directory getDirectory(java.io.File baseStorePath, IndexIdentifier identifier) throws java.io.IOException;
			  ...
			  internal abstract java.io.File ensureDirectoryExists( Org.Neo4j.Io.fs.FileSystemAbstraction fileSystem, java.io.File path );
