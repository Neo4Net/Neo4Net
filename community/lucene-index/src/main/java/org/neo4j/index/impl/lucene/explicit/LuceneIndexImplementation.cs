...
namespace Org.Neo4j.Index.impl.lucene.@explicit
{
	...
	public class LuceneIndexImplementation : LifecycleAdapter, IndexImplementation
	{
		 ...
		 public static readonly IDictionary<string, string> ExactConfig = Collections.unmodifiableMap( MapUtil.stringMap( Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER, SERVICE_NAME, KEY_TYPE, "exact" ) );

		 public static readonly IDictionary<string, string> FulltextConfig = Collections.unmodifiableMap( MapUtil.stringMap( Org.Neo4j.Graphdb.index.IndexManager_Fields.PROVIDER, SERVICE_NAME, KEY_TYPE, "fulltext", KEY_TO_LOWER_CASE, "true" ) );
