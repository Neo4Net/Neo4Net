...
//JAVA TO C# CONVERTER WARNING: Java wildcard generics have no direct equivalent in .NET:
				  Visitor<object, ? extends Exception> target = Org.Neo4j.Helpers.Collection.Visitor_SafeGenerics.CastOrNull( typeof( DiagnosticsProvider ), typeof( Exception ), visitor );
				  if ( target != null )
				  {
						foreach ( DiagnosticsProvider provider in _outerInstance.providers )
						{
