...
namespace Org.Neo4j.tools.dump
{
	...
	using PrintStreamLogger = Org.Neo4j.Logging.PrintStreamLogger;
...
//	import static org.neo4j.kernel.impl.store.record.RecordLoad.FORCE;
	...
	public class DumpStore<RECORD, STORE> where RECORD : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord where STORE : Org.Neo4j.Kernel.impl.store.RecordStore<RECORD>
	{
...
//ORIGINAL LINE: private static <R extends org.neo4j.kernel.impl.store.record.AbstractBaseRecord, S extends org.neo4j.kernel.impl.store.RecordStore<R>> void dump(IdRange[] ids, S store) throws Exception
		 private static void Dump<R, S>( IdRange[] ids, S store ) where R : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord where S : Org.Neo4j.Kernel.impl.store.RecordStore<R>
		 {
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are ignored unless the option to convert to C# 7.2 'in' parameters is selected:
		 private static void DumpTokens<R>( TokenStore<R> store, IdRange[] ids ) where R : Org.Neo4j.Kernel.impl.store.record.TokenRecord
		 {
			  try
			  {
