...
//JAVA TO C# CONVERTER NOTE: Fields cannot have the same name as methods:
			  ...
			  {
					return ClientWith( Org.Neo4j.com.storecopy.ResponseUnpacker_Fields.NoOpResponseUnpacker );
			  }
		 ...
		 public class TransactionStreamVerifyingResponseHandler : Response.Handler, Visitor<CommittedTransactionRepresentation, Exception>
		 {
			 private readonly CommunicationIT _outerInstance;
			  ...
			  {
					assertEquals( ExpectedTxId + Org.Neo4j.Kernel.impl.transaction.log.TransactionIdStore_Fields.BASE_TX_ID, element.CommitEntry.TxId );
					ExpectedTxId++;
					assertThat( element.CommitEntry.TxId, lessThanOrEqualTo( TxCount + Org.Neo4j.Kernel.impl.transaction.log.TransactionIdStore_Fields.BASE_TX_ID ) );
					return false;
			  }
		 }
