...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldSendExceptionBackToClientOnInvalidChecksum()
		 {
			  ...
			  try
			  {
					_protocol.deserializeResponse( _channel.asBlockingReadHandler(), ByteBuffer.allocate((int) ByteUnit.kibiBytes(1)), 1, VOID_DESERIALIZER, mock(typeof(ResourceReleaser)), new VersionAwareLogEntryReader<Org.Neo4j.Kernel.impl.transaction.log.ReadableClosablePositionAwareChannel>() );
					fail( "Should have failed." );
			  }
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldNotSendExceptionBackToClientOnInvalidChecksumIfThereAreNoTransactions()
		 {
			  // Given
			  Server<object, object> server = NewServer( _checksumVerifier );
			  RequestContext ctx = new RequestContext( 0, 1, 0, Org.Neo4j.Kernel.impl.transaction.log.TransactionIdStore_Fields.BASE_TX_ID, Org.Neo4j.Kernel.impl.transaction.log.TransactionIdStore_Fields.BASE_TX_CHECKSUM );
...
//ORIGINAL LINE: private Server<Object,Object> newServer(final TxChecksumVerifier checksumVerifier)
		 ...
		 private class ServerAnonymousInnerClass : Server<object, object>
		 {
			 ...
			 public ServerAnonymousInnerClass( ServerTest outerInstance, Org.Neo4j.com.Server.Configuration conf, UnknownType getInstance, UnknownType defaultFrameLength, Org.Neo4j.com.TxChecksumVerifier checksumVerifier, java.time.Clock systemClock, UnknownType mock, UnknownType mock ) : base( null, conf, getInstance, defaultFrameLength, new ProtocolVersion( ( sbyte ) 0, ProtocolVersion.INTERNAL_PROTOCOL_VERSION ), checksumVerifier, systemClock, mock, mock )
			 {
