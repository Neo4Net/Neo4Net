...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: entered { @Override public ClusterState handle(ClusterContext context, org.neo4j.cluster.com.message.Message<ClusterMessage> message, org.neo4j.cluster.com.message.MessageHolder outgoing) { switch(message.getMessageType()) { case addClusterListener: { context.addClusterListener(message.getPayload()); break; } case removeClusterListener: { context.removeClusterListener(message.getPayload()); break; } case configurationRequest: { ClusterMessage.ConfigurationRequestState request = message.getPayload(); request = new ClusterMessage.ConfigurationRequestState(request.getJoiningId(), java.net.URI.create(message.getHeader(org.neo4j.cluster.com.message.Message.HEADER_FROM))); org.neo4j.cluster.InstanceId joiningId = request.getJoiningId(); java.net.URI joiningUri = request.getJoiningUri(); boolean isInCluster = context.getMembers().containsKey(joiningId); boolean isCurrentlyAlive = context.isCurrentlyAlive(joiningId); boolean messageComesFromSameHost = request.getJoiningId().equals(context.getMyId()); boolean otherInstanceJoiningWithSameId = context.isInstanceJoiningFromDifferentUri(joiningId, joiningUri); boolean isFromSameURIAsTheOneWeAlreadyKnow = context.getUriForId(joiningId) != null && context.getUriForId(joiningId).equals(joiningUri); boolean somethingIsWrong = (isInCluster && !messageComesFromSameHost && isCurrentlyAlive && !isFromSameURIAsTheOneWeAlreadyKnow) || otherInstanceJoiningWithSameId ; if(somethingIsWrong) { if(otherInstanceJoiningWithSameId) { context.getLog(ClusterState.class).info(format("Denying entry to instance %s" + " because another instance is currently joining with the same id.", joiningId)); } else { context.getLog(ClusterState.class).info(format("Denying entry to " + "instance %s because that instance is already in the cluster.", joiningId)); } outgoing.offer(message.copyHeadersTo(respond(ClusterMessage.joinDenied, message, new ClusterMessage.ConfigurationResponseState(context.getConfiguration().getRoles(), context.getConfiguration().getMembers(), new org.neo4j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId(context.getLastDeliveredInstanceId()), context.getFailedInstances(), context.getConfiguration().getName())))); } else { context.instanceIsJoining(joiningId, joiningUri); outgoing.offer(message.copyHeadersTo(respond(ClusterMessage.configurationResponse, message, new ClusterMessage.ConfigurationResponseState(context.getConfiguration().getRoles(), context.getConfiguration().getMembers(), new org.neo4j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId(context.getLastDeliveredInstanceId()), context.getFailedInstances(), context.getConfiguration().getName())))); } break; } case configurationChanged: { ClusterMessage.ConfigurationChangeState state = message.getPayload(); state.apply(context); break; } case leave: { java.util.List<java.net.URI> nodeList = new java.util.ArrayList<>(context.getConfiguration().getMemberURIs()); if(nodeList.size() == 1) { context.getLog(ClusterState.class).info(format("Shutting down cluster: %s", context.getConfiguration().getName())); context.left(); return start; } else { context.getLog(ClusterState.class).info(format("Leaving:%s", nodeList)); ClusterMessage.ConfigurationChangeState newState = new ClusterMessage.ConfigurationChangeState(); newState.leave(context.getMyId()); outgoing.offer(internal(org.neo4j.cluster.protocol.atomicbroadcast.multipaxos.AtomicBroadcastMessage.broadcast, newState)); context.setTimeout("leave", timeout(ClusterMessage.leaveTimedout, message)); return leaving; } } default: break; } return this; } },
		 entered
		 {
			 public ClusterState handle( ClusterContext context, Message<ClusterMessage> message, MessageHolder outgoing )
			 {
				 switch ( message.MessageType )
				 {
					 case addClusterListener: { context.addClusterListener( message.Payload ); break; } case removeClusterListener: { context.removeClusterListener( message.Payload ); break; } case configurationRequest:
					 {
						 ...
						 {
							 if ( otherInstanceJoiningWithSameId ) { context.getLog( typeof( ClusterState ) ).info( format( "Denying entry to instance %s" + " because another instance is currently joining with the same id.", joiningId ) ); } else { context.getLog( typeof( ClusterState ) ).info( format( "Denying entry to " + "instance %s because that instance is already in the cluster.", joiningId ) ); } outgoing.offer( message.copyHeadersTo( respond( ClusterMessage.JoinDenied, message, new ClusterMessage.ConfigurationResponseState( context.Configuration.Roles, context.Configuration.Members, new Org.Neo4j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId( context.LastDeliveredInstanceId ), context.FailedInstances, context.Configuration.Name ) ) ) );
						 }
						 else { context.instanceIsJoining( joiningId, joiningUri ); outgoing.offer( message.copyHeadersTo( respond( ClusterMessage.ConfigurationResponse, message, new ClusterMessage.ConfigurationResponseState( context.Configuration.Roles, context.Configuration.Members, new Org.Neo4j.cluster.protocol.atomicbroadcast.multipaxos.InstanceId( context.LastDeliveredInstanceId ), context.FailedInstances, context.Configuration.Name ) ) ) ); } break;
					 }
