	...
	internal class ClusterContextImpl : AbstractContextImpl, ClusterContext
	{
		 ...
		 private void InvalidateElectorIfNecessary( InstanceId server )
		 {
			  if ( server.Equals( _lastElector ) )
			  {
					...
					_electorVersion = Org.Neo4j.cluster.protocol.cluster.ClusterContext_Fields.NO_ELECTOR_VERSION;
			  }
		 }
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are ignored unless the option to convert to C# 7.2 'in' parameters is selected:
//ORIGINAL LINE: public void elected(final String roleName, final org.neo4j.cluster.InstanceId instanceId)
		 public override void Elected( string roleName, InstanceId instanceId )
		 {
			  Elected( roleName, instanceId, InstanceId.NONE, Org.Neo4j.cluster.protocol.cluster.ClusterContext_Fields.NO_ELECTOR_VERSION );
		 }
...
//JAVA TO C# CONVERTER WARNING: 'final' parameters are ignored unless the option to convert to C# 7.2 'in' parameters is selected:
//ORIGINAL LINE: public void unelected(final String roleName, final org.neo4j.cluster.InstanceId instanceId)
		 public override void Unelected( string roleName, InstanceId instanceId )
		 {
			  Unelected( roleName, instanceId, InstanceId.NONE, Org.Neo4j.cluster.protocol.cluster.ClusterContext_Fields.NO_ELECTOR_VERSION );
		 }
