...
namespace Org.Neo4j.Server.security.enterprise.auth
{
	...
	using InitialUserTest = Org.Neo4j.Server.Security.Auth.InitialUserTest;
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldFailWhenAuthTokenIsInvalid()
		 {
			  ...
			  assertException( () => _manager.login(map(Org.Neo4j.Kernel.api.security.AuthToken_Fields.SCHEME_KEY, "supercool", Org.Neo4j.Kernel.api.security.AuthToken_Fields.PRINCIPAL, "neo4j")), typeof(InvalidAuthTokenException), "Unsupported authentication token: { scheme='supercool', principal='neo4j' }" );

			  assertException( () => _manager.login(map(Org.Neo4j.Kernel.api.security.AuthToken_Fields.SCHEME_KEY, "none")), typeof(InvalidAuthTokenException), "Unsupported authentication token, scheme='none' only allowed when auth is disabled: { scheme='none' }" );
			  ...
			  assertException( () => _manager.login(map(Org.Neo4j.Kernel.api.security.AuthToken_Fields.SCHEME_KEY, "basic", Org.Neo4j.Kernel.api.security.AuthToken_Fields.PRINCIPAL, "neo4j")), typeof(InvalidAuthTokenException), "Unsupported authentication token, missing key `credentials`: { scheme='basic', principal='neo4j' }" );

			  assertException( () => _manager.login(map(Org.Neo4j.Kernel.api.security.AuthToken_Fields.SCHEME_KEY, "basic", Org.Neo4j.Kernel.api.security.AuthToken_Fields.CREDENTIALS, "very-secret")), typeof(InvalidAuthTokenException), "Unsupported authentication token, missing key `principal`: { scheme='basic', credentials='******' }" );
		 }
