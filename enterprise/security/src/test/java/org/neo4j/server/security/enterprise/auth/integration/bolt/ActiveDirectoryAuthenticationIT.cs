...
//ORIGINAL LINE: private void assertAuthFail(String username, String password) throws Exception
		 private void AssertAuthFail( string username, string password )
		 {
			  ...
			  assertThat( _client, eventuallyReceives( new sbyte[]{ 0, 0, 0, 1 } ) );
			  assertThat( _client, _util.eventuallyReceives( msgFailure( Org.Neo4j.Kernel.Api.Exceptions.Status_Security.Unauthorized, "The client is unauthorized due to authentication failure." ) ) );
		 }
...
//ORIGINAL LINE: protected void assertReadFails(String username) throws Exception
		 protected internal virtual void AssertReadFails( string username )
		 {
			  ...
			  // Then
			  assertThat( _client, _util.eventuallyReceives( msgFailure( Org.Neo4j.Kernel.Api.Exceptions.Status_Security.Forbidden, string.Format( "Read operations are not allowed for user {0}.", username ) ) ) );
		 }
...
//ORIGINAL LINE: protected void assertWriteFails(String username) throws Exception
		 protected internal virtual void AssertWriteFails( string username )
		 {
			  ...
			  // Then
			  assertThat( _client, _util.eventuallyReceives( msgFailure( Org.Neo4j.Kernel.Api.Exceptions.Status_Security.Forbidden, string.Format( "Write operations are not allowed for user {0}.", username ) ) ) );
		 }
