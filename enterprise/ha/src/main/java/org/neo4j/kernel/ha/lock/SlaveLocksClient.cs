...
//	import static org.neo4j.kernel.impl.locking.LockType.WRITE;
	...
	/// </summary>
	internal class SlaveLocksClient : Org.Neo4j.Kernel.impl.locking.Locks_Client
	{
		 private readonly Master _master;
		 private readonly Org.Neo4j.Kernel.impl.locking.Locks_Client _client;
...
//ORIGINAL LINE: java.util.List<? extends org.neo4j.kernel.impl.locking.ActiveLock> activeLocks = client.activeLocks().filter(activeLock -> org.neo4j.kernel.impl.locking.ActiveLock_Fields.SHARED_MODE.equals(activeLock.mode())).filter(this::isLabelOrRelationshipType).collect(java.util.stream.Collectors.toList());
			  IList<ActiveLock> activeLocks = _client.activeLocks().filter(activeLock => Org.Neo4j.Kernel.impl.locking.ActiveLock_Fields.SHARED_MODE.Equals(activeLock.mode())).filter(this.isLabelOrRelationshipType).collect(Collectors.toList());
		 ...
		 private RequestContext NewRequestContextFor( Org.Neo4j.Kernel.impl.locking.Locks_Client client )
		 {
