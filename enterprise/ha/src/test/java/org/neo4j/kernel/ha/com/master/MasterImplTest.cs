...
namespace Org.Neo4j.Kernel.ha.com.master
{
	...
	using Locks_Client = Org.Neo4j.Kernel.impl.locking.Locks_Client;
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 ...
		 {
			  when( spi.PackEmptyResponse( any() ) ).thenAnswer(invocation => new TransactionObligationResponse<>(invocation.getArgument(0), StoreId.DEFAULT, Org.Neo4j.Kernel.impl.transaction.log.TransactionIdStore_Fields.BASE_TX_ID, Org.Neo4j.com.ResourceReleaser_Fields.NoOp));
		 }
