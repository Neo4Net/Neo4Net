...
namespace Org.Neo4j.Kernel.impl.store.format.highlimit.v300
{
	...
	using CompositePageCursor = Org.Neo4j.Io.pagecache.impl.CompositePageCursor;
...
//	import static org.neo4j.kernel.impl.store.RecordPageLocationCalculator.pageIdForRecord;
	...
	/// higher level entity write locks and so the <seealso cref="PageCursor"/> can simply move from the first on to the second
	...
	internal abstract class BaseHighLimitRecordFormatV3_0_0<RECORD> : BaseOneByteHeaderRecordFormat<RECORD> where RECORD : Org.Neo4j.Kernel.impl.store.record.AbstractBaseRecord
	{
		 private static readonly int _headerByte = Byte.BYTES;
		 ...
		 protected internal BaseHighLimitRecordFormatV3_0_0( System.Func<StoreHeader, int> recordSize, int recordHeaderSize ) : base( recordSize, recordHeaderSize, IN_USE_BIT, HighLimitV3_0_0.DEFAULT_MAXIMUM_BITS_PER_ID )
		 {
		 }
...
//ORIGINAL LINE: public void read(RECORD record, org.neo4j.io.pagecache.PageCursor primaryCursor, org.neo4j.kernel.impl.store.record.RecordLoad mode, int recordSize) throws java.io.IOException
		 public override void Read( RECORD record, PageCursor primaryCursor, RecordLoad mode, int recordSize )
		 {
			  ...
			  if ( doubleRecordUnit )
			  {
					...
					secondaryCursor.Offset = offset + _headerByte;
					...
					int secondarySize = recordSize - _headerByte;
		 ...
		 public override void Write( RECORD record, PageCursor primaryCursor, int recordSize )
		 {
			  if ( record.inUse() )
			  {
					...
					if ( record.requiresSecondaryUnit() )
					{
						 ...
						 int recordSizeWithoutHeader = recordSize - _headerByte;
