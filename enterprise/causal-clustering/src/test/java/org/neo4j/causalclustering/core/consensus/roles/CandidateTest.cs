...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldDeclineVoteRequestsIfFromSameTerm()
		 {
			  ...
			  // then
			  assertThat( outcome.OutgoingMessages, hasItem( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_Directed( _member1, voteResponse().term(raftState.Term()).from(_myself).deny().build() ) ) );
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldBecomeFollowerIfReceiveVoteRequestFromLaterTerm()
		 {
			  ...
			  assertThat( outcome.OutgoingMessages, hasItem( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_Directed( _member1, voteResponse().term(newTerm).from(_myself).grant().build() ) ) );
		 }
