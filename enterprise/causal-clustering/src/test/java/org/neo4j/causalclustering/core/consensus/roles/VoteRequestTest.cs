...
//ORIGINAL LINE: final long candidateTerm = state.term() + 1;
			  ...
			  // then
			  assertTrue( ( ( Org.Neo4j.causalclustering.core.consensus.RaftMessages_Vote_Response ) messageFor( outcome, _member1 ) ).voteGranted() );
		 }
...
//ORIGINAL LINE: final long candidateTerm = state.term() + 1;
			  ...
			  // then
			  assertFalse( ( ( Org.Neo4j.causalclustering.core.consensus.RaftMessages_Vote_Response ) messageFor( outcome2, _member2 ) ).voteGranted() );
		 }
