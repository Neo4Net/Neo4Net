...
namespace Org.Neo4j.causalclustering.core.consensus.shipping
{
	...
	using Matchers = Org.Neo4j.Test.matchers.Matchers;
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in C#:
		 public virtual void ShouldSendLogCompactionInfoToFollowerOnMatchIfEntryHasBeenPrunedAway()
		 {
			  ...
			  assertThat( _outbound.sentTo( _follower ), hasMessage( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_LogCompactionInfo( _leader, 0, 2 ) ) );
		 }
