...
//JAVA TO C# CONVERTER TODO TASK: This Java 'import static' statement cannot be converted to C#:
	using static Org.Neo4j.causalclustering.protocol.Protocol_ApplicationProtocol;
//JAVA TO C# CONVERTER TODO TASK: This Java 'import static' statement cannot be converted to C#:
	using static Org.Neo4j.causalclustering.protocol.Protocol_ApplicationProtocolCategory;
//JAVA TO C# CONVERTER TODO TASK: This Java 'import static' statement cannot be converted to C#:
	using static Org.Neo4j.causalclustering.protocol.Protocol_ModifierProtocolCategory;
//JAVA TO C# CONVERTER TODO TASK: This Java 'import static' statement cannot be converted to C#:
...
//	import static org.neo4j.causalclustering.protocol.handshake.TestProtocols_TestModifierProtocols.SNAPPY;
	...
	public class HandshakeClientTest
	{
		...
		private void InitializeInstanceFields()
		{
		 ...
		 private ICollection<ModifierSupportedProtocols> _supportedModifierProtocols = Stream.of( Org.Neo4j.causalclustering.protocol.Protocol_ModifierProtocolCategory.values() ).map(id => new ModifierSupportedProtocols(id, emptyList())).collect(Collectors.toList());
...
//ORIGINAL LINE: @Test public void shouldSendSwitchOverRequestOnModifierProtocolResponseIfAllModifierProtocolResponsesReceived()
		 public virtual void ShouldSendSwitchOverRequestOnModifierProtocolResponseIfAllModifierProtocolResponsesReceived()
		 {
			  ...
			  // when
			  _client.handle( new ModifierProtocolResponse( StatusCode.Success, ModifierProtocolCategory.COMPRESSION.canonicalName(), SNAPPY.implementation() ) );
			  _client.handle( new ModifierProtocolResponse( StatusCode.Success, Org.Neo4j.causalclustering.protocol.Protocol_ModifierProtocolCategory.GratuitousObfuscation.canonicalName(), ROT13.implementation() ) );
...
//ORIGINAL LINE: @Test public void shouldNotIncludeModifierProtocolInSwitchOverRequestIfUnsupportedProtocol()
		 public virtual void ShouldNotIncludeModifierProtocolInSwitchOverRequestIfUnsupportedProtocol()
		 {
			  ...
			  // then
			  IList<Pair<string, string>> switchOverModifierProtocols = new IList<Pair<string, string>> { Pair.of( Org.Neo4j.causalclustering.protocol.Protocol_ModifierProtocolCategory.Compression.canonicalName(), SNAPPY.implementation() ) };
			  verify( _channel ).writeAndFlush( new SwitchOverRequest( _applicationProtocolIdentifier.canonicalName(), _raftVersion, switchOverModifierProtocols ) );
			  assertFalse( protocolStackCompletableFuture.Done );
		 }
