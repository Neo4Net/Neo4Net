...
//ORIGINAL LINE: @Test public void shouldReturnEmptyIfUnknownVersion()
		 ...
		 {
			  // when
			  Optional<Org.Neo4j.causalclustering.protocol.Protocol_ApplicationProtocol> applicationProtocol = _applicationProtocolRepository.select( RAFT.canonicalName(), -1 );
...
//ORIGINAL LINE: @Test(expected = IllegalArgumentException.class) public void shouldNotInstantiateIfDuplicateProtocolsSupplied()
		 ...
		 private class Protocol_ApplicationProtocolAnonymousInnerClass : Org.Neo4j.causalclustering.protocol.Protocol_ApplicationProtocol
		 {
