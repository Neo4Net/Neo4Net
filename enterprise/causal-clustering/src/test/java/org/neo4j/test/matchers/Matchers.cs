...
//JAVA TO C# CONVERTER WARNING: Java wildcard generics have no direct equivalent in .NET:
		 public static Matcher<object> HasMessage( Org.Neo4j.causalclustering.core.consensus.RaftMessages_BaseRaftMessage message )
		 {
			  return new TypeSafeMatcherAnonymousInnerClass( message );
		 }

		 private class TypeSafeMatcherAnonymousInnerClass : TypeSafeMatcher<IList<Org.Neo4j.causalclustering.core.consensus.RaftMessages_RaftMessage>>
		 {
			 private Org.Neo4j.causalclustering.core.consensus.RaftMessages_BaseRaftMessage _message;

			 public TypeSafeMatcherAnonymousInnerClass( Org.Neo4j.causalclustering.core.consensus.RaftMessages_BaseRaftMessage message )
			 {
				 this._message = message;
			 }

			 protected internal override bool matchesSafely( IList<Org.Neo4j.causalclustering.core.consensus.RaftMessages_RaftMessage> messages )
			 {
...
//JAVA TO C# CONVERTER WARNING: Java wildcard generics have no direct equivalent in .NET:
		 ...
		 private class TypeSafeMatcherAnonymousInnerClass2 : TypeSafeMatcher<IList<Org.Neo4j.causalclustering.core.consensus.RaftMessages_RaftMessage>>
		 {
			 ...
			 protected internal override bool matchesSafely( IList<Org.Neo4j.causalclustering.core.consensus.RaftMessages_RaftMessage> messages )
			 {
				  IList<RaftLogEntry> entries = messages.Where( message => message is Org.Neo4j.causalclustering.core.consensus.RaftMessages_AppendEntries_Request ).Select( m => ( Org.Neo4j.causalclustering.core.consensus.RaftMessages_AppendEntries_Request ) m ).flatMap( x => Arrays.stream( x.entries() ) ).ToList();
