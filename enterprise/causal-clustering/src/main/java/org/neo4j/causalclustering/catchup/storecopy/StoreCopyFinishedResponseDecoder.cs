...
namespace Org.Neo4j.causalclustering.catchup.storecopy
{
	...
	using Status = Org.Neo4j.causalclustering.catchup.storecopy.StoreCopyFinishedResponse.Status;

	public class StoreCopyFinishedResponseDecoder : ByteToMessageDecoder
	{
		 protected internal override void Decode( ChannelHandlerContext ctx, ByteBuf msg, IList<object> @out )
		 {
			  ...
			  @out.Add( new StoreCopyFinishedResponse( Enum.GetValues( typeof( Status ) )[statusOrdinal] ) );
		 }
	}
