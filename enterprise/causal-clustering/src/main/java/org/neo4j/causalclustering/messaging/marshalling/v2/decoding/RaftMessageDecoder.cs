...
//ORIGINAL LINE: public void decode(io.netty.channel.ChannelHandlerContext ctx, io.netty.buffer.ByteBuf buffer, java.util.List<Object> list) throws Exception
		 public override void Decode( ChannelHandlerContext ctx, ByteBuf buffer, IList<object> list )
		 {
			  ...
			  if ( messageType.Equals( VOTE_REQUEST ) )
			  {
					...
					composer = new SimpleMessageComposer( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_Vote_Request( from, term, candidate, lastLogIndex, lastLogTerm ) );
			  }
			  else if ( messageType.Equals( VOTE_RESPONSE ) )
			  {
					...
					composer = new SimpleMessageComposer( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_Vote_Response( from, term, voteGranted ) );
			  }
			  else if ( messageType.Equals( PRE_VOTE_REQUEST ) )
			  {
					...
					composer = new SimpleMessageComposer( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_PreVote_Request( from, term, candidate, lastLogIndex, lastLogTerm ) );
			  }
			  else if ( messageType.Equals( PRE_VOTE_RESPONSE ) )
			  {
					...
					composer = new SimpleMessageComposer( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_PreVote_Response( from, term, voteGranted ) );
			  }
			  ...
			  else if ( messageType.Equals( APPEND_ENTRIES_RESPONSE ) )
			  {
					...
					composer = new SimpleMessageComposer( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_AppendEntries_Response( from, term, success, matchIndex, appendIndex ) );
			  }
			  ...
			  else if ( messageType.Equals( HEARTBEAT ) )
			  {
					...
					composer = new SimpleMessageComposer( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_Heartbeat( from, leaderTerm, commitIndex, commitIndexTerm ) );
			  }
			  else if ( messageType.Equals( HEARTBEAT_RESPONSE ) )
			  {
					composer = new SimpleMessageComposer( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_HeartbeatResponse( from ) );
			  }
			  else if ( messageType.Equals( LOG_COMPACTION_INFO ) )
			  {
					...
					composer = new SimpleMessageComposer( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_LogCompactionInfo( from, leaderTerm, prevIndex ) );
			  }
			  else
			  {
...
//ORIGINAL LINE: private org.neo4j.causalclustering.identity.MemberId retrieveMember(org.neo4j.storageengine.api.ReadableChannel buffer) throws java.io.IOException, org.neo4j.causalclustering.messaging.EndOfStreamException
		 ...
		 internal interface LazyComposer
		 {
			  ...
			  /// </summary>
			  Optional<Org.Neo4j.causalclustering.core.consensus.RaftMessages_RaftMessage> MaybeComplete( LinkedList<long> terms, LinkedList<ReplicatedContent> contents );
		 }
		 ...
		 {
			  internal readonly Org.Neo4j.causalclustering.core.consensus.RaftMessages_RaftMessage Message;

			  internal SimpleMessageComposer( Org.Neo4j.causalclustering.core.consensus.RaftMessages_RaftMessage message )
			  {
		 ...
		 private class NewEntryRequestComposer : LazyComposer
		 {
			  ...
			  public override Optional<Org.Neo4j.causalclustering.core.consensus.RaftMessages_RaftMessage> MaybeComplete( LinkedList<long> terms, LinkedList<ReplicatedContent> contents )
			  {
					...
					{
						 return ( new Org.Neo4j.causalclustering.core.consensus.RaftMessages_NewEntry_Request( From, contents.RemoveFirst() ) );
					}
			  }
		 }
	}
