 ...
 */
namespace Org.Neo4j.causalclustering.logging
{
...
//           OUTBOUND("-->"),
		 ...
		 public override void LogOutbound<M>( MEMBER me, M message, MEMBER remote ) where M : Org.Neo4j.causalclustering.core.consensus.RaftMessages_RaftMessage
		 {
			  Log( me, Direction.Outbound, remote, NullSafeMessageType( message ), valueOf( message ) );
		 }

		 public override void LogInbound<M>( MEMBER remote, M message, MEMBER me ) where M : Org.Neo4j.causalclustering.core.consensus.RaftMessages_RaftMessage
		 {
			  Log( me, Direction.Inbound, remote, NullSafeMessageType( message ), valueOf( message ) );
		 }

		 private string NullSafeMessageType<M>( M message ) where M : Org.Neo4j.causalclustering.core.consensus.RaftMessages_RaftMessage
		 {
