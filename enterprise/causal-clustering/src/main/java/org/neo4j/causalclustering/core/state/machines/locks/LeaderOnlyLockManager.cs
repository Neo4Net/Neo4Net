...
//	import static org.neo4j.kernel.api.exceptions.Status_Transaction.Interrupted;
	...
	public class LeaderOnlyLockManager : Locks
	{
		 ...
		 /// </summary>
		 private class LeaderOnlyLockClient : Org.Neo4j.Kernel.impl.locking.Locks_Client
		 {
			 private readonly LeaderOnlyLockManager _outerInstance;

			  internal readonly Org.Neo4j.Kernel.impl.locking.Locks_Client LocalClient;
			  internal int LockTokenId = LockToken_Fields.INVALID_LOCK_TOKEN_ID;

			  internal LeaderOnlyLockClient( LeaderOnlyLockManager outerInstance, Org.Neo4j.Kernel.impl.locking.Locks_Client localClient )
			  {
