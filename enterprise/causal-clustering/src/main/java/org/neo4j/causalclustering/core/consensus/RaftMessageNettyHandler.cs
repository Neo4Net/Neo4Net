...
//ORIGINAL LINE: private org.neo4j.causalclustering.messaging.Inbound_MessageHandler<RaftMessages_ReceivedInstantClusterIdAwareMessage<?>> actual;
		 private Org.Neo4j.causalclustering.messaging.Inbound_MessageHandler<RaftMessages_ReceivedInstantClusterIdAwareMessage<object>> _actual;
		 private Log _log;
		 ...
		 public override void RegisterHandler<T1>( Org.Neo4j.causalclustering.messaging.Inbound_MessageHandler<T1> actual )
		 {
			  this._actual = actual;
		 }
