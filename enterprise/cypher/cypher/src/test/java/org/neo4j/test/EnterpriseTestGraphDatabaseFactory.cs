...
//ORIGINAL LINE: public static final System.Predicate<org.neo4j.kernel.extension.KernelExtensionFactory<?>> INDEX_PROVIDERS_FILTER = extension -> extension instanceof org.neo4j.kernel.impl.index.schema.AbstractIndexProviderFactory;
		 ...
		 public EnterpriseTestGraphDatabaseFactory( LogProvider logProvider ) : base( new TestGraphDatabaseFactoryState() )
		 {
...
//ORIGINAL LINE: org.neo4j.graphdb.config.Setting<?> key = entry.getKey();
		 ...
		 public virtual EnterpriseTestGraphDatabaseFactory setFileSystem( FileSystemAbstraction fileSystem )
		 {
			  CurrentState.FileSystem = fileSystem;
			  return this;
		 }

		 public override EnterpriseTestGraphDatabaseFactory setMonitors( Monitors monitors )
		 {
			  CurrentState.Monitors = monitors;
			  return this;
		 }

		 public override EnterpriseTestGraphDatabaseFactory setUserLogProvider( LogProvider logProvider )
		 {
			  return ( EnterpriseTestGraphDatabaseFactory ) base.setUserLogProvider( logProvider );
		 }

		 public virtual EnterpriseTestGraphDatabaseFactory setInternalLogProvider( LogProvider logProvider )
		 {
			  CurrentState.InternalLogProvider = logProvider;
			  return this;
		 }

		 public virtual EnterpriseTestGraphDatabaseFactory setClock( SystemNanoClock clock )
		 {
			  CurrentState.Clock = clock;
			  return this;
		 }

		 public virtual EnterpriseTestGraphDatabaseFactory AddKernelExtensions<T1>( IEnumerable<T1> newKernelExtensions )
		 {
			  CurrentState.addKernelExtensions( newKernelExtensions );
			  return this;
		 }

		 public virtual EnterpriseTestGraphDatabaseFactory AddKernelExtension<T1>( KernelExtensionFactory<T1> newKernelExtension )
		 {
			  return AddKernelExtensions( Collections.singletonList( newKernelExtension ) );
		 }

		 public virtual EnterpriseTestGraphDatabaseFactory setKernelExtensions<T1>( IEnumerable<T1> newKernelExtensions )
		 {
			  CurrentState.KernelExtensions = newKernelExtensions;
			  return this;
		 }

		 public virtual EnterpriseTestGraphDatabaseFactory RemoveKernelExtensions<T1>( System.Predicate<T1> filter )
		 {
			  CurrentState.removeKernelExtensions( filter );
			  return this;
		 }

		 public override EnterpriseTestGraphDatabaseFactory AddURLAccessRule( string protocol, URLAccessRule rule )
		 {
			  return ( EnterpriseTestGraphDatabaseFactory ) base.AddURLAccessRule( protocol, rule );
		 }
...
//ORIGINAL LINE: protected org.neo4j.graphdb.factory.GraphDatabaseBuilder.DatabaseCreator createImpermanentDatabaseCreator(final java.io.File storeDir, final TestGraphDatabaseFactoryState state)
		 ...
		 {
			 private readonly EnterpriseTestGraphDatabaseFactory _outerInstance;

			 private File _storeDir;
			 private Org.Neo4j.Test.TestGraphDatabaseFactoryState _state;

			 public DatabaseCreatorAnonymousInnerClass( EnterpriseTestGraphDatabaseFactory outerInstance, File storeDir, Org.Neo4j.Test.TestGraphDatabaseFactoryState state )
			 {
...
//ORIGINAL LINE: public org.neo4j.graphdb.GraphDatabaseService newDatabase(@Nonnull Config config)
		 ...
		 public class TestGraphDatabaseFacadeFactory : GraphDatabaseFacadeFactory
		 {
			  ...
			  protected internal TestGraphDatabaseFacadeFactory( TestGraphDatabaseFactoryState state, bool impermanent ) : this( state, impermanent, DatabaseInfo.ENTERPRISE, EnterpriseEditionModule::new )
			  {
//JAVA TO C# CONVERTER TODO TASK: Method reference constructor syntax is not converted by Java to C# Converter:
			  ...
			  {
				  private readonly EnterpriseTestGraphDatabaseFactory.TestGraphDatabaseFacadeFactory _outerInstance;


					internal TestDatabasePlatformModule( EnterpriseTestGraphDatabaseFactory.TestGraphDatabaseFacadeFactory outerInstance, File storeDir, Config config, Dependencies dependencies, DatabaseInfo databaseInfo ) : base( storeDir, config, databaseInfo, dependencies )
					{
			  ...
			  private class ImpermanentTestDatabasePlatformModule : TestDatabasePlatformModule
			  {
				  private readonly EnterpriseTestGraphDatabaseFactory.TestGraphDatabaseFacadeFactory _outerInstance;
					...
					internal ImpermanentTestDatabasePlatformModule( EnterpriseTestGraphDatabaseFactory.TestGraphDatabaseFacadeFactory outerInstance, File storeDir, Config config, Dependencies dependencies, DatabaseInfo databaseInfo ) : base( outerInstance, storeDir, config, dependencies, databaseInfo )
					{
						this._outerInstance = outerInstance;
					}
